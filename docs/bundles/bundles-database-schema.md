...
На уровне API/кода числовой kind мапится в `ProductKind` (`0` → `BASE`, `1` → `BUNDLE`).
...
import { foreignKey, index, integer, uniqueIndex, uuid } from "drizzle-orm/pg-core";
...
export const variant = catalogSchema.table(
  "variant",
  {
    ...
    kind: integer("kind").notNull().default(0), // must equal parent product.kind
  },
  (table) => [
