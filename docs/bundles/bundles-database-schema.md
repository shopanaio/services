...
Bundle — это продукт с `product.kind = 2` и отдельной 1:1 записью в таблице `bundle`.
Варианты продукта хранят тот же discriminator в `variant.kind`; DB строго гарантирует
`variant.kind = product.kind` через составной FK.
На уровне API/кода числовой kind мапится в `ProductKind` (`1` → `BASE`, `2` → `BUNDLE`).
...
```typescript
enum ProductKind {
  BASE = "BASE",     // DB value: 1
  BUNDLE = "BUNDLE", // DB value: 2
}
...
import { check, foreignKey, index, integer, uniqueIndex, uuid } from "drizzle-orm/pg-core";

export const product = catalogSchema.table(
  "product",
  {
    // existing product columns...
    kind: integer("kind").notNull().default(1), // 1 = BASE, 2 = BUNDLE
  },
  (table) => [
    check("product_kind_positive", sql`kind > 0`),
    // Required so variant can enforce product_id + kind as a strict composite FK.
...
export const variant = catalogSchema.table(
  "variant",
  {
    ...
    kind: integer("kind").notNull().default(1), // must equal parent product.kind
  },
  (table) => [
...
configuration joins explicit. It must never diverge from the parent product kind; the
database enforces this with the composite FK above.
...
|--------|----------|-----------|
| **Product kind** | `product.kind = 1 \| 2 \| ...` in DB, mapped to API enum | DB stores compact positive numeric discriminator; API/code exposes `BASE` / `BUNDLE` |
