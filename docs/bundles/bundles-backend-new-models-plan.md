# План миграции backend бандлов на новые модели
...
Его нужно рассматривать как compatibility surface, который надо заменить или мигрировать.
...
- Drizzle models соответствуют `bundles-database-schema.md`.
- Любое оставшееся legacy-поле в `bundle.ts` либо удалено, либо явно замаплено через шаг миграции.

## Фаза 2: Миграция базы данных

Файлы:

- catalog Drizzle migration output
- существующая папка catalog migrations

Задачи:

1. Создать явную стратегию миграции из legacy tables в новые tables.
2. Если существующие данные можно мигрировать:
   - создать одну строку `bundle` на каждый legacy bundle product;
   - создать default `bundle_configuration` для каждого bundle;
   - перенести legacy product-scoped groups/templates/rules под эту configuration;
   - создать translations из legacy `title` fields, используя store default locale;
   - создать price rules из legacy `priceType` и `priceValue`;
   - назначить существующие bundle variants через `bundle_configuration_variant`, где это возможно.
3. Если существующим данным нельзя доверять:
   - задокументировать destructive migration path;
   - требовать явное подтверждение перед удалением legacy bundle data.
4. Генерировать migration через project command, не редактируя generated migration output вручную.

Проверка:

- Запустить schema generation через project tooling, когда меняется model.
- Не запускать tests или `tsc`.
...
- Избегать повторных DB reads для `BundleConfiguration.groups`, `pricingTemplates` и `dependencyRules`.
...
## Фаза 8: Product and Variant integration
...
## Фаза 12: Pricing and Currency rules
...
1. Нормализовать bundle price types между GraphQL, scripts и DB.
...
## Фаза 13: Events and Search Index
...
## Фаза 14: Codegen, Build and Verification
...
Если нужны migration files:

1. `yarn workspace @shopana/catalog db:generate` или эквивалентная project command.
2. Проверить generated migration output.
3. Не редактировать changeset files вручную.

## Фаза 15: Rollout strategy
...
- Drizzle bundle models соответствуют `bundles-database-schema.md`.
