...
**Key idea**: Events are just another way to invoke an action. We use the same `ActionRequest`/`ActionResponse` contract and the same `withIdempotency()` helper.
...
│                                                                             │
│  2. Framework (idempotency): Per-handler tracking                           │
...
9. **Full Audit Trail**: `domain_events` + `dead_letter_queue` tables
