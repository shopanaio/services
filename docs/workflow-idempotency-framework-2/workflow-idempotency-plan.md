...
When a reserved record exceeds its hard TTL, a cleanup job should:

- mark it as failed (to allow retry), or delete it (more aggressive)
- emit metrics and a warning log for visibility

### Stuck Detection & Alerting

A cron job should detect reserved records with stale leases and emit alerts
for investigation (PagerDuty/Slack), plus a gauge for `idempotency.stuck_count`.
...
```typescript
function buildInProgressResponse(existing: IdempotencyRecord): ActionResponse<null> {
...
Idempotency storage is service-owned and must provide:

* payloadHash verification for any existing key
* lease acquisition/expiry to prevent dual execution
* owner-aware complete/fail to avoid stale writes
...
1. Provide a service-local idempotency store:

   * reserve/claim/complete with lease semantics
   * enforce `payloadHash` for existing keys
