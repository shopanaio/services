...
```text
source: kind=source, handle=nike, parent_id=NULL
```

Поэтому такой insert нарушит unique index root values:
...
Catalog migrations в этом проекте handwritten SQL через `node-pg-migrate`.
Не использовать Drizzle migration generation, не добавлять
`services/catalog/migrations/meta/*.json` и не создавать новый migration file
для этого рефакторинга. Так как проект еще без stage/production данных,
структура facet migrations обновляется в существующих domain files:

- `0501_facets__values.sql` - целевая структура `facet_value`,
  `facet_value_translation`, indexes/constraints;
- `0503_facets__relations.sql` - удалить создание
  `facet_value_source_handle` и связанных indexes/constraints.
...
- заменить old unique `facet_value_facet_id_slug_uniq` на target partial unique
  indexes/checks из раздела выше;
...
1. Обновить handwritten catalog migrations в существующих файлах:
   - `services/catalog/migrations/domains/0500_facets/0501_facets__values.sql`;
   - `services/catalog/migrations/domains/0500_facets/0503_facets__relations.sql`.
2. Review SQL migration diff вручную.
...
2. Обновить существующий handwritten SQL
   `services/catalog/migrations/domains/0500_facets/0501_facets__values.sql`
   под целевую структуру `facet_value`.
3. Обновить существующий handwritten SQL
   `services/catalog/migrations/domains/0500_facets/0503_facets__relations.sql`,
   удалив создание `facet_value_source_handle`.
4. Проверить migration diff вручную.

Acceptance:

- models не экспортируют старую source-handle table;
- `0501_facets__values.sql` создает `facet_value` уже с `handle`, `kind`,
  `parent_id` и target indexes/constraints;
- `0503_facets__relations.sql` больше не создает source-handle table.
