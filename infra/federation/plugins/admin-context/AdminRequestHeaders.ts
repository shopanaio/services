...
export function parseAdminRequest(request: Request): {
  ...
  if (authorization) {
    ...
    if (!match || match[1].length > 16_384) {
      throw requestError(401, "ADMIN_ACCESS_TOKEN_INVALID", "Invalid admin access token");
    }
    accessToken = match[1];
  }

  const storeName = singleHeader(request, "x-store-name");
  if (storeName && (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(storeName) || storeName.length > 63)) {
    throw requestError(400, "ADMIN_STORE_NAME_INVALID", "Invalid admin store name");
  }
  const organizationId = singleHeader(request, "x-organization-id");
  if (organizationId && organizationId.length > 255) {
    throw requestError(400, "ADMIN_ORGANIZATION_ID_INVALID", "Invalid admin organization ID");
  }
  return {
...
export function parseAdminRequestId(request: Request): string | undefined {
  const requestId = singleHeader(request, ADMIN_REQUEST_ID_HEADER);
  if (requestId && requestId.length > 255) {
    throw requestError(400, "ADMIN_REQUEST_ID_INVALID", "Request ID is invalid");
  }
  return requestId;
}
