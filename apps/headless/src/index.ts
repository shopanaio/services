...
export default defineApp({
  ...
  graphql: {
    admin: {
      ...
      handlers: {
        "Query.headlessAppQuery": appGraphQL.handler((_parent, _args, context) => {
          assertHeadlessAdminAccess(context, "read");
          return new QueryResolver({}, createHeadlessResolverContext(context)).headlessAppQuery();
        }),
        "Mutation.headlessAppMutation": appGraphQL.handler((_parent, _args, context) => {
        ...
        "HeadlessStorefrontConnection.__resolveReference": appGraphQL.handler(
        ...
        "StorefrontCredential.__resolveReference": appGraphQL.handler((parent, _args, context) => {
          assertHeadlessAdminAccess(context, "read");
          const reference = parent as { id?: unknown };
          if (typeof reference.id !== "string") {
            return null;
          ...
          try {
            const id = decodeGlobalIdByType(reference.id, GlobalIdEntity.StorefrontCredential);
            return new StorefrontCredentialResolver(id, createHeadlessResolverContext(context));
          } catch {
            return null;
          }
        }),
