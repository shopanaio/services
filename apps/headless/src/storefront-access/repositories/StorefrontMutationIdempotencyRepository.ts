...
export class StorefrontMutationIdempotencyRepository extends BaseRepository {
  async lockAndFind(
    ...
    const lockKey = `${scope.installationId}:${operation}:${clientMutationId}`;
    ...
    const rows = await this.connection
      ...
      .where(
        and(
          eq(storefrontMutationIdempotency.installationId, scope.installationId),
          ...
          eq(storefrontMutationIdempotency.clientMutationId, clientMutationId),
        ),
      )
      .limit(1);
