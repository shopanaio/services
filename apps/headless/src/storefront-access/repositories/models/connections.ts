...
const appInstallationsReference = appsSchema.table(
  "app_installations",
  {
...
export const headlessStorefrontConnections = headlessSchema.table(
  "storefront_connections",
  {
    ...
    status: headlessStorefrontConnectionStatus("status")
      .notNull()
      .default("ACTIVE"),
  ...
  (table) => [
    index("storefront_connections_installation_status_idx").on(
      table.installationId,
      table.status,
    ),
    index("storefront_connections_store_status_idx").on(
      table.storeId,
      table.status,
    ),
    index("storefront_connections_organization_status_idx").on(
...
export type HeadlessStorefrontConnectionModel =
  typeof headlessStorefrontConnections.$inferSelect;
