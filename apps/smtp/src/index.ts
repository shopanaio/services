...
export default defineApp({
  ...
  graphql: {
    admin: {
      ...
      handlers: {
        "Query.smtpAppQuery": appGraphQL.handler((_parent, _args, context) => {
          assertSmtpAdminAccess(context, "read");
          return new QueryResolver({}, createSmtpResolverContext(context)).smtpAppQuery();
        }),
        "Mutation.smtpAppMutation": appGraphQL.handler((_parent, _args, context) => {
          assertSmtpAdminAccess(context, "write");
          return new MutationResolver({}, createSmtpResolverContext(context)).smtpAppMutation();
        }),
        "SmtpConnection.__resolveReference": appGraphQL.handler((parent, _args, context) => {
          assertSmtpAdminAccess(context, "read");
          const reference = parent as { id?: unknown };
          if (typeof reference.id !== "string") return null;
          try {
            const id = decodeGlobalIdByType(reference.id, GlobalIdEntity.SmtpConnection);
            return new SmtpConnectionResolver(id, createSmtpResolverContext(context));
          } catch {
            return null;
          }
        }),
