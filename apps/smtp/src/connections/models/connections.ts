...
export const smtpConnectionProvider = smtpSchema.enum("smtp_connection_provider", [
  "CUSTOM",
  "SENDGRID",
...
export const smtpConnectionStatus = smtpSchema.enum("smtp_connection_status", [
  "ACTIVE",
  "INACTIVE",
  "DISCONNECTED",
]);

export const smtpConnectionSecurity = smtpSchema.enum("smtp_connection_security", [
  "NONE",
  "STARTTLS",
  "TLS",
]);
...
export const smtpConnections = smtpSchema.table(
  ...
  (table) => [
    index("smtp_connections_installation_status_idx").on(table.installationId, table.status),
    index("smtp_connections_store_status_idx").on(table.storeId, table.status),
...
export type SmtpConnectionProvider = (typeof smtpConnectionProvider.enumValues)[number];
export type SmtpConnectionStatus = (typeof smtpConnectionStatus.enumValues)[number];
