...
export class OnlineStoreLoader {
  ...
  readonly navigationMenuItem: DataLoader<string, NavigationMenuItemRecord | null>;
  readonly navigationMenuItemTranslation: DataLoader<
    string,
    NavigationMenuItemTranslationRecord | null
  >;

  constructor(repository: OnlineStoreRepository, scope: OnlineStoreScope, locale: string) {
    ...
    this.pageTranslation = new DataLoader(async (ids: readonly string[]) => {
      const records = await repository.translation.getPageTranslations(scope, ids, locale);
      return ids.map((id) => records.find((record) => record.pageId === id) ?? null);
    });
    ...
    this.navigationMenuItemTranslation = new DataLoader(async (ids: readonly string[]) => {
      const records = await repository.translation.getMenuItemTranslations(scope, ids, locale);
      return ids.map((id) => records.find((record) => record.itemId === id) ?? null);
    });
  }
}
