# Gift With Purchase (GWP)

Подарок при выполнении условия (по товарам / количеству / сумме). Gift-SKU показывается и становится доступен только при выполнении триггера.

## UI редактирования

### Основные поля

- **Условие активации** — select + поля:
  - Товар в корзине (product picker)
  - Количество товаров ≥ N (number input)
  - Сумма ≥ X (number input)
  - Товар из категории (category picker)
- **Подарок:**
  - Товар-подарок (product picker)
  - Количество подарков (number input, default 1)
  - Цена подарка — radio: Бесплатно | Символическая цена (number input)
- **Лимит** — max подарков на заказ (number input, default 1)

### Отличия от generic rule editor

- UI фокусируется на **одном триггере → один подарок**
- Условие — единый select (товар / кол-во / сумма) вместо condition builder
- Подарок всегда скрыт по умолчанию, показывается при срабатывании
- Цена подарка — почти всегда 0, поэтому "Бесплатно" — default

### Генерируемые правила (dependency-rules)

```
Начальное состояние (подарок скрыт):
  action: HIDE на ITEM(gift)  — default state

Показать подарок по условию:
  // Вариант: по товару
  condition: ITEM_SELECTED(triggerItem) IS_SELECTED
  // Вариант: по количеству
  condition: GROUP_TOTAL_QTY(triggerGroup) GTE(N)
  // Вариант: по сумме
  condition: BUNDLE_SUBTOTAL GTE(X)

  action: SHOW на ITEM(gift)
  action: ENABLE на ITEM(gift)
  action: OVERRIDE_PRICE на ITEM(gift) (priceType: FIXED, priceValue: 0)
  action: SET_QTY_LIMITS на ITEM(gift) (min: 0, max: giftLimit)

Скрыть подарок когда условие не выполнено:
  // Инверсия условия
  condition: BUNDLE_SUBTOTAL LT(X)
  action: HIDE на ITEM(gift)
```

### Wireframe

```
┌─────────────────────────────────────────────────┐
│  Gift With Purchase                             │
├─────────────────────────────────────────────────┤
│                                                 │
│  Условие                                        │
│  ┌──────────────────────────────────────┐       │
│  │ ▾ Сумма заказа не менее...           │       │
│  └──────────────────────────────────────┘       │
│  Сумма: [___2000___] ₽                          │
│                                                 │
│  ── или ──                                      │
│                                                 │
│  ┌──────────────────────────────────────┐       │
│  │ ▾ Товар в заказе...                  │       │
│  └──────────────────────────────────────┘       │
│  Товар: [Premium Headphones_________]           │
│                                                 │
│  ┌─ Подарок ──────────────────────────────┐     │
│  │  Товар: [Carrying Case____________]    │     │
│  │  Количество: [__1__]                   │     │
│  │                                        │     │
│  │  Цена:                                 │     │
│  │  ● Бесплатно                           │     │
│  │  ○ Символическая  [____1___] ₽         │     │
│  └────────────────────────────────────────┘     │
│                                                 │
│  Макс. подарков на заказ: [__1__]               │
└─────────────────────────────────────────────────┘
```
