...
test.describe('Stock API', () => {
  ...
  test.describe('Variant Stock', () => {
    test('should set variant stock in warehouse', async ({ api }) => {
      ...
      // Set stock
      const { data } = await api.admin.mutation('inventory/VariantSetStock', {
        variables: {
          input: {
            variantId,
    ...
    test('should set stock in multiple warehouses', async ({ api }) => {
      ...
      const { data: data1 } = await api.admin.mutation('inventory/VariantSetStock', {
        variables: {
          input: {
            variantId,
      ...
      const { data: data2 } = await api.admin.mutation('inventory/VariantSetStock', {
        variables: {
          input: {
            variantId,
