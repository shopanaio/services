...
describe("createRelayQuery (fluent API)", () => {
  describe("validation", () => {
    ...
    it("ignores before cursor when first is used (first takes precedence)", () => {
      ...
      const cursor = encode({
        ...
        filtersHash,
        seek: [
          { field: "id", value: "prod-999", direction: "desc" },
          { field: "id", value: "prod-999", direction: "desc" },
        ],
      });
  ...
  describe("cursor seek conditions", () => {
    it("forward pagination (first) with cursor builds correct WHERE", () => {
      ...
      const cursor = encode({
        ...
        filtersHash,
        seek: [
          { field: "id", value: "prod-123", direction: "desc" }, // sort field
          { field: "id", value: "prod-123", direction: "desc" }, // tie-breaker
        ],
      });
    ...
    it("forward pagination with two-field cursor (price DESC, id DESC)", () => {
      ...
      // orderBy: [{ field: "price", direction: "desc" }] + tie-breaker (id follows price direction = desc)
      const cursor = encode({
        type: "product",
        filtersHash,
        seek: [
    ...
    it("forward pagination with ASC sort uses _gt operator", () => {
      ...
      // orderBy: [{ field: "handle", direction: "asc" }] + tie-breaker (id follows handle direction = asc)
      const cursor = encode({
        type: "product",
        filtersHash,
        seek: [
          { field: "handle", value: "apple", direction: "asc" },
          { field: "id", value: "prod-100", direction: "asc" }, // tie-breaker
        ],
      });
    ...
    it("forward pagination with mixed ASC/DESC sort (handle ASC, price DESC)", () => {
      ...
      // orderBy: [{ field: "handle", direction: "asc" }, { field: "price", direction: "desc" }] + tie-breaker (id follows LAST sort field = desc)
      const cursor = encode({
        type: "product",
        filtersHash,
        seek: [
          { field: "handle", value: "iphone", direction: "asc" },
          { field: "price", value: 999, direction: "desc" },
          { field: "id", value: "prod-777", direction: "desc" }, // tie-breaker follows price:desc
        ],
      });
    ...
    it("backward pagination (last + before) with ASC sort uses _lt operator", () => {
      ...
      const cursor = encode({
        ...
        filtersHash,
        seek: [
          { field: "handle", value: "macbook", direction: "asc" },
          { field: "id", value: "prod-500", direction: "asc" },
        ],
      });
    ...
    it("backward pagination (last + before) with mixed ASC/DESC sort", () => {
      ...
      const cursor = encode({
        ...
        seek: [
          { field: "handle", value: "iphone", direction: "asc" },
          { field: "price", value: 999, direction: "desc" },
          { field: "id", value: "prod-777", direction: "desc" },
        ],
      });
  ...
  describe("user filters combined with cursor", () => {
    it("merges user WHERE with cursor seek condition", () => {
      ...
      const cursor = encode({
        ...
        filtersHash,
        seek: [
          { field: "price", value: 75, direction: "desc" },
          { field: "id", value: "prod-500", direction: "desc" },
        ],
      });
    ...
    it("complex user filter with cursor", () => {
      ...
      const cursor = encode({
        ...
        filtersHash,
        seek: [
          { field: "price", value: 100, direction: "desc" },
          { field: "id", value: "prod-999", direction: "desc" },
        ],
      });
  ...
  describe("tie-breaker handling", () => {
    it("adds tie-breaker to sort when not present", () => {
      ...
      expect(meta.sortParams).toEqual([{ field: "price", direction: "desc" }]);
  ...
  describe("first page without cursor", () => {
    ...
    it("uses default sort field when no order specified", () => {
      ...
      expect(meta.sortParams).toEqual([{ field: "id", direction: "desc" }]);
  ...
  describe("cursor order mismatch", () => {
    it("throws when cursor sort order differs from query order", () => {
      ...
      const cursor = encode({
        ...
        seek: [
          { field: "price", value: 100, direction: "desc" },
          { field: "id", value: "prod-1", direction: "desc" },
        ],
      });
    ...
    it("throws when cursor has wrong number of seek values", () => {
      ...
      const cursor = encode({
        ...
        filtersHash,
        seek: [
          { field: "handle", value: "test", direction: "asc" },
          { field: "price", value: 100, direction: "desc" },
          { field: "id", value: "prod-1", direction: "desc" },
        ],
      });
  ...
  describe("null values in cursor", () => {
    ...
    it("handles undefined value in cursor (treated as null)", () => {
      ...
      const cursor = encode({
        ...
        filtersHash,
        seek: [
          { field: "price", value: undefined as unknown, direction: "desc" },
          { field: "id", value: "prod-456", direction: "desc" },
        ],
      });
  ...
  describe("cursor with joins", () => {
    ...
    it("cursor on joined field builds correct seek", () => {
      ...
      const cursor = encode({
        ...
        filtersHash,
        seek: [
          { field: "translation.value", value: "Apple iPhone", direction: "asc" },
          { field: "id", value: "prod-001", direction: "asc" },
        ],
      });
    ...
    it("filter on joined field with cursor", () => {
      ...
      const cursor = encode({
        ...
        filtersHash,
        seek: [
          { field: "price", value: 599, direction: "desc" },
          { field: "id", value: "prod-100", direction: "desc" },
        ],
      });
...
describe("cursor encode/decode", () => {
  it("encode creates valid base64url string", () => {
    const params = {
      type: "product",
      filtersHash: "abc123",
      seek: [{ field: "id", value: "prod-1", direction: "desc" as const }],
    };
  ...
  it("decode restores original params", () => {
    const params = {
      ...
      filtersHash: "abc123",
      seek: [
        { field: "price", value: 99.99, direction: "desc" as const },
        { field: "id", value: "prod-1", direction: "desc" as const },
      ],
    };
