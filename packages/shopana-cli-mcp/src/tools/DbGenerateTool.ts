...
const DbGenerateToolSchema = z.object({
  service: z
    ...
    .optional()
    .describe(
      `Generate migrations for specific service only. Available: ${formatServiceNames(DB_GENERATE_SERVICE_NAMES)}`,
    ),
  list: z.boolean().optional().describe("List services with db:generate script"),
