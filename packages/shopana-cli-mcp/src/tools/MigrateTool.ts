...
const MigrateToolSchema = z.object({
  service: z
    ...
    .optional()
    .describe(
      `Migrate specific service only. Available: ${formatServiceNames(MIGRATION_SERVICE_NAMES)}`,
    ),
  app: z.string().min(1).optional().describe("Migrate a specific hosted App from apps/*"),
