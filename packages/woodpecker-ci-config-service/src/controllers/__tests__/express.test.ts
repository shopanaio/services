...
describe("Express Router", () => {
  ...
  describe("POST /", () => {
    it("should return configs on successful generation", async () => {
      ...
      const mockConfigs = [createMockConfig(".woodpecker.yml", "steps:\n  - name: test\n")];

      mockConfigService.generate.mockResolvedValue(mockConfigs);

      const response = await request(app).post("/").send(mockRequest).expect(200);
    ...
    it("should return 500 on service error", async () => {
      ...
      const response = await request(app).post("/").send(mockRequest).expect(500);
    ...
    it("should parse JSON body correctly", async () => {
      ...
      await request(app).post("/").send(complexRequest).expect(200);
    ...
    it("should reject body larger than 2mb", async () => {
      ...
      await request(app).post("/").send(largeBody).expect(413);
    });
  ...
  describe("Middleware chain", () => {
    it("should process request through JSON middleware before handler", async () => {
      ...
      await request(app).post("/").send(mockRequest).expect(200);
