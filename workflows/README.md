...
- Docker Compose stack:
  ...
  - `config-service` mounts that volume at `/app/workflows/workflows` and serves them.
- Update process:
...
- Local development:
  - Bind-mount `workflows/dist/workflows` into the server container for hot reload.
- Verification:
...
Add target builds: `server` and `workflows`. Below is an example `workflows/esbuild.js` with target support. If the file already exists, replace its contents with this:
...
Update scripts in `workflows/package.json`:
...
Replace `workflows/Dockerfile` with a multi-stage file that exposes two targets. Build from the `services/` root.

```dockerfile
# Build context: services/
# Build examples:
#   docker build --platform linux/amd64 -t config-service:server -f workflows/Dockerfile --target server .
#   docker build --platform linux/amd64 -t config-service:workflows -f workflows/Dockerfile --target workflows .
...
COPY --from=deps /app/workflows/package.json ./workflows/

RUN yarn install --frozen-lockfile --production

# Runtime dist
COPY --from=build-server /app/workflows/dist ./workflows/dist
COPY --from=build-server /app/packages/woodpecker-ci-config-service/dist ./packages/woodpecker-ci-config-service/dist

# Public key
COPY --from=deps /app/workflows/public-key.pem ./workflows/public-key.pem
...
WORKDIR /workflows
COPY --from=build-workflows /app/workflows/dist/workflows /workflows
...
services:
  ...
  config-service:
    ...
    environment:
      ...
      - PORT=3000
    volumes:
      - workflows-data:/app/workflows/workflows
    ports:
      - "3000:3000"
...
docker build -t config-service:server -f workflows/Dockerfile --target server .

# Build the workflows image
docker build -t config-service:workflows -f workflows/Dockerfile --target workflows .
```
...
docker compose -f workflows/docker-compose.yml run --rm workflows-loader

# 2) Start/restart the server
docker compose -f workflows/docker-compose.yml up -d config-service
```
...
docker compose -f workflows/docker-compose.yml restart config-service
```
...
docker run -d --name config-service \
  ...
  -e PORT=3000 \
  -p 3000:3000 \
  -v $(pwd)/workflows/dist/workflows:/app/workflows/workflows \
...
- Verify files were copied into the volume:
  ```bash
  docker compose -f workflows/docker-compose.yml run --rm workflows-loader sh -lc "ls -la /mounted"
  ```
- Check what the server sees:
  ```bash
  docker compose -f workflows/docker-compose.yml exec config-service sh -lc "ls -la /app/workflows/workflows"
  ```
- Server logs:
  ```bash
  docker compose -f workflows/docker-compose.yml logs -f config-service
  ```
