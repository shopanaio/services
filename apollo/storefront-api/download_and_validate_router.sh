...
if [ -z "${ARTIFACT_URL}" ]; then
    # Determine target architecture triple for router tarball
    UNAME_M=$(uname -m)
    case "${UNAME_M}" in
        x86_64|amd64)
...
if [ -z "${ARTIFACT_URL}" ]; then
    # Resolve 'latest' to a concrete release tag to ensure checksum exists
    RESOLVED_RELEASE="${ROUTER_RELEASE}"
    if [ "${RESOLVED_RELEASE}" = "latest" ]; then
        echo "Resolving latest Router release tag from GitHub"
        RESOLVED_RELEASE=$(curl -sSL https://api.github.com/repos/apollographql/router/releases/latest | grep -m1 '"tag_name"' | cut -d '"' -f 4)
    ...
    echo "Downloading Router release: ${RESOLVED_RELEASE}"
    # Download router tarball directly instead of using installer
    TARBALL_NAME="router-${RESOLVED_RELEASE}-${ARCH_TRIPLE}.tar.gz"
    ...
    curl -sSL \
      "https://rover.apollo.dev/tar/router/${ARCH_TRIPLE}/${RESOLVED_RELEASE}" \
        -o "${TARBALL_NAME}"

    # Download and validate checksum
    curl -sSL \
      "https://github.com/apollographql/router/releases/download/${RESOLVED_RELEASE}/sha256sums.txt" \
        -o sha256sums.txt
