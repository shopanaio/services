  ...
  return (
    <Paper data-testid="review-ratings-section">
      <PaperHeader
        ...
        actions={
          <Button size="small" icon={<EditOutlined />} onClick={() => onEdit("ratings")}>
            Edit
          </Button>
        }
      />
        ...
        <div className={styles.criteriaPanel}>
          ...
          {review.ratings.length ? (
                ...
                <div className={styles.criterionScale} aria-label={`${item.value} out of 5`}>
                  {Array.from({ length: 5 }, (_, index) => (
                    <span
                      key={index}
                      className={cx(
                        styles.criterionSegment,
                        index < item.value && styles.criterionSegmentActive,
                      )}
                    />
                  ))}
                </div>
            ...
            <EntityDetailsEmptyState
              state={{
                title: "No criterion ratings",
                description: "No criterion scores were submitted.",
              }}
            />
          )}
        </div>
