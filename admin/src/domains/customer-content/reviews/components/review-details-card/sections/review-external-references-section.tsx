...
export function ReviewExternalReferencesSection({
  review,
  onAdd,
  onEdit,
}: ReviewExternalReferencesSectionProps) {
      ...
      {references.length ? (
        references.map((reference) => (
          <div key={reference.id} className={styles.externalRow}>
            <Tag color={syncColor[reference.syncStatus]}>
              {humanizeEnum(reference.syncStatus).toUpperCase()}
            </Tag>
            <Flex vertical gap={4}>
              ...
              <Flex gap={8} align="center" wrap="wrap">
                <Typography.Text type="secondary">
                  {reference.syncStatus === ReviewExternalSyncStatus.Failed
                    ? "Sync failed"
                    : "Last synced"}{" "}
                  {formatReviewDateTime(reference.lastSyncedAt ?? reference.updatedAt)}
                </Typography.Text>
              ...
              {reference.lastError ? (
                <Typography.Text type="danger">{reference.lastError}</Typography.Text>
              ) : null}
            </Flex>
            <EditAction
              onEdit={() => onEdit(reference)}
              label="Edit external reference"
              testId={`review-external-${reference.id}`}
            />
          </div>
        ))
      ) : (
        <EntityDetailsEmptyState
          ...
          state={{
            ...
            description: "Connect this review to an external review system.",
          }}
        />
      )}
      {!references.length ? (
