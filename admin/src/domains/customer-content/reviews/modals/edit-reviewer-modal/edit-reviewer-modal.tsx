...
export function EditReviewerModal() {
  ...
  useEffect(() => {
    ...
    setCustomer(
      linked ? { id: linked.id, title: linked.displayName, email: linked.email ?? "" } : null,
    );
    reset({
  ...
  const changeAuthorType = async (next: ReviewContentAuthorType) => {
    if (authorType === ReviewContentAuthorType.Customer && next !== authorType && customerId) {
      const confirmed = await modal.confirm({
        ...
        content:
          "The reviewer snapshot will remain editable, but the customer link will be cleared.",
        okText: "Remove link",
      });
      if (!confirmed) return;
      setCustomer(null);
    ...
    await state.save<ReviewReviewerSectionValues>(
      {
        content: {
          author: {
            ...
            customerId:
              values.authorType === ReviewContentAuthorType.Customer ? values.customerId : null,
            ...
            <div className={styles.fieldGrid}>
              <ReviewFormField label="Display name *" error={errors.displayName?.message}>
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <Input {...field} status={errors.displayName ? "error" : undefined} />
                  )}
                />
              </ReviewFormField>
