...
export function ReviewCreateModal() {
  ...
  const canSubmit = useMemo(() => !!product && !!customer && body.trim().length >= 20, [body, customer, product]);
  ...
  const submit = async () => {
    ...
    const result = await mutation.createReview({
      ...
      content: {
        ...
        body: body.trim(),
        locale,
        author: { type: ReviewContentAuthorType.Customer, customerId: customer.id, displayName: customer.title },
        source: { channel: "ADMIN" },
        status: ReviewContentStatus.Pending,
      },
      verification: { status: ReviewVerificationStatus.Unverified },
    });
    if (!result.review || result.userErrors.length) return setError(result.userErrors.map((item) => item.message).join(" ") || "Unable to create review");
    await value.onSaved?.(); setDirty(false); message.success("Review created"); forcePop();
  };

  return <ModalLayout name="review-create" header={<ModalHeader name="review-create" title="New review" onClose={pop} submitButtonProps={{ children: "Create", loading: mutation.loading, disabled: !canSubmit, onClick: submit }} />}>
    ...
    <Paper><PaperHeader title="Basic information" /><Flex vertical gap="middle">
      ...
      <div><Typography.Text strong>Rating *</Typography.Text><div style={{ marginTop: 8 }}><Rate value={rating} onChange={setRating} /></div></div>
      <div><Typography.Text strong>Title</Typography.Text><Input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={150} showCount style={{ marginTop: 8 }} /></div>
      <div><Typography.Text strong>Review *</Typography.Text><Input.TextArea value={body} onChange={(event) => setBody(event.target.value)} autoSize={{ minRows: 5, maxRows: 10 }} maxLength={5000} showCount placeholder="At least 20 characters" style={{ marginTop: 8 }} /></div>
    </Flex></Paper>
  </ModalLayout>;
}
