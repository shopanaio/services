...
export const reviewContentSectionSchema = z.object({
  locale: z.string().trim().min(2, "Locale is required").max(35),
  title: z.string().trim().max(150, "Title must be at most 150 characters"),
  body: z.string().trim().min(20, "Review must contain at least 20 characters").max(5000, "Review must be at most 5,000 characters"),
});

export const reviewReviewerSectionSchema = z.object({
  authorType: z.enum(ReviewContentAuthorType),
  customerId: z.string(),
  ...
  if (values.authorType === ReviewContentAuthorType.Customer && !values.customerId) {
    context.addIssue({ code: "custom", path: ["customerId"], message: "Customer is required for a customer author" });
  }
});
...
export const reviewRatingsSectionSchema = z.object({
  ...
  criteria: z.array(z.object({ criterionId: z.string(), title: z.string(), value: z.number().int().min(1).max(5) })),
});

export const reviewModerationSectionSchema = z.object({
  status: z.enum(ReviewContentStatus),
  moderationNote: z.string().trim().max(1000, "Moderation note must be at most 1,000 characters"),
}).superRefine((values, context) => {
  if (values.status === ReviewContentStatus.Rejected && !values.moderationNote) {
    context.addIssue({ code: "custom", path: ["moderationNote"], message: "Add a reason when rejecting a review" });
  }
});

export const reviewVerificationSectionSchema = z.object({
  ...
  if (values.status !== ReviewVerificationStatus.Unverified && !values.method) {
    context.addIssue({ code: "custom", path: ["method"], message: "Verification method is required" });
  }
  if (values.status !== ReviewVerificationStatus.Unverified && !values.verifiedAt) {
    context.addIssue({ code: "custom", path: ["verifiedAt"], message: "Verification date is required" });
  }
});

export const reviewIncentiveSectionSchema = z.object({
  ...
  if (values.isIncentivized && !values.disclosure) {
    context.addIssue({ code: "custom", path: ["disclosure"], message: "Public disclosure is required" });
  }
});
...
export const reviewMediaItemSchema = z.object({
  ...
  if (values.status === ReviewContentStatus.Rejected && !values.moderationNote) {
    context.addIssue({ code: "custom", path: ["moderationNote"], message: "Moderation note is required when media is rejected" });
  }
});
