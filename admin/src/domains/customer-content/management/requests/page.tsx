...
import {
  buildReviewRequestSearchCondition,
  buildReviewRequestsQueryVariables,
  reviewRequestSortFieldMapping,
} from "./page-config";
  ...
  return (
    <Flex vertical gap={2} style={{ minWidth: 0 }}>
      ...
      <Typography.Text type="secondary" ellipsis>
        {data.customer.email ?? data.customer.id}
      </Typography.Text>
    </Flex>
  );
}
function ProductCell({ data }: CustomCellRendererProps<ApiReviewRequest>) {
  ...
  return (
    <Flex vertical gap={2} style={{ minWidth: 0 }}>
      <Typography.Text ellipsis>{data.product.title}</Typography.Text>
      <Typography.Text type="secondary" ellipsis>
        Order {data.orderId}
      </Typography.Text>
    </Flex>
...
export default function ReviewRequestsPage() {
  ...
  const pageConfig = usePageConfig<
    ...
    ReviewRequestOrderField
  >({
    gridRef,
    storageKey: "review-requests-grid-state",
    filterSchema,
    sortFieldMapping: reviewRequestSortFieldMapping,
    defaultSort: [{ colId: "createdAt", sort: "desc" }],
    defaultPageSize: 10,
    pageSizeOptions: [10, 20, 50],
    buildSearchCondition: buildReviewRequestSearchCondition,
  });
  const variables = useMemo(
    () => buildReviewRequestsQueryVariables(pageConfig),
    [
      pageConfig.first,
      pageConfig.after,
      pageConfig.last,
  ...
  const { reviewRequests, totalCount, pageInfo, loading, error, refetch } =
    useReviewRequests(variables);
  ...
  const columnDefs = useMemo<ColDef<ApiReviewRequest>[]>(
    () => [
      ...
      },
      {
        ...
        cellRenderer: ({
          value,
        }: CustomCellRendererProps<ApiReviewRequest, ReviewRequestStatus>) => (
          <Tag
            color={
              value === ReviewRequestStatus.Submitted
                ? "green"
                : value === ReviewRequestStatus.Failed
                  ? "red"
                  : value === ReviewRequestStatus.Scheduled
                    ? "gold"
                    : "blue"
            }
          >
      ...
      { headerName: "Locale", field: "locale", width: 105 },
      {
        ...
        field: "scheduledAt",
        minWidth: 180,
        cellRenderer: ({ value }: CustomCellRendererProps<ApiReviewRequest, string>) => (
  ...
  const defaultColDef = useMemo<ColDef<ApiReviewRequest>>(
    () => ({
      resizable: true,
      sortable: true,
      comparator: () => 0,
  ...
  return (
    ...
    >
      <DataLayout.Toolbar
        left={
          <FilterWidget
            ...
            searchPlaceholder="Search request delivery details..."
          />
        }
      />
      <div style={{ height: "100%", paddingBottom: 16, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1 }} data-testid="review-requests-table">
          <AgGridReact<ApiReviewRequest>
            ref={gridRef}
            theme={theme}
            rowData={reviewRequests}
            loading={loading}
            ...
            onRowClicked={({ data }) => data && push({ reviewRequest: data, onSaved: refetch })}
        ...
        <CursorPagination
          name="review requests"
          total={totalCount}
          rangeStart={pageConfig.getRangeStart(reviewRequests.length)}
          rangeEnd={Math.min(pageConfig.getRangeEnd(reviewRequests.length), totalCount)}
