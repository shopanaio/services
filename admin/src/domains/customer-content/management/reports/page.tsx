...
import { buildReportSearchCondition, buildReportsQueryVariables, reportSortFieldMapping } from "./page-config";
...
export default function ContentReportsPage() {
  ...
  const pageConfig = usePageConfig<ApiReviewContentReport, ApiReviewContentReportWhereInput, ReviewContentReportOrderField>({
    gridRef, storageKey: "content-reports-grid-state", filterSchema, sortFieldMapping: reportSortFieldMapping,
    defaultSort: [{ colId: "createdAt", sort: "desc" }], defaultPageSize: 10, pageSizeOptions: [10, 20, 50], buildSearchCondition: buildReportSearchCondition,
  });
  const variables = useMemo(() => buildReportsQueryVariables(pageConfig), [pageConfig.first, pageConfig.after, pageConfig.last, pageConfig.before, pageConfig.where, pageConfig.orderBy]);
  ...
  const openReport = useCallback((report: ApiReviewContentReport) => { setSelected(report); setAssignee(report.assignedToPrincipalId ?? ""); setStatus([ReviewContentReportStatus.Actioned, ReviewContentReportStatus.Dismissed].includes(report.status) ? report.status : undefined); setNote(report.resolutionNote ?? ""); }, []);
  const save = useCallback(async () => {
    if (!selected) return;
    const result = await mutations.updateReport(selected.id, selected.updatedAt, { assignment: { assignedToPrincipalId: assignee.trim() || null }, resolution: status ? { status, note: note.trim() || null } : undefined });
    if (result.errors.length) return message.error(result.errors.map((item) => item.message).join(" "));
    await refetch(); setSelected(null); message.success("Report updated");
  ...
  const columnDefs = useMemo<ColDef<ApiReviewContentReport>[]>(() => [
    { headerName: "Reported content", colId: "content", cellRenderer: ContentCell, minWidth: 360, flex: 2, sortable: false },
    { headerName: "Reason", field: "reason", minWidth: 180, valueFormatter: ({ value }) => String(value).toLowerCase().replaceAll("_", " ") },
    { headerName: "Reporter", colId: "reporterCustomerId", minWidth: 180, valueGetter: ({ data }) => data?.reporterCustomer?.displayName ?? "Anonymous" },
    { headerName: "Status", field: "status", width: 130, cellRenderer: ({ value }: CustomCellRendererProps<ApiReviewContentReport, ReviewContentReportStatus>) => <Tag color={value === ReviewContentReportStatus.Open ? "gold" : value === ReviewContentReportStatus.Actioned ? "green" : value === ReviewContentReportStatus.UnderReview ? "blue" : undefined}>{String(value).toLowerCase().replaceAll("_", " ")}</Tag> },
    { headerName: "Assignee", field: "assignedToPrincipalId", minWidth: 180, valueFormatter: ({ value }) => value ?? "Unassigned" },
    { headerName: "Created", field: "createdAt", minWidth: 180, cellRenderer: ({ value }: CustomCellRendererProps<ApiReviewContentReport, string>) => <Typography.Text>{value ? dateFormatter.format(new Date(value)) : ""}</Typography.Text> },
  ], []);
  const defaultColDef = useMemo<ColDef<ApiReviewContentReport>>(() => ({ resizable: true, sortable: true, comparator: () => 0, cellStyle: { display: "flex", alignItems: "center" } }), []);

  return <DataLayout fullWidth name="content-reports" title="Content reports" count={totalCount} onBack={backToUgc}>
    <DataLayout.Toolbar left={<FilterWidget {...pageConfig.filterWidgetProps} searchPlaceholder="Search principal IDs..." />} />
    <div style={{ height: "100%", paddingBottom: 16, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1 }} data-testid="content-reports-table"><AgGridReact<ApiReviewContentReport> ref={gridRef} theme={theme} rowData={reports} loading={loading} columnDefs={columnDefs} defaultColDef={defaultColDef} getRowId={({ data }) => data.id} rowHeight={68} suppressCellFocus suppressMovableColumns rowStyle={{ cursor: "pointer" }} onRowClicked={({ data }) => data && openReport(data)} onSortChanged={pageConfig.onSortChanged} initialState={pageConfig.gridStateProps.initialState} onStateUpdated={pageConfig.gridStateProps.onStateUpdated} /></div>
      <CursorPagination name="content reports" total={totalCount} rangeStart={pageConfig.getRangeStart(reports.length)} rangeEnd={Math.min(pageConfig.getRangeEnd(reports.length), totalCount)} pageSize={pageConfig.pageSize} pageSizeOptions={pageConfig.pageSizeOptions} hasNext={pageInfo?.hasNextPage ?? false} hasPrev={pageInfo?.hasPreviousPage ?? false} onNext={next} onPrev={prev} onPageSizeChange={pageConfig.setPageSize} />
    </div>
