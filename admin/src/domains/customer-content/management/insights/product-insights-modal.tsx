...
import {
  ...
  LuCircleHelp as QuestionOutlined,
...
export function ProductInsightsModal() {
  ...
  const ratingBreakdown: Record<number, number> = summary
    ...
    : { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 };
  const criteria = summary?.criteria.length
      ...
      <Paper>
        ...
        <Flex vertical gap="small" style={{ marginTop: 20 }}>
          {[5, 4, 3, 2, 1].map((rating) => (
            <Flex key={rating} gap="middle" align="center">
              <Typography.Text style={{ width: 52 }}>{rating} stars</Typography.Text>
              <Progress
                percent={
                  reviewCount ? Math.round((ratingBreakdown[rating] / reviewCount) * 100) : 0
                }
                format={() => String(ratingBreakdown[rating])}
              />
            </Flex>
          ))}
        </Flex>
