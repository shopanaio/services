...
export function ModerationCaseModal() {
  ...
  const value = payload as ModerationCaseModalPayload;
  const current = value.moderationCase;
  const mutations = useManagementMutations();
  const [contentId, setContentId] = useState(current?.content.id ?? value.contentId ?? "");
  ...
  const [reasonCode, setReasonCode] = useState(current?.reasonCode ?? "MANUAL_REVIEW");
  const [assignedTo, setAssignedTo] = useState(current?.assignedToPrincipalId ?? "");
  const [dueAt, setDueAt] = useState(current?.dueAt?.slice(0, 16) ?? "");
  const [resolutionStatus, setResolutionStatus] = useState<
    ReviewModerationCaseStatus | undefined
  >();
  const [resolutionCode, setResolutionCode] = useState(current?.resolutionCode ?? "");
  const [resolutionNote, setResolutionNote] = useState(current?.resolutionNote ?? "");
  ...
  const save = async () => {
    setError(null);
    if (!contentId.trim() || !reasonCode.trim())
      return setError("Content and reason code are required.");
    const result = current
      ? await mutations.updateCase(current.id, current.updatedAt, {
          details: {
            priority,
            reasonCode: reasonCode.trim(),
            assignedToPrincipalId: assignedTo.trim() || null,
    ...
    await value.onSaved?.();
    setDirty(false);
    message.success(current ? "Moderation case updated" : "Moderation case created");
    forcePop();
  };
  return (
    <ModalLayout
      ...
      header={
        <ModalHeader
          name="moderation-case"
          title={current ? "Moderation case" : "New moderation case"}
          onClose={pop}
          extra={
            current ? (
          ...
          submitButtonProps={{
            ...
            disabled: !contentId || !reasonCode || (!!current && !dirty),
            onClick: save,
          }}
        />
      }
    >
      ...
      <Paper>
        <PaperHeader title="Case details" />
        <Flex vertical gap="middle">
          <div>
            ...
            <Flex gap="small" style={{ marginTop: 8 }}>
              ...
              <Button
                ...
                disabled={!!current || !!value.contentId}
                onClick={contentPicker.openPicker}
              >
                Select
              </Button>
            </Flex>
          ...
          <Flex gap="middle">
            <div style={{ flex: 1 }}>
              <Typography.Text strong>Reason code *</Typography.Text>
              <Input
                value={reasonCode}
                onChange={(event) => {
          ...
          <div>
            <Typography.Text strong>Due at</Typography.Text>
            <Input
              type="datetime-local"
              value={dueAt}
              onChange={(event) => {
      ...
      {current ? (
        <Paper>
          <PaperHeader title="Resolution" />
          <Flex vertical gap="middle">
            <Select
              allowClear
              placeholder="Leave case open"
              value={resolutionStatus}
              options={[
                ReviewModerationCaseStatus.Resolved,
                ReviewModerationCaseStatus.Cancelled,
              ].map((status) => ({ value: status, label: status.toLowerCase() }))}
              onChange={(next) => {
                setResolutionStatus(next);
                change();
              }}
            />
            <Input
