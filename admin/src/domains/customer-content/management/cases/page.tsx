...
import {
  buildModerationCaseSearchCondition,
  buildModerationCasesQueryVariables,
  moderationCaseSortFieldMapping,
} from "./page-config";
...
export default function ModerationCasesPage() {
  ...
  const pageConfig = usePageConfig<
    ...
    ReviewModerationCaseOrderField
  >({
    gridRef,
    storageKey: "moderation-cases-grid-state",
    filterSchema,
    sortFieldMapping: moderationCaseSortFieldMapping,
    defaultSort: [{ colId: "createdAt", sort: "desc" }],
    defaultPageSize: 10,
    pageSizeOptions: [10, 20, 50],
    buildSearchCondition: buildModerationCaseSearchCondition,
  });
  const variables = useMemo(
    () => buildModerationCasesQueryVariables(pageConfig),
    [
      pageConfig.first,
      pageConfig.after,
      pageConfig.last,
  ...
  const { moderationCases, totalCount, pageInfo, loading, error, refetch } =
    useModerationCases(variables);
  ...
  const columnDefs = useMemo<ColDef<ApiReviewModerationCase>[]>(
    () => [
      ...
      { headerName: "Priority", field: "priority", width: 105 },
      {
        ...
        cellRenderer: ({
          value,
        }: CustomCellRendererProps<ApiReviewModerationCase, ReviewModerationCaseStatus>) => (
          <Tag
            color={
              value === ReviewModerationCaseStatus.Open
                ? "gold"
                : value === ReviewModerationCaseStatus.Resolved
                  ? "green"
                  : value === ReviewModerationCaseStatus.InReview
                    ? "blue"
                    : undefined
            }
          >
      ...
      },
      {
        ...
        field: "dueAt",
        minWidth: 180,
        cellRenderer: ({ value }: CustomCellRendererProps<ApiReviewModerationCase, string>) => (
  ...
  const defaultColDef = useMemo<ColDef<ApiReviewModerationCase>>(
    () => ({
      resizable: true,
      sortable: true,
      comparator: () => 0,
  ...
  return (
    ...
    >
      ...
      <div style={{ height: "100%", paddingBottom: 16, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1 }} data-testid="moderation-cases-table">
          <AgGridReact<ApiReviewModerationCase>
            ref={gridRef}
            theme={theme}
            rowData={moderationCases}
            loading={loading}
            ...
            onRowClicked={({ data }) => data && push({ moderationCase: data, onSaved: refetch })}
        ...
        <CursorPagination
          name="moderation cases"
          total={totalCount}
          rangeStart={pageConfig.getRangeStart(moderationCases.length)}
          rangeEnd={Math.min(pageConfig.getRangeEnd(moderationCases.length), totalCount)}
