...
import {
  ...
  LuFileSearch as FileSearchOutlined,
...
export function ExternalReferenceModal() {
  ...
  const [contentLabel, setContentLabel] = useState(
    value.contentLabel ?? currentContent?.body ?? value.contentId ?? "",
  );
  ...
  const [direction, setDirection] = useState(
    current?.direction ?? ReviewExternalSyncDirection.Bidirectional,
  );
      ...
      const result = current
        ? await mutations.updateExternalReference(current.id, current.updatedAt, {
            ...
            sync: {
              ...
              status,
              etag: etag.trim() || null,
              contentChecksum: checksum.trim() || null,
              metadata: parsed,
            },
          })
  ...
  const remove = async () => {
    ...
    if (!confirmed) return;
    try {
      const result = await mutations.deleteExternalReference({
        id: current.id,
      ...
      header={
        <ModalHeader
            ...
            current ? (
                  ...
                  items: [
                    {
                      ...
                      icon: <DeleteOutlined />,
                      danger: true,
                      "data-testid": "external-reference-delete",
                      onClick: () => void remove(),
                    },
                  ],
                }}
              >
                <Button
                  size="small"
                  icon={<MoreOutlined />}
                  aria-label="External reference actions"
                />
              </Dropdown>
            ) : null
          }
          submitButtonProps={{
            ...
            disabled: (!!current && !dirty) || !contentId || !system || !type || !externalId,
            onClick: () => void save(),
          }}
        />
      }
          ...
          <div>
            ...
            <Flex gap="small" style={{ marginTop: 8 }}>
              ...
              <Button
                ...
                disabled={Boolean(current || value.contentId)}
      ...
      <Paper>
        ...
        <Flex vertical gap="middle">
          <div>
            ...
            <Select
              value={direction}
              options={Object.values(ReviewExternalSyncDirection).map((item) => ({
                value: item,
          ...
          {current ? (
            <div>
              ...
              <Select
                value={status}
                options={Object.values(ReviewExternalSyncStatus).map((item) => ({
                  value: item,
                  label: humanizeEnum(item),
                }))}
                onChange={(next) => {
              ...
              items={[
                {
                  key: "last",
                  label: "Last synced",
                  children: current.lastSyncedAt ? formatDetailDate(current.lastSyncedAt) : "Never",
                },
