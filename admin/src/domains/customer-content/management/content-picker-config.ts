...
function useContentPickerData(options: {
  pageSize: number;
  first?: number;
  after?: string | null;
  last?: number;
  ...
  orderBy?: object[] | null;
  excludeIds: string[];
}): IEntityPickerDataResult<IPickableEntity> {
  ...
  const query = useModerationContents({
    first: options.first,
    after: options.after,
    last: options.last,
    before: options.before,
    where,
    orderBy: options.orderBy as ApiReviewContentOrderByInput[] | null,
  });
  return {
    data: query.contents.map((node) => ({
      id: node.id,
      title: node.title || `${node.kind}: ${node.body.slice(0, 100)}`,
      status: node.status,
    })),
    ...
    pagination: {
      ...
      hasPrev: query.pageInfo?.hasPreviousPage ?? false,
      startCursor: query.pageInfo?.startCursor,
      endCursor: query.pageInfo?.endCursor,
    },
  };
}
...
export const contentPickerConfig: IEntityPickerConfig<
  IPickableEntity,
...
> = {
  ...
  entityName: "Content",
  entityNamePlural: "Content",
  filterSchema: [],
  searchEnabled: true,
  columns,
  pageConfig: {
    storageKey: "review-content-picker-grid-state",
    sortFieldMapping: { title: ReviewContentOrderField.Title },
