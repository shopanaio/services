...
import {
  buildModerationQueryVariables,
  buildModerationSearchCondition,
  moderationSortFieldMapping,
} from "./page-config";
  ...
  return (
    <Flex vertical gap={2} style={{ minWidth: 0 }}>
      <Typography.Text strong ellipsis>
        {data.title || data.kind.toLowerCase().replaceAll("_", " ")}
      </Typography.Text>
...
export default function ModerationQueuePage() {
  ...
  const pageConfig = usePageConfig<
    ...
    ReviewContentOrderField
  >({
    gridRef,
    storageKey: "moderation-grid-state",
    filterSchema,
    sortFieldMapping: moderationSortFieldMapping,
    defaultSort: [{ colId: "updatedAt", sort: "desc" }],
    defaultPageSize: 10,
    pageSizeOptions: [10, 20, 50],
    buildSearchCondition: buildModerationSearchCondition,
  });
  const variables = useMemo(
    () => buildModerationQueryVariables(pageConfig),
    [
      pageConfig.first,
      pageConfig.after,
      pageConfig.last,
  ...
  const { contents, totalCount, pageInfo, loading, error, refetch } =
    useModerationContents(variables);
  ...
  const open = useCallback(
    (content: ApiReviewContent) => {
      if (content.kind === ReviewContentKind.Review)
        reviewModal.push({ entityId: content.id, onSaved: refetch });
      if (content.kind === ReviewContentKind.ProductQuestion)
        questionModal.push({ entityId: content.id, onSaved: refetch });
    },
    [questionModal, refetch, reviewModal],
  );
  const redact = useCallback(
    async (content: ApiReviewContent) => {
      const confirmed = await modal.confirm({
        title: "Redact content?",
        content: "Personal content will be irreversibly redacted.",
        okText: "Redact",
      ...
      await refetch();
      message.success("Content redacted");
    },
  ...
  const columnDefs = useMemo<ColDef<ApiReviewContent>[]>(
    () => [
      ...
      {
        headerName: "Author",
        colId: "authorDisplayName",
        minWidth: 180,
        valueGetter: ({ data }) => data?.author.displayName,
      },
      { headerName: "Locale", field: "locale", width: 105 },
      {
        ...
        cellRenderer: ({
          value,
        }: CustomCellRendererProps<ApiReviewContent, ReviewContentStatus>) => (
          <Tag
            color={
      ...
      },
      {
        ...
        colId: "reportCount",
        width: 105,
        valueGetter: ({ data }) => data?.metrics.reportCount ?? 0,
      },
      {
        headerName: "Updated",
        field: "updatedAt",
        minWidth: 180,
        cellRenderer: ({ value }: CustomCellRendererProps<ApiReviewContent, string>) => (
      ...
      },
      {
        ...
        width: 190,
        sortable: false,
        cellRenderer: ({ data }: CustomCellRendererProps<ApiReviewContent>) =>
          data ? (
            <Flex gap="small">
              <Button
                size="small"
                ...
                onClick={(event) => {
                  ...
                  caseModal.push({ contentId: data.id, onSaved: refetch });
                }}
              >
                Case
              </Button>
  ...
  const defaultColDef = useMemo<ColDef<ApiReviewContent>>(
    () => ({
      resizable: true,
      sortable: true,
      comparator: () => 0,
      ...
      <DataLayout.Toolbar
        left={
          <FilterWidget
            ...
            searchPlaceholder="Search moderated content..."
          />
        }
      />
      <div style={{ height: "100%", paddingBottom: 16, display: "flex", flexDirection: "column" }}>
        {error || actions.error ? (
          <Alert
            type="error"
            showIcon
            message={(error ?? actions.error)?.message}
            style={{ marginBottom: 12 }}
          />
        ) : null}
        <div style={{ flex: 1 }} data-testid="moderation-table">
          <AgGridReact<ApiReviewContent>
            ref={gridRef}
            theme={theme}
            rowData={contents}
            ...
            onRowClicked={({ data }) => data && open(data)}
        ...
        <CursorPagination
          name="moderation"
          total={totalCount}
          rangeStart={pageConfig.getRangeStart(contents.length)}
          rangeEnd={Math.min(pageConfig.getRangeEnd(contents.length), totalCount)}
