...
export function QuestionModal() {
  ...
  const handleDelete = async () => {
    ...
    if (!result.id || result.errors.length)
      throw new Error(
        result.errors.map((item) => item.message).join(" ") || "Unable to delete question",
      );
    await value.onSaved?.();
  ...
  const handleRedact = async () => {
    ...
    if (!result.content || result.errors.length)
      throw new Error(
        result.errors.map((item) => item.message).join(" ") || "Unable to redact question",
      );
    await refresh();
    message.success("Question redacted");
  };
  const handleSubscriptionUpdate = async (
    subscriptionId: string,
    expectedUpdatedAt: string,
    status: import("@/graphql/types").ProductQuestionSubscriptionStatus,
  ) => {
    const result = await managementMutations.updateSubscription(subscriptionId, expectedUpdatedAt, {
      status,
    });
  ...
  return (
    <ModalLayout
      ...
      headerProps={{ title: "Question details", onClose: pop, submitButtonProps: null }}
    >
      {query.loading && !question ? <Skeleton active paragraph={{ rows: 14 }} /> : null}
      {query.error || actions.error ? (
        <Alert type="error" showIcon message={(query.error ?? actions.error)?.message} />
      ) : null}
      {!query.loading && !question ? (
        <Alert type="error" showIcon message="Question not found" />
      ) : null}
      {question ? (
        <QuestionDetailsCard
          question={question}
          onEdit={(section) => openEdit({ entityId: question.id, section, onSaved: refresh })}
          onDelete={handleDelete}
          onRedact={handleRedact}
          onManageExternalReferences={() =>
            openExternalReference({ contentId: question.id, onSaved: refresh })
          }
          onUpdateSubscription={handleSubscriptionUpdate}
        />
      ) : null}
    </ModalLayout>
  );
