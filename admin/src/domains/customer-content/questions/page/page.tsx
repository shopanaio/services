...
import {
  buildQuestionSearchCondition,
  buildQuestionsQueryVariables,
  questionSortFieldMapping,
} from "./page-config";
...
function QuestionCell({ data }: CustomCellRendererProps<ApiProductQuestion>) {
  if (!data) return null;
  return (
    <Typography.Text ellipsis title={data.body}>
      {data.body}
    </Typography.Text>
  );
}
...
function AnswerCell({ data }: CustomCellRendererProps<ApiProductQuestion>) {
  ...
  return (
    <Flex align="center" gap={6}>
      ...
      <Typography.Text>
        {answered ? `${data.metrics.childCount} answered` : "Unanswered"}
      </Typography.Text>
    </Flex>
  );
}
  ...
  return (
    <Flex gap="middle">
      ...
      <Flex align="center" gap={5} title="Abuse reports">
        ...
        <Typography.Text type={data.metrics.reportCount ? "danger" : undefined}>
...
export default function CustomerQuestionsPage() {
  ...
  const createQuestion = useCallback(
    () => openQuestionCreateModal({ onSaved: refetch }),
    [openQuestionCreateModal, refetch],
  );
  const editQuestion = useCallback(
    (question: ApiProductQuestion) =>
      openQuestionModal({ entityId: question.id, onSaved: refetch }),
    [openQuestionModal, refetch],
  );
  ...
  const defaultColDef = useMemo<ColDef<ApiProductQuestion>>(
    () => ({
      resizable: true,
      sortable: true,
      comparator: () => 0,
  ...
  return (
    <DataLayout
      ...
      onBack={backToUgc}
      actions={
        <Button icon={<PlusOutlined />} onClick={createQuestion}>
          Create question
        </Button>
      }
    >
      <DataLayout.Toolbar
        left={
          <FilterWidget {...pageConfig.filterWidgetProps} searchPlaceholder="Search questions..." />
        }
      />
