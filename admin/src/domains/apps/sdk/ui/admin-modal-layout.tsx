...
export function AdminAppModalLayout({ title, children, actions }: AdminAppModalLayoutProps) {
  const { pop } = useModalStackContext();

  return (
    <ModalLayout
      name="admin-app"
      header={<ModalHeader title={title} onClose={pop} extra={actions} submitButtonProps={null} />}
    >
      {children}
    </ModalLayout>
  );
