...
import { AppInstallationStatus, type ApiAppWhereInput } from "@/graphql/types";
...
export const useAppsManagement = (where?: ApiAppWhereInput) => {
  ...
  const apps = useMemo(() => {
    const sourceApps = data?.appsQuery.apps.edges.map(({ node }) => node) ?? [];

    return [...sourceApps].sort((left, right) => {
      const leftRank =
        left.installation?.status === AppInstallationStatus.Active ? 0 : left.installed ? 1 : 2;
      const rightRank =
        right.installation?.status === AppInstallationStatus.Active ? 0 : right.installed ? 1 : 2;
