...
export function CustomerGroupModal() {
  ...
  useEffect(() => {
    ...
    setSelectedCustomers(
      memberships.map((item) => ({ id: item.customer.id, title: item.customer.displayName })),
    );
  ...
  const existing = useMemo(
    () => group?.customerMemberships.edges.map((edge) => edge.node) ?? [],
    [group],
  );
  const { openPicker } = useEntityPicker<IPickableEntity>({
    entityType: "customer",
    selectionMode: "multi",
    initialSelection: customerIds,
  ...
  const save = async () => {
    ...
    const result = !group
      ...
      : await mutations.updateGroup(group.id, group.revision, {
          definition: {
            code: definition.code.trim(),
            name: definition.name.trim(),
            description: definition.description.trim() || null,
          ...
          memberships: {
            create: customerIds
              .filter((id) => !existing.some((item) => item.customer.id === id))
              .map((customerId) => ({ customerId, isPrimary: customerId === primaryCustomerId })),
            update: existing
              .filter((item) => customerIds.includes(item.customer.id))
              .map((item) => ({
                membershipId: item.id,
                isPrimary: item.customer.id === primaryCustomerId,
    ...
    if (!result.group || result.userErrors.length)
      return setError(
        result.userErrors.map((item) => item.message).join(" ") || "Unable to save group",
      );
    await value.onSaved?.();
    setDirty(false);
    message.success(group ? "Group updated" : "Group created");
    forcePop();
  };
  ...
  const remove = async () => {
    ...
    if (!confirmed) return;
    const result = await mutations.deleteGroup({ id: group.id });
  ...
  if (isEdit && query.loading && !group)
    return (
      <ModalLayout
        name="customer-group"
        headerProps={{ title: "Customer group", onClose: pop, submitButtonProps: null }}
      >
        <Skeleton active />
      </ModalLayout>
    );
  return (
    <ModalLayout
      ...
      header={
        <ModalHeader
          title={group ? group.name : "New customer group"}
          onClose={pop}
          extra={
            group ? (
              <Tag color={group.isActive ? "green" : undefined}>
                {group.isActive ? "Active" : "Inactive"}
              </Tag>
            ) : null
          }
          submitButtonProps={{
            children: group ? "Save" : "Create",
            loading: mutations.loading,
            disabled: !dirty && !!group,
            onClick: save,
          }}
        />
      }
    >
      <Flex vertical gap={12}>
        {query.error || mutations.error ? (
          <Alert type="error" showIcon message={(query.error ?? mutations.error)?.message} />
        ) : null}
          ...
          <div className={styles.fields}>
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  setDefinition({ ...definition, code: e.target.value });
                  setLocalDirty(true);
                }}
              />
            </div>
          ...
          <Paper>
            ...
            {customerIds.length ? (
              <Flex vertical gap="middle">
                <Flex gap={4} wrap="wrap">
                  {selectedCustomers.map((item) => (
                    <Tag key={item.id} color={item.id === primaryCustomerId ? "blue" : undefined}>
                      {item.title}
                    </Tag>
                  ))}
                </Flex>
                <div>
                  <Typography.Text strong>Primary customer membership</Typography.Text>
                  <Select
                    allowClear
                    value={primaryCustomerId}
                    options={customerOptions}
                    onChange={(id) => {
                      setPrimaryCustomerId(id ?? null);
                      setLocalDirty(true);
                    }}
              ...
              <Empty
                ...
                description="No customers in this group"
              />
            )}
          </Paper>
        ) : null}
        {group ? (
          <Paper>
            ...
            <Descriptions
              items={[
                { key: "id", label: "ID", children: group.id },
                { key: "revision", label: "Revision", children: group.revision },
                {
                  key: "updated",
                  label: "Updated",
                  children: new Date(group.updatedAt).toLocaleString(),
                },
              ]}
            />
