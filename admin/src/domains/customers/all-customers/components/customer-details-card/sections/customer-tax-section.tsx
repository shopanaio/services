  ...
  const items = [
    {
      ...
      "data-testid": "customer-tax-identifiers-menu-item",
      onClick: () => onEdit("taxIdentifiers"),
    },
    {
      key: "exemptions",
      label: "Manage tax exemptions",
      "data-testid": "customer-tax-exemptions-menu-item",
  ...
  return (
    <Paper data-testid="customer-tax-section">
      <PaperHeader
        ...
        actions={
          <EditAction
            onEdit={() => onEdit("taxIdentifiers")}
            items={items}
            testId="customer-tax-actions"
          />
        }
      />
        ...
        identifiers.map((item) => (
          <div key={item.id} className={styles.taxRow}>
            ...
            <Flex align="center" gap={6} wrap="wrap">
              <Typography.Text>
                {compactParts([item.identifierType, item.countryCode])}
              </Typography.Text>
            ...
            <Typography.Text type="secondary">
              {item.verifiedAt
                ? `Verified ${formatCustomerDate(item.verifiedAt)}`
                : item.validTo
                  ? `Valid through ${formatCustomerDate(item.validTo)}`
                  : "No expiry"}
            </Typography.Text>
          </div>
        ))
      ...
      {customer.taxIdentifiers.totalCount > identifiers.length ? (
        <Typography.Text type="secondary" className={styles.truncated}>
          Showing {identifiers.length} of {customer.taxIdentifiers.totalCount}
        </Typography.Text>
      ) : null}
            ...
            <Flex vertical gap={2}>
              <Typography.Text>
                {compactParts([
                  ...
                  item.validTo ? `Valid through ${formatCustomerDate(item.validTo)}` : null,
                ])}
              </Typography.Text>
              {item.reason || item.certificateFile ? (
                <Typography.Text type="secondary">
                  ...
                  {item.certificateFile ? (
                    <a
                      href={item.certificateFile.url}
                      target="_blank"
                      rel="noreferrer"
                      aria-label="Open tax exemption certificate"
                    >
                      {" "}
                      <FileOutlined />
                    </a>
      ...
      {customer.taxExemptions.totalCount > exemptions.length ? (
        <Typography.Text type="secondary" className={styles.truncated}>
          Showing {exemptions.length} of {customer.taxExemptions.totalCount}
        </Typography.Text>
      ) : null}
    </Paper>
  );
