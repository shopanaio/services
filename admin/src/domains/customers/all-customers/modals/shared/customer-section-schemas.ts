...
const optionalPhone = z
  ...
  .refine(
    ...
    "Use E.164 format, for example +12025550147",
  );
...
export const customerGroupsSchema = z
  .object({ groupIds: z.array(z.string()), primaryGroupId: z.string().nullable() })
  .refine((value) => !value.primaryGroupId || value.groupIds.includes(value.primaryGroupId), {
    path: ["primaryGroupId"],
    message: "Primary group must be selected",
  });
...
export const customerAddressDraftSchema = z.object({
  ...
  validationStatus: z.enum(CustomerAddressValidationStatus),
  isDefaultShipping: z.boolean(),
  isDefaultBilling: z.boolean(),
});
export const customerAddressManagerSchema = z.object({
  items: z.array(customerAddressDraftSchema),
  deletedIds: z.array(z.string()),
  defaultShippingKey: z.string().nullable(),
  defaultBillingKey: z.string().nullable(),
});

export const customerConsentRowSchema = z
  .object({
    channel: z.enum(CustomerConsentChannel),
    enabled: z.boolean(),
    locked: z.boolean(),
    existingState: z.string().nullable(),
    state: z.enum(CustomerConsentAdminState).nullable(),
    optInLevel: z.enum(CustomerConsentOptInLevel),
    contactPoint: z.string(),
  })
  .superRefine((value, context) => {
    if (value.enabled && !value.state)
      context.addIssue({ code: "custom", path: ["state"], message: "Choose a consent state" });
    if (value.enabled && !value.contactPoint.trim())
      context.addIssue({
        code: "custom",
        path: ["contactPoint"],
...
export const customerTaxIdentifierDraftSchema = z.object({
  key: z.string(),
  id: z.string().optional(),
  identifierType: z.string().trim().min(1, "Type is required"),
  countryCode: z.string(),
  value: z.string().trim().min(1, "Value is required"),
  status: z.enum(CustomerTaxIdentifierStatus),
  isPrimary: z.boolean(),
  validFrom: z.string(),
  validTo: z.string(),
});
export const customerTaxIdentifiersManagerSchema = z.object({
  items: z.array(customerTaxIdentifierDraftSchema),
  deletedIds: z.array(z.string()),
});
export const customerTaxExemptionDraftSchema = z.object({
  key: z.string(),
  id: z.string().optional(),
  ...
  regionCode: z.string(),
  reason: z.string(),
  status: z.enum(CustomerTaxExemptionStatus),
  certificateFile: z
    .object({ id: z.string(), originalName: z.string().nullable().optional(), url: z.string() })
    .nullable(),
  validFrom: z.string(),
  validTo: z.string(),
});
export const customerTaxExemptionsManagerSchema = z.object({
  items: z.array(customerTaxExemptionDraftSchema),
  deletedIds: z.array(z.string()),
});
