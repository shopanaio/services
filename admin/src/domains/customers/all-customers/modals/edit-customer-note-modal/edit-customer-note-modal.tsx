...
import {
  ...
  customerNoteSchema,
  type CustomerNoteValues,
  useCustomerSectionModal,
} from "../shared";

export function EditCustomerNoteModal() {
  const state = useCustomerSectionModal("Merchant note updated");
  const initialized = useRef(false);
  const lastReload = useRef(-1);
  const form = useForm<CustomerNoteValues>({
  ...
  useEffect(() => {
    if (!state.customer || (initialized.current && lastReload.current === state.reloadVersion))
      return;
    reset({ note: state.customer.note ?? "" });
  ...
  const submit = handleSubmit(async (values) => {
    await state.save<CustomerNoteValues>(
      { note: { note: cleanOptional(values.note) } },
      { "note.note": "note" },
      setError,
    );
  });
  return (
    ...
    >
      {state.customer ? (
        <Paper>
          <PaperHeader title="Merchant note" />
          <CustomerFormField
            label="Merchant note"
            error={errors.note?.message}
            help="Visible only to store administrators."
          >
            <Controller
              name="note"
              control={control}
