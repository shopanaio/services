...
import {
  ...
  type CustomerCompanyValues,
  useCustomerSectionModal,
} from "../shared";

export function EditCustomerCompanyModal() {
  const state = useCustomerSectionModal("Company updated");
  const initialized = useRef(false);
  const lastReload = useRef(-1);
  const form = useForm<CustomerCompanyValues>({
    resolver: zodResolver(customerCompanySchema),
    defaultValues: { companyName: "", jobTitle: "" },
    mode: "onChange",
  });
  const {
    control,
  ...
  useEffect(() => {
    ...
    reset({
      ...
      jobTitle: state.customer.jobTitle ?? "",
    });
  ...
  const submit = handleSubmit(async (values) => {
    await state.save<CustomerCompanyValues>(
      {
        company: {
      ...
      { "company.companyName": "companyName", "company.jobTitle": "jobTitle" },
      setError,
    );
  });
  return (
    ...
    >
      {state.customer ? (
        <Paper>
          <PaperHeader title="Company" />
          <CustomerFormField label="Company name" error={errors.companyName?.message}>
            <Controller
              name="companyName"
              control={control}
          ...
          <div style={{ marginTop: 16 }}>
            <CustomerFormField label="Job title" error={errors.jobTitle?.message}>
              <Controller
                name="jobTitle"
                control={control}
