...
import {
  CustomerFormField,
  customerTaxIdentifierDraftSchema,
...
export function EditCustomerTaxIdentifierModal() {
  ...
  const value = payload as CustomerTaxIdentifierItemModalPayload;
  const { styles } = useCustomerFormStyles();
  const form = useForm<CustomerTaxIdentifierDraft>({
    resolver: zodResolver(customerTaxIdentifierDraftSchema),
  ...
  return (
    <ModalLayout
      name="customer-edit-tax-identifier"
      header={
        <ModalHeader
          name="customer-edit-tax-identifier"
          title={value.title}
    ...
    >
      <Paper>
        <PaperHeader title="Tax identifier" />
        <div className={styles.grid}>
          <CustomerFormField label="Type *" error={errors.identifierType?.message}>
            <Controller
              name="identifierType"
              control={control}
              render={({ field }) => (
                <Input {...field} autoFocus status={errors.identifierType ? "error" : undefined} />
              )}
            />
          </CustomerFormField>
          ...
          <div className={styles.full}>
            <CustomerFormField label="Value *" error={errors.value?.message}>
              <Controller
                name="value"
                control={control}
                render={({ field }) => (
                  <Input {...field} status={errors.value ? "error" : undefined} />
                )}
              />
            </CustomerFormField>
          </div>
          <CustomerFormField label="Status *" error={errors.status?.message}>
            <Controller
              ...
              render={({ field }) => (
                <Select
                  {...field}
                  options={Object.values(CustomerTaxIdentifierStatus).map((item) => ({
                    value: item,
