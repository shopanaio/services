...
export function EditCustomerContactModal() {
  ...
  const form = useForm<CustomerContactValues>({
  ...
  useEffect(() => {
    if (!state.customer || (initialized.current && lastReload.current === state.reloadVersion))
      return;
    reset({ email: state.customer.email ?? "", phoneE164: state.customer.phoneE164 ?? "" });
  ...
  const submit = handleSubmit(async (values) => {
    await state.save<CustomerContactValues>(
      {
        contact: {
          email: cleanOptional(values.email.toLowerCase()),
          phoneE164: cleanOptional(values.phoneE164),
        },
      },
      { "contact.email": "email", "contact.phoneE164": "phoneE164" },
      setError,
    );
  });
  return (
    ...
    >
      {state.customer ? (
        <Paper>
          <PaperHeader title="Contact details" />
          <CustomerFormField
            label="Email"
            error={errors.email?.message}
            help={`Current state: ${state.customer.emailVerified ? "Verified" : "Not verified"}`}
          >
            <Controller
              name="email"
              control={control}
          ...
          <div style={{ marginTop: 16 }}>
            <CustomerFormField
              ...
              help={`Current state: ${state.customer.phoneVerified ? "Verified" : "Not verified"}`}
            >
              <Controller
                name="phoneE164"
                control={control}
          ...
          <Alert
            ...
            type="info"
            showIcon
            message="Changing a contact value does not mark it as verified."
          />
          {state.customer.consents.some(
            (consent) =>
              consent.contactPoint === state.customer?.email ||
              consent.contactPoint === state.customer?.phoneE164,
          ) ? (
            <Typography.Paragraph type="secondary" style={{ marginTop: 12, marginBottom: 0 }}>
              Marketing consent contact points are managed separately and will not be changed.
