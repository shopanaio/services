...
export function EditCustomerAddressModal() {
  ...
  const value = payload as CustomerAddressItemModalPayload;
  const { styles } = useCustomerFormStyles();
  const form = useForm<CustomerAddressDraft>({
    resolver: zodResolver(customerAddressDraftSchema),
  ...
  return (
    ...
    >
      <Paper>
        <PaperHeader title="Address identity" />
        <div className={styles.grid}>
          <CustomerFormField label="Label" error={errors.label?.message}>
            <Controller
              name="label"
              control={control}
          ...
          <CustomerFormField label="Company" error={errors.companyName?.message}>
            <Controller
              name="companyName"
              control={control}
          ...
          <CustomerFormField label="Phone" error={errors.phoneE164?.message}>
            <Controller
              name="phoneE164"
              control={control}
              render={({ field }) => (
                <Input {...field} status={errors.phoneE164 ? "error" : undefined} />
              )}
            />
          </CustomerFormField>
        </div>
      ...
      <Paper>
        ...
        <div className={styles.grid}>
          <div className={styles.full}>
            <CustomerFormField label="Address line 1 *" error={errors.address1?.message}>
              <Controller
                name="address1"
                control={control}
                render={({ field }) => (
                  <Input {...field} status={errors.address1 ? "error" : undefined} />
                )}
              />
            </CustomerFormField>
          </div>
          <div className={styles.full}>
            <CustomerFormField label="Address line 2" error={errors.address2?.message}>
              <Controller
                name="address2"
                control={control}
                render={({ field }) => <Input {...field} />}
          ...
          <CustomerFormField label="City *" error={errors.city?.message}>
            <Controller
              name="city"
              control={control}
              render={({ field }) => (
                <Input {...field} status={errors.city ? "error" : undefined} />
              )}
            />
          </CustomerFormField>
          <CustomerFormField label="Region" error={errors.regionName?.message}>
            <Controller
              name="regionName"
              control={control}
              render={({ field }) => <Input {...field} />}
          ...
          <CustomerFormField label="Postal code" error={errors.postalCode?.message}>
            <Controller
              name="postalCode"
              control={control}
              render={({ field }) => <Input {...field} />}
