...
export function CustomerTagModal() {
  ...
  const value = payload as CustomerTagModalPayload;
  const isEdit = value.mode === "edit";
  const query = useCustomerTag(isEdit ? value.entityId : undefined);
  const mutations = useCustomerTagMutations();
  ...
  useEffect(() => {
    ...
    setSelectedCustomers(
      tag.customerAssignments.edges.map((edge) => ({
        id: edge.node.customer.id,
        title: edge.node.customer.displayName,
      })),
    );
    setLocalDirty(false);
  }, [tag]);
  ...
  const existing = useMemo(
    () => tag?.customerAssignments.edges.map((edge) => edge.node) ?? [],
    [tag],
  );
  ...
  const save = async () => {
    ...
    const result = !tag
      ? await mutations.createTag({ name: name.trim() })
      : await mutations.updateTag(tag.id, {
          name: name.trim(),
          assignments: {
            create: customerIds
    ...
    if (!result.tag || result.userErrors.length)
      return setError(
        result.userErrors.map((item) => item.message).join(" ") || "Unable to save tag",
      );
    await value.onSaved?.();
    setDirty(false);
  ...
  const remove = async () => {
    ...
    if (!confirmed) return;
    const result = await mutations.deleteTag({ id: tag.id });
    if (!result.deletedTagId)
      return setError(
        result.userErrors.map((item) => item.message).join(" ") || "Unable to delete tag",
      );
    await value.onSaved?.();
    forcePop();
  };
  if (isEdit && query.loading && !tag)
    return (
      <ModalLayout
        name="customer-tag"
        headerProps={{ title: "Customer tag", onClose: pop, submitButtonProps: null }}
      >
        <Skeleton active />
      </ModalLayout>
    );
  return (
    <ModalLayout
      ...
      header={
        <ModalHeader
          title={tag ? tag.name : "New customer tag"}
          onClose={pop}
          submitButtonProps={{
            children: tag ? "Save" : "Create",
    ...
    >
      <Flex vertical gap={12}>
        ...
        <Paper>
          <PaperHeader title="Tag" icon={<TagsOutlined />} />
          ...
          <Paper>
            ...
            {selectedCustomers.length ? (
              ...
              <Empty
                ...
                description="No customers with this tag"
              />
            )}
          </Paper>
        ) : null}
        {tag ? (
          <Paper>
            ...
            <Descriptions
              items={[
                { key: "normalized", label: "Normalized name", children: tag.normalizedName },
                { key: "id", label: "ID", children: tag.id },
                {
                  key: "updated",
                  label: "Updated",
                  children: new Date(tag.updatedAt).toLocaleString(),
                },
              ]}
            />
