...
export const useDerivedGraph = ({
  ...
  return useMemo(() => {
    ...
    let hasComponentSource = false;
    let hasComponentTarget = false;
    ...
    // 4. Create component node if needed
    if (hasComponentSource || hasComponentTarget) {
      const componentNodeId = "component:main";
      nodeIds.add(componentNodeId);
      nodes.push({
        id: componentNodeId,
        type: "component",
        data: {
