...
interface IDependencyChartInnerProps {
  ...
  onSave?: (rules: ApiProductComponentDependencyRule[]) => Promise<void> | void;
  onCancel: () => void;
}
    ...
    const items: MenuProps["items"] = [
      {
        ...
        icon: <PlusOutlined />,
        label: "Add Rule",
        "data-testid": "dependency-chart-add-rule-menu-item",
      },
    ];
        ...
        <div className={styles.chartArea}>
          ...
          <div className={styles.controls}>
            ...
            >
              <Button
                size="small"
                data-testid="dependency-chart-sort-button"
                icon={
                  ruleSortMode === "desc" ? (
...
export const DependencyChartModal = () => {
  ...
  const handleSave = useCallback(
    async (rules: ApiProductComponentDependencyRule[]) => {
      const shouldClose = await modalPayload?.onSave?.(rules);
      if (shouldClose !== false) pop();
    },
    [modalPayload, pop],
  );
