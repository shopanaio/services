...
describe("prepareMediaFileIds", () => {
  ...
  it("handles files with various ID formats", () => {
    ...
    expect(prepareMediaFileIds(files)).toEqual(["uuid-123-456", "12345", "file_with_underscore"]);
  });
});
...
describe("prepareOptions", () => {
  ...
  it("generates correct slugs for option names with spaces", () => {
    const options = [createMockOption("1", "T-Shirt Size", [{ value: "S", slug: "s" }])];
  ...
  it("generates correct slugs for cyrillic option names", () => {
    const options = [createMockOption("1", "Цвет", [{ value: "Красный", slug: "krasnyi" }])];
...
describe("prepareVariants", () => {
  ...
  it("uses variant id as handle", () => {
    const variants = [createMockVariant("red-small-cotton", "Red / Small / Cotton", [], true)];
  ...
  it("transforms multiple enabled variants", () => {
    ...
    expect(result).toEqual([{ handle: "red-s" }, { handle: "red-m" }, { handle: "blue-s" }]);
  });
});
  ...
  describe("edge cases", () => {
    ...
    it("handles product with hasVariants true but all variants disabled", () => {
      const input = createBaseInput({
        hasVariants: true,
        options: [createMockOption("1", "Color", [{ value: "Red", slug: "red" }])],
        variants: [createMockVariant("red", "Red", [], false)],
      });
