...
Карточка детального просмотра категории каталога. Секционная компоновка, UI-kit компоненты (`Paper`,
`PaperHeader`, `EditAction`), Ant Design, модальное редактирование через хуки.
...
type CategorySection = "info" | "hierarchy" | "products" | "media" | "seo" | "tags";
```
...
Каждая секция обёрнута в `Paper` > `PaperHeader` > контент. Паттерн один к одному с
`product-details-card/sections/*`.
...
- `Tag` color-coded status: ACTIVE → `success` + `CheckOutlined`, DRAFT → `warning` +
  `EditOutlined`, ARCHIVED → `default` + `InboxOutlined`
...
- `Description` — `description.html` через `dangerouslySetInnerHTML`, max 120px с gradient fade
  (`::after`)
...
- `Dropdown` → `[Edit content]` →
  `openEditDescriptionModal({ description, excerpt, product, onSave })`
...
- Если `gallery.length > 12` → последняя видимая ячейка = `+{gallery.length - 11}` (кнопка
  `mediaMoreButton`)
...
- `extra` = issues badge: `WarningOutlined` + `"N issues"` (red), показан если
...
По паттерну `use-product-modals.ts`: импорт `{ push }` из каждого модального хука, `useCallback`
обёртки.
