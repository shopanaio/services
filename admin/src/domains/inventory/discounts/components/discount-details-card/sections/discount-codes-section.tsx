...
import { DiscountCodeStatus, DiscountEffectiveStatus, DiscountMethod } from "@/graphql/types";
...
import { formatDiscountCompactDate, formatDiscountCount } from "../formatters";
...
export function DiscountCodesSection({ discount, onEdit }: DiscountCodesSectionProps) {
    ...
    <Paper className={styles.section} data-testid="discount-codes-section">
      ...
      {codes.length > 0 ? (
        <Flex vertical gap={8}>
              ...
              <div className={styles.codeRow} key={code.id}>
                <div className={styles.codeRowContent}>
                  <DiscountSectionIcon
                    ...
                    shape="square"
                    size={32}
                    tone={code.status === DiscountCodeStatus.Active ? "primary" : "neutral"}
                  />
                  <div className={styles.codeDetails}>
                    <Typography.Text strong className={styles.codeValue}>
                      {code.code}
                    </Typography.Text>
                    <Typography.Text type="secondary" className={styles.codeMeta}>
                      {code.disabledAt
                        ? `Disabled ${formatDiscountCompactDate(code.disabledAt)}`
                        : `${
                            discount.primaryCode === code.code ? "Primary · " : ""
                          }Updated ${formatDiscountCompactDate(code.updatedAt)}`}
                    </Typography.Text>
                  </div>
                  <Flex vertical align="end" className={styles.codeUsage}>
                    <Typography.Text strong className={styles.codeUsagePrimary}>
                      {formatDiscountCount(code.usageCount)} used ·{" "}
                      {formatDiscountCount(code.reservedCount)} reserved
                    </Typography.Text>
                    <Typography.Text type="secondary" className={styles.codeUsageSecondary}>
                      {code.remainingCount == null
                        ? "unlimited"
                        : `${formatDiscountCount(code.remainingCount)} remaining`}
                    </Typography.Text>
                  </Flex>
                  <Flex align="center" justify="flex-end" gap={6} className={styles.codeStatuses}>
                    {discount.primaryCode === code.code && <Tag color="blue">Primary</Tag>}
                    <Tag color={code.status === DiscountCodeStatus.Active ? "success" : "default"}>
                      {code.status === DiscountCodeStatus.Active ? "Active" : "Disabled"}
                    </Tag>
                  </Flex>
                </div>
              </div>
          ...
          {discount.codes.totalCount > codes.length && (
            <Typography.Text type="secondary">
              Showing {codes.length} of {formatDiscountCount(discount.codes.totalCount)} codes
            </Typography.Text>
          )}
        </Flex>
        ...
        <div className={styles.inlineEmpty}>
          <DiscountSectionIcon icon={<LuTicket />} shape="square" size={32} tone="neutral" />
        ...
        <EntityDetailsEmptyState
          ...
          state={{
            ...
            description: "Create a code after the discount method and value are configured.",
          }}
        />
      )}
    </Paper>
