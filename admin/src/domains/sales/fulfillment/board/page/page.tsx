...
export default function FulfillmentPage() {
  ...
  const variables = useMemo(
    () =>
      buildFulfillmentBoardQueryVariables({
        search: deferredSearch,
        includeArchivedCancelled,
        fulfillmentStatus: fulfillmentStatuses,
        orderBy: DEFAULT_FULFILLMENT_ORDER,
      }),
    [deferredSearch, fulfillmentStatuses, includeArchivedCancelled],
  );
  ...
  const openCreateStage = () =>
    stageModal.push({
      mode: "create",
      initialSortIndex: Math.max(-1, ...query.stages.map((stage) => stage.sortIndex)) + 1,
  ...
  const navigation = (
    <Flex align="center" gap="small" wrap="wrap">
      <Input.Search
        ...
        placeholder="Search orders, customers, email or phone"
        style={{ width: 340 }}
      ...
      <Select
        ...
        onChange={setFulfillmentStatuses}
      ...
      />
      <Checkbox
        checked={includeArchivedCancelled}
        onChange={(event) => setIncludeArchivedCancelled(event.target.checked)}
      >
  ...
  return (
    <FulfillmentLayout
      title="Fulfillment"
      count={count}
      loading={query.loading && Boolean(query.connection)}
      onCreate={() => orderModal.push({ mode: "create", onSaved: query.refetch })}
      navigation={navigation}
    >
      {query.error && query.connection ? (
        <Alert
          type="warning"
          showIcon
          closable
          message="The latest board refresh failed"
          description={query.error.message}
        />
      ) : null}
      {query.loading && !query.connection ? (
      ...
      {!query.loading && query.error && !query.connection ? (
        <BoardErrorState error={query.error} onRetry={() => void query.refetch()} />
      ) : null}
      {!query.loading && !query.error && query.connection && !query.connection.totalCount ? (
        <BoardEmptyState onCreate={openCreateStage} />
      ) : null}
      {view ? (
        <FulfillmentBoard
          view={view}
          onRefetch={query.refetch}
          onOpenOrder={(entityId) =>
            orderModal.push({ mode: "edit", entityId, onSaved: query.refetch })
          }
          onEditStage={(column) =>
            stageModal.push({ mode: "edit", entityId: column.id, onSaved: query.refetch })
          }
