...
export const FulfillmentColumn = memo(function FulfillmentColumn({
  ...
  data?: LegacyFulfillmentColumnView;
  onEdit: (column: LegacyFulfillmentColumnView) => void;
}) {
  const { styles } = useFulfillmentBoardStyles();
  if (!data) return <div>Column unavailable</div>;
  return (
    <section className={styles.column} aria-label={`${data.title}, ${count} tickets`}>
      <div className={styles.columnHeader} data-testid="board-column-header">
        <Flex align="center">
          <Button
            ...
            type="text"
            icon={<DragOutlined />}
            aria-label={`Drag ${data.title} column`}
          />
          <Button data-testid="board-column-title" type="text" onClick={() => onEdit(data)}>
            {data.title} <Badge count={count} overflowCount={9999} color="blue" showZero />
          </Button>
        </Flex>
      </div>
      {!dragOverlay ? (
        <div className={styles.columnList} data-testid="board-column">
          {children}
        </div>
      ) : null}
    </section>
