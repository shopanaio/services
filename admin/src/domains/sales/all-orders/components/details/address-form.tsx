...
const useStyles = createStyles(({ token }) => ({
  field: { flex: 1 },
  label: { display: "block", marginBottom: token.marginXXS, fontWeight: 500 },
}));
const fields: Array<{ key: keyof OrderAddressInput; label: string; placeholder: string }> = [
  ...
  { key: "address1", label: "Address line 1", placeholder: "Street and house number" },
  { key: "address2", label: "Address line 2", placeholder: "Apartment, suite, etc." },
  { key: "countryCode", label: "Country", placeholder: "Country code" },
  { key: "city", label: "City", placeholder: "City" },
  { key: "province", label: "State / Province", placeholder: "State or province" },
...
export function AddressForm({
  ...
  onChange: (value: OrderAddressInput) => void;
  readOnly?: boolean;
}) {
        ...
        <Flex key={index} gap="middle">
          {row.map((field) => (
            <div className={styles.field} key={field.key}>
              <Typography.Text className={styles.label}>{field.label}</Typography.Text>
              <Input
                readOnly={readOnly}
