...
export function DraftFulfillment({
  order,
  refetch,
}: {
  order: ApiOrder;
  ...
  const onConfirm = useCallback(
    async (products: IPickableEntity[]) => {
      const newProducts = products.filter(
        (product) => !order.orderItems.some((item) => item.product.id === product.id),
      );
      ...
      for (const product of newProducts) {
        ...
        if (!result.order) {
          message.error(result.userErrors[0]?.message ?? "Failed to add product");
          break;
        }
  ...
  const { openPicker } = useProductPicker({
    selectionMode: "multi",
    initialSelection: order.orderItems.map((item) => item.product.id),
    onConfirm,
  });
  const remove = async (item: ApiOrderItem) => {
    const confirmed = await modal.confirm({
      icon: null,
      title: "Delete product?",
      content: "This item will be removed from the order.",
      okText: "Yes",
      cancelText: "No",
    });
    if (!confirmed) return;
    ...
    if (!result.order)
      return void message.error(result.userErrors[0]?.message ?? "Failed to delete product");
    await refetch();
    ...
    },
    {
      ...
      render: (_: unknown, item: ApiOrderItem) => (
        <Typography.Text strong>
          <OrderPrice amount={item.totalAmount} />
        </Typography.Text>
      ),
    },
    {
      ...
      render: (_: unknown, item: ApiOrderItem) => (
        <Dropdown
          ...
          menu={{
            items: [
              { key: "delete", label: "Delete", danger: true, onClick: () => void remove(item) },
            ],
          }}
        >
          <Button type="text" loading={loading || deleteMutation.loading} icon={<MoreOutlined />} />
        </Dropdown>
      ),
    },
  ];
  return (
    <Paper>
      <PaperHeader
        title="Products"
        actions={
          <Button loading={loading} icon={<PlusOutlined />} onClick={openPicker}>
            Add products
          </Button>
        }
      />
      <Table<ApiOrderItem>
        ...
        locale={{ emptyText: <Typography.Text type="secondary">No products</Typography.Text> }}
      />
    </Paper>
  );
}
