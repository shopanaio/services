...
const multiSourceFacetTypes = new Set<FacetType>([FacetType.Option, FacetType.Feature]);

export const createFacetSchema = z
  .object({
  ...
  .superRefine((values, ctx) => {
    if (values.sources.length > 1 && !multiSourceFacetTypes.has(values.facetType)) {
      ctx.addIssue({
