...
import type { ApiFacetScopesUpdateInput, ApiGenericUserError } from "@/graphql/types";
import { FACET_DETAILS_QUERY, FACET_GRID_QUERY, FACET_SCOPES_UPDATE_MUTATION } from "../graphql";
import type {
  FacetGridFields,
...
interface UseUpdateFacetScopesReturn {
  updateFacetScopes: (input: ApiFacetScopesUpdateInput) => Promise<FacetScopesUpdateResult>;
  loading: boolean;
  error: Error | null;
  reset: () => void;
}
...
export function useUpdateFacetScopes(): UseUpdateFacetScopesReturn {
  ...
  const updateFacetScopes = useCallback(
    async (input: ApiFacetScopesUpdateInput): Promise<FacetScopesUpdateResult> => {
      try {
