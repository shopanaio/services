...
test.describe('Warehouse API', () => {
  ...
  test('should update a warehouse', async ({ api }) => {
    ...
    // Update warehouse
    const { data } = await api.admin.mutation('inventory-api/WarehouseUpdate', {
      variables: {
        input: {
          id: warehouseId,
  ...
  test('should set warehouse as default', async ({ api }) => {
    ...
    const { data: firstData } = await api.admin.mutation('inventory-api/WarehouseCreate', {
      variables: {
        input: {
          code: 'WH-DEFAULT-1',
          name: 'First Default',
    ...
    const { data: secondData } = await api.admin.mutation('inventory-api/WarehouseCreate', {
      variables: {
        input: {
          code: 'WH-DEFAULT-2',
          name: 'Second Default',
  ...
  test('should delete a warehouse', async ({ api }) => {
    ...
    // Delete warehouse
    const { data } = await api.admin.mutation('inventory-api/WarehouseDelete', {
      variables: {
        input: {
          id: warehouseId,
        },
