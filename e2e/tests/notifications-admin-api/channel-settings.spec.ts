...
definePendingContractSuite('Notifications Admin API - channel settings', [
  ...
  ['NTF-CHAN-003', 'setChannelEnabled creates the first channel override with expectedVersion zero'],
  ['NTF-CHAN-004', 'successful channel update increments version exactly once'],
  ['NTF-CHAN-005', 'stale channel expectedVersion returns VERSION_CONFLICT without state change'],
  ['NTF-CHAN-006', 'parallel channel writes with one version produce exactly one success'],
