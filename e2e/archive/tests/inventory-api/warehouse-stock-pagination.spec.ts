...
import type { ApiWarehouseStock, ApiInventoryMutation, ApiInventoryQuery } from '@codegen/admin-gql-v2';
...
async function prepareWarehouseStocks(api: ApiFixtures['api']): Promise<PrepareResult> {
  ...
  // Create a warehouse
  const { data: warehouseData } = await api.admin.mutation('inventory/WarehouseCreate', {
    variables: {
      input: {
        code: prefix,
  ...
  for (const item of stockData) {
    ...
    const { data: productData } = await api.admin.mutation('inventory/ProductCreate', {
      variables: {},
    });
    ...
    await api.admin.mutation('inventory/VariantSetStock', {
      variables: {
        input: {
          variantId,
  ...
  const { data: stockQueryData } = await api.admin.query('inventory/WarehouseStockFindMany', {
    variables: {
      warehouseId: warehouse.id,
      first: 100,
...
createConnectionPaginationTests<ApiWarehouseStock>({
  queryName: 'inventory/WarehouseStockFindMany',
