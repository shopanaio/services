...
test.describe('Media API - External Media', () => {
  test.describe('YouTube videos', () => {
    ...
    test('creates YouTube video with full metadata via mutation', async ({ api }) => {
      ...
      const { data } = await api.admin.mutation('media/FileCreateExternal', {
        variables: {
          input: {
            provider: 'YOUTUBE',
    ...
    test('supports idempotency for YouTube videos', async ({ api }) => {
      ...
      const { data: data1 } = await api.admin.mutation('media/FileCreateExternal', {
        variables: {
          input: {
            provider: 'YOUTUBE',
      ...
      const { data: data2 } = await api.admin.mutation('media/FileCreateExternal', {
        variables: {
          input: {
            provider: 'YOUTUBE',
