...
test.describe('Orders UI test 7', async () => {
  test('Cancellation of confirmed order', async ({
    ...
    await test.step('Main part', async () => {
      ...
      await test.step('Cancel Fulfillment', async () => {
        ...
        await ordersPage.clickCancelFulfillment();

        await ordersPage.clickConfirmCancelFulfillment();
      });

      await test.step('Check timeline', async () => {
        await ordersPage.checkTimeline('Fulfillment cancelled');
      });

      await test.step('Cancel payment', async () => {
        await ordersPage.clickPaymentMenu();
        await ordersPage.clickCancelPayment();

        await ordersPage.clickChangePaymentStatus();
      });

      await test.step('Check timeline', async () => {
        await ordersPage.checkTimeline('Payment cancelled');
      });

      await test.step('Cancel order', async () => {
        await ordersPage.clickCancelOrder();
        await ordersPage.fillCommentToChangeOrder('Comment for cancellation of confirmed order');

        await ordersPage.clickConfirmChangeOrder();
      });

      await test.step('Check timeline', async () => {
        await ordersPage.checkTimeline('Order cancelled');
        await ordersPage.checkCommentInTimeline('Comment for cancellation of confirmed order');
      });
