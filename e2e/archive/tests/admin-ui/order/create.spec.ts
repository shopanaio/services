...
test.describe('Orders UI test 1', async () => {
  test('Order create', async ({
    api,   signInPage, storesPage, customersPage, productsPage, ordersPage }) => {
    ...
    await test.step('Main part', async () => {
      await test.step('Change quantity', async () => {
        ...
        await ordersPage.clickChangeQuantity(1);
        await ordersPage.clickIncrementQuantity(1);
        await ordersPage.clickIncrementQuantity(1);
        await ordersPage.clickSaveInOrderItem();
      ...
      await test.step('Confirm order', async () => {
        ...
        await ordersPage.fillCommentToChangeOrder('Comment to confirm order');
        await ordersPage.clickConfirmChangeOrder();

        //await ordersPage.checkTimeline('Order confirmed');
        await ordersPage.checkCommentInTimeline('Comment to confirm order');
      });

      await test.step('Change cost price', async () => {
        await ordersPage.clickChangeCostPrice(0);
        await ordersPage.fillCostPrice(30);
        await ordersPage.clickSaveInOrderItem();

        await ordersPage.checkTimeline('Order item updated');

        await ordersPage.clickChangeCostPrice(1);
        await ordersPage.fillCostPrice(40);
        await ordersPage.clickSaveInOrderItem();

        await ordersPage.checkTimeline('Order item updated');

        await ordersPage.clickChangeCostPrice(2);
        await ordersPage.fillCostPrice(25);
        await ordersPage.clickSaveInOrderItem();
      ...
      await test.step('Complete order', async () => {
        await ordersPage.clickCompleteOrder();
        await ordersPage.fillCommentToChangeOrder('Comment to complete order');
        await ordersPage.clickConfirmChangeOrder();

        await ordersPage.checkTimeline('Order completed');
        await ordersPage.checkCommentInTimeline('Comment to complete order');
      });

      await test.step('Check total price', async () => {
        await ordersPage.checkTotalPrice('$2.46');
      });
    });
  })
});
