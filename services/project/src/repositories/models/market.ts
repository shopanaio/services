...
export const marketStatusEnum = storeSchema.enum("market_status", [
  "active",
  "inactive",
]);
...
export const market = storeSchema.table(
  ...
  (table) => [
    check(
      "market_code_format_check",
      sql`${table.code} ~ '^[a-z][a-z0-9_-]{0,63}$'`
    ),
    check("market_name_not_blank_check", sql`btrim(${table.name}) <> ''`),
    check(
      "market_timezone_not_blank_check",
    ...
    index("market_store_status_idx").on(
      table.storeId,
      table.status,
      table.id
    ),
    index("market_store_currency_idx").on(
      table.storeId,
...
export const marketLocale = storeSchema.table(
  ...
  (table) => [
    ...
    index("market_locale_store_locale_idx").on(
      table.storeId,
      table.localeCode,
      table.marketId
    ),
