...
import { boolean, check, timestamp, uniqueIndex, uuid, varchar } from "drizzle-orm/pg-core";
import { storeSchema } from "./schema.js";

export const automaticFulfillmentModeEnum = storeSchema.enum("automatic_fulfillment_mode", [
  "all_line_items",
  "gift_cards_only",
  "disabled",
]);
...
export const storeOrderSettings = storeSchema.table(
  "store_order_settings",
  {
    ...
    orderNumberPrefix: varchar("order_number_prefix", { length: 16 }).notNull().default("#"),
    orderNumberSuffix: varchar("order_number_suffix", { length: 16 }),
    requireCheckoutConfirmation: boolean("require_checkout_confirmation").notNull().default(true),
    automaticFulfillmentMode: automaticFulfillmentModeEnum("automatic_fulfillment_mode")
      .notNull()
      .default("disabled"),
    automaticallyArchiveOrders: boolean("automatically_archive_orders").notNull().default(true),
...
export type AutomaticFulfillmentMode = (typeof automaticFulfillmentModeEnum.enumValues)[number];
