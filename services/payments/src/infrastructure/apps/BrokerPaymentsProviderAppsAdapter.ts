...
import {
  providerConfigurationResultSchema,
...
export class BrokerPaymentsProviderAppsAdapter implements PaymentsProviderAppsPort {
  ...
  async listRoutes(params: Apps.ListPaymentProviderRoutesParams) {
    ...
    >("apps.listCapabilityRoutes", params);
  ...
  async resolveRoute(input: {
    ...
    const routes = await this.listRoutes({
      ...
      capability: PAYMENTS_PROVIDER_CAPABILITY,
  ...
  async validateConfiguration(
    ...
    return providerConfigurationResultSchema.parse(
      (await this.execute(route, request)).data,
    ) as Payments.PaymentProviderConfigurationValidationResult;
  }
  async getMethods(
    route: Payments.PaymentProviderRouteSnapshot,
    request: Payments.PaymentProviderMethodDiscoveryRequest,
  ) {
    return providerDiscoveryResultSchema.parse(
      (await this.execute(route, request)).data,
    ) as Payments.PaymentProviderMethodDiscoveryResult;
  }
  async createPayment(
    route: Payments.PaymentProviderRouteSnapshot,
    request: Payments.PaymentProviderCreatePaymentRequest,
  ) {
    return parseProviderOperationResult((await this.execute(route, request)).data);
  }
  ...
  async reconcile(
    ...
    return parseProviderReconcileResult((await this.execute(route, request)).data);
  }
  ...
  private async execute<T>(
    ...
    if (!storeId)
      throw new PaymentsCheckoutError(
        ...
        "Payment provider route is not attached to a store.",
        false,
      );
    ...
    if (
      ...
      throw new PaymentsCheckoutError(
        "PAYMENT_PROVIDER_ROUTE_UNAVAILABLE",
        "Apps returned a different payment provider route.",
        false,
      );
    return result;
  }
  ...
  private snapshot(
    ...
    return {
      protocolVersion: PAYMENTS_PROVIDER_PROTOCOL_VERSION,
