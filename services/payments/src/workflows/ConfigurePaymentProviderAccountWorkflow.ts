...
@Injectable()
export class ConfigurePaymentProviderAccountWorkflow extends BrokerWorkflows<
  ConfigurePaymentProviderAccountWorkflowInput,
  Payments.ConfigurePaymentProviderAccountResult
> {
  constructor(
  ...
  async run(input: ConfigurePaymentProviderAccountWorkflowInput) {
    const result = await this.configure(input);
    return { ...result, workflowId: DBOS.workflowID! };
  }

  @WorkflowStep()
  private configure(input: ConfigurePaymentProviderAccountWorkflowInput) {
