...
const methods = [
  { handle: "one", code: "one", title: "One", provider: "provider", flow: "ONLINE" as const, metadata: null },
  { handle: "two", code: "two", title: "Two", provider: "provider", flow: "ONLINE" as const, metadata: null },
];

describe("payment method customization policy", () => {
  it("applies rename, move and hide without changing executable identities", () => {
    const hidden = new Set<string>();
    const result = applyPaymentMethodCustomizationOperations([...methods], [
  ...
  it("rejects operations for fabricated handles", () => {
    expect(() => applyPaymentMethodCustomizationOperations([...methods], [{ type: "RENAME", methodHandle: "unknown", title: "No" }], new Set())).toThrow("Unknown payment method handle");
  });
});
