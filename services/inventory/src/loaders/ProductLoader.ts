...
export class ProductLoader {
  ...
  constructor(repository: Repository) {
    this.product = new DataLoader<string, Product | null>(async (productIds) => {
      const results = await repository.productLoaderQuery.getByIds(productIds);
    ...
    this.productTranslation = new DataLoader<string, ProductTranslation | null>(async (productIds) => {
      const results = await repository.productLoaderQuery.getTranslationsByProductIds(productIds);
    ...
    this.productOptionIds = new DataLoader<string, string[]>(async (productIds) => {
      const results = await repository.productLoaderQuery.getOptionIdsByProductIds(productIds);
    ...
    this.productFeatureIds = new DataLoader<string, string[]>(async (productIds) => {
      const results = await repository.productLoaderQuery.getFeatureIdsByProductIds(productIds);
      return productIds.map((id) =>
    ...
    this.productOption = new DataLoader<string, ProductOption | null>(async (optionIds) => {
      const results = await repository.productLoaderQuery.getOptionsByIds(optionIds);
    ...
    this.productFeature = new DataLoader<string, ProductFeature | null>(async (featureIds) => {
      const results = await repository.productLoaderQuery.getFeaturesByIds(featureIds);
