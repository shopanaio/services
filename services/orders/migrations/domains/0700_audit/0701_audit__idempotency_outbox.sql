...
CREATE TABLE "orders"."order_outbox" (
  ...
  "status" "orders"."order_outbox_status" NOT NULL DEFAULT 'PENDING',
  ...
  CONSTRAINT "order_outbox_attempts_check" CHECK ("attempts" >= 0),
  CONSTRAINT "order_outbox_published_check" CHECK (
...
CREATE INDEX "order_outbox_pending_idx"
  ON "orders"."order_outbox" ("available_at", "created_at", "id")
  WHERE "status" IN ('PENDING', 'FAILED');

CREATE INDEX "order_outbox_store_order_idx"
  ON "orders"."order_outbox" ("store_id", "order_id", "created_at", "id");

CREATE TRIGGER "order_outbox_touch_updated_at"
BEFORE UPDATE ON "orders"."order_outbox"
FOR EACH ROW
EXECUTE FUNCTION "orders"."touch_updated_at"();
