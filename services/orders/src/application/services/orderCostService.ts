...
export type OrderCost = Readonly<{
  subtotal: Money;
  discountTotal: Money;
  taxTotal: Money;
  shippingTotal: Money;
...
export class OrderCostService {
   ...
   * @param input.orderLines - List of items in cart
  ...
  async computeTotals(input: ComputeTotalsInput): Promise<ComputeTotalsResult> {
    const baseLineItems = this.buildBaseLineItems(input.orderLines);
    ...
    return {
      orderCost,
      orderLinesCost,
      appliedDiscounts: discountsResult.orderDiscounts,
    };
  }
   ...
   *
   * @param orderLines - List of items in cart with their state
  ...
  private async resolveDiscounts(input: ComputeTotalsInput): Promise<{
    ...
    try {
      ...
      return {
        orderDiscounts: input.appliedDiscounts ?? [],
        lineDiscounts: {},
      };
    }
  }
  ...
  /**
   * Calculates the final cart cost with order-level discounts applied.
   *
   * Simplified logic without distributing discounts across items:
   * 1. Calculates subtotal of all items
   * 2. Applies order-level discounts to the total amount
   * 3. Returns final cost indicators
   *
   * @param lineItems - Base items without discounts
   * @param orderDiscounts - Cart-level discounts from pricing service
