...
export const SEARCH_NORMALIZATION_COMPATIBILITY_CORPUS: readonly SearchNormalizationCompatibilityCase[] =
  Object.freeze([
    Object.freeze({
      ...
      expectedSurfaceTerms: Object.freeze(["Running", "shoes", "and", "jackets"]),
    }),
    Object.freeze({
      ...
      expectedSurfaceTerms: Object.freeze(["Красивые", "зимние", "куртки", "и", "ботинки"]),
    }),
    Object.freeze({
      ...
      expectedSurfaceTerms: Object.freeze(["Красиві", "зимові", "куртки", "та", "черевики"]),
    }),
    Object.freeze({
...
export function verifySearchNormalizationCompatibility(
  ...
  for (const entry of SEARCH_NORMALIZATION_COMPATIBILITY_CORPUS) {
    ...
    const actualLexemes = result.lexicalizedQuery.originalUnits.flatMap((unit) => unit.ftsLexemes);
    const actualSurfaceTerms = result.lexicalizedQuery.tokens.flatMap((token) => token.typoTerms);

    assertEqual(entry, "display", result.normalizedQuery.display, entry.expectedDisplay);
    assertEqual(entry, "lookupKey", result.normalizedQuery.lookupKey, entry.expectedLookupKey);
    assertEqual(
      entry,
      "identifierForm",
    ...
    assertArrayEqual(entry, "surfaceTerms", actualSurfaceTerms, entry.expectedSurfaceTerms);
  }
}
...
function assertEqual(
  ...
  if (actual !== expected) {
    throw normalizationFailure(`Compatibility corpus ${entry.id} failed for ${field}`);
  }
}
