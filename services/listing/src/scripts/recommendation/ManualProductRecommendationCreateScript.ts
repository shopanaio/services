...
> {
  @Transactional()
  protected async execute(input: ManualRecommendationCreateParams): Promise<ManualRecommendationResult> {
    ...
    await this.repository.recommendationBuildRequest.lockOrCreateMutex(input.anchorProductId, input.placement);
    const normalized = {
    ...
    const owned = await this.repository.manualProductRecommendation.findOwnedProductIds([input.anchorProductId, input.targetProductId]);
    if (owned.size !== 2) userErrors.push({ message: "Product not found", code: "NOT_FOUND" });
    if (await this.repository.manualProductRecommendation.countForAnchorPlacement(input.anchorProductId, input.placement) >= MAX_MANUAL_ROWS_PER_ANCHOR_PLACEMENT) {
      userErrors.push({ message: "Manual recommendation row limit reached", code: "MANUAL_ROW_LIMIT_EXCEEDED" });
    }
    ...
    try {
      ...
      const request = await this.repository.recommendationBuildRequest.request(input.anchorProductId, input.placement, triggerKey);
      return { recommendation, generation: request.generation, triggerKey, anchorProductId: input.anchorProductId, placement: input.placement, userErrors: [] };
    } catch (error) {
