...
CREATE TABLE listing.search_configuration_audit (
  store_id uuid NOT NULL,
  ...
  CONSTRAINT chk_search_configuration_audit_uuid_v7
    CHECK (
      substring(store_id::text FROM 15 FOR 1) = '7'
      AND substring(audit_id::text FROM 15 FOR 1) = '7'
      AND (resource_id IS NULL OR substring(resource_id::text FROM 15 FOR 1) = '7')
      AND substring(actor_id::text FROM 15 FOR 1) = '7'
    ),
  CONSTRAINT chk_search_configuration_audit_resource_version CHECK (resource_version > 0),
  CONSTRAINT chk_search_configuration_audit_resource
    CHECK (
      (resource_type = 'settings' AND resource_id IS NULL)
      OR (resource_type IN ('synonym_group', 'product_boost') AND resource_id IS NOT NULL)
    ),
  CONSTRAINT chk_search_configuration_audit_action
    CHECK (action IN ('create', 'update', 'delete')),
  CONSTRAINT chk_search_configuration_audit_values
    CHECK (
      (action = 'create' AND before_value IS NULL AND after_value IS NOT NULL)
      OR (action = 'update' AND before_value IS NOT NULL AND after_value IS NOT NULL)
      OR (action = 'delete' AND before_value IS NOT NULL AND after_value IS NULL)
    ),
  CONSTRAINT chk_search_configuration_audit_request CHECK (request_id <> '')
);

CREATE INDEX search_configuration_audit_resource_idx
  ON listing.search_configuration_audit (
    store_id,
    resource_type,
    resource_id,
    resource_version,
