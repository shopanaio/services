...
export class DiscountUpdateCodesScript extends BaseDiscountUpdateScript<DiscountUpdateCodesParams> {
  protected async update(
    ...
    await this.repository.discount.applyCodeChanges(
...
function mapCodeChanges(
  ...
  for (const code of aggregate.codes) {
    if (!deletedIds.has(code.id) && !renamedIds.has(code.id)) {
      normalizedCodes.set(
        code.normalizedCode ?? normalizeCode(code.code),
        code.id,
      );
    }
  }
  ...
  const update = (input.update ?? []).map((item, index) => {
    ...
    if (hasOwn(item, "code")) {
      if (item.code == null) {
        ...
        patch.code = code;
        if (current) {
          normalizedCodes.delete(
            current.normalizedCode ?? normalizeCode(current.code),
          );
        }
        registerNormalizedCode(
          normalizedCodes,
    ...
    if (hasOwn(item, "usageLimit")) {
      ...
      const counter = aggregate.codeUsageCounters.find(
        (value) => value.codeId === item.codeId,
      );
      ...
      if (usageLimit !== null && usageLimit < consumed) {
        errors.push({
          message:
            "Code usage limit cannot be lower than current reserved and consumed usage",
