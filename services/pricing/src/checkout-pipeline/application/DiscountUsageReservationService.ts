  ...
  async reserve(params: Pricing.ReserveCheckoutDiscountUsageParams): Promise<Pricing.ReserveCheckoutDiscountUsageResult> {
    ...
    return this.db.transaction(async (tx) => {
      ...
      for (const group of groups) {
        ...
        if (row && (row.status !== "ACTIVE" || row.checkoutId !== params.checkoutId || row.discountId !== group.discountId || row.codeId !== group.codeId || row.customerId !== group.customerId)) throw new PricingCheckoutError("PRICING_QUOTE_SNAPSHOT_CONFLICT", "Usage reservation idempotency key refers to a different or closed reservation", false);
        if (!row) {
