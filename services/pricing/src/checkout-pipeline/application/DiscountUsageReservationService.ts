  ...
  async reserve(
    ...
    if (
      ...
      throw new PricingCheckoutError(
        ...
        "Usage requirements do not match the final quote",
        false,
      );
    ...
    return this.db.transaction(async (tx) => {
      ...
      for (const group of groups) {
          ...
          await tx
            ...
            .where(
              and(
                ...
                eq(discountUsageReservation.idempotencyKey, groupKey),
              ),
            )
            .limit(1)
        )[0];
        if (row) {
          ...
          const storedApplicationIds = stringArray(metadata.applicationIds).sort((left, right) =>
            left.localeCompare(right),
          );
          const expectedApplicationIds = [...group.applicationIds].sort((left, right) =>
            left.localeCompare(right),
          );
          const replayable = row.status === "ACTIVE" || row.status === "COMMITTED";
          if (
            !replayable ||
            row.checkoutId !== params.checkoutId ||
            row.discountId !== group.discountId ||
            ...
            throw new PricingCheckoutError(
              "PRICING_QUOTE_SNAPSHOT_CONFLICT",
              "Usage reservation idempotency key refers to a different or closed reservation",
              false,
            );
        }
        if (!row) {
          const owner = (
            await tx
              .select()
              .from(discount)
              .where(and(eq(discount.storeId, params.storeId), eq(discount.id, group.discountId)))
              .limit(1)
              .for("update")
          )[0];
          if (!owner)
          ...
          const used = aggregate.reservedCount + aggregate.committedCount - aggregate.reversedCount;
          if (owner.usageLimit !== null && used >= owner.usageLimit)
            throw new PricingCheckoutError(
              "PRICING_DISCOUNT_USAGE_LIMIT_REACHED",
              "Discount usage limit was reached",
              false,
            );
            ...
            const [active, redeemed] = await Promise.all([
              tx
                ...
                .where(
                  and(
                    eq(discountUsageReservation.storeId, params.storeId),
                    eq(discountUsageReservation.discountId, group.discountId),
                    eq(discountUsageReservation.customerId, group.customerId),
                    inArray(discountUsageReservation.status, ["ACTIVE", "COMMITTED"]),
                    ne(discountUsageReservation.checkoutId, params.checkoutId),
                  ),
                )
                .limit(1),
              tx
                ...
                .where(
                  and(
                    ...
                    eq(discountRedemption.customerId, group.customerId),
                    eq(discountRedemption.status, "COMMITTED"),
                  ),
                )
                .limit(1),
            ]);
            if (active.length || redeemed.length)
              throw new PricingCheckoutError(
                "PRICING_DISCOUNT_USAGE_LIMIT_REACHED",
          ...
          if (group.codeId) {
            const code = (
              await tx
                ...
                .from(discountCode)
                .where(
                  and(eq(discountCode.storeId, params.storeId), eq(discountCode.id, group.codeId)),
                )
                .limit(1)
                .for("update")
            )[0];
            ...
            const codeUsed = counter.reservedCount + counter.committedCount - counter.reversedCount;
            if (
              !code ||
              code.status !== "ACTIVE" ||
              code.discountId !== group.discountId ||
              (code.usageLimit !== null && codeUsed >= code.usageLimit)
            )
              throw new PricingCheckoutError(
                "PRICING_DISCOUNT_USAGE_LIMIT_REACHED",
                "Discount code is inactive or its usage limit was reached",
                false,
              );
            await tx
          ...
          [row] = await tx
            .insert(discountUsageReservation)
            .values({
        ...
        for (const applicationId of group.applicationIds)
          reservations.push({ applicationId, reservationId: row!.id });
      }
      return { reservations };
    });
  }
...
async function expireReservations(tx: any, storeId: string, discountId: string): Promise<void> {
  const expired = await tx
    ...
    .where(
      and(
        ...
        eq(discountUsageReservation.discountId, discountId),
        eq(discountUsageReservation.status, "ACTIVE"),
        lt(discountUsageReservation.expiresAt, new Date().toISOString()),
      ),
    )
    .orderBy(discountUsageReservation.id)
    .for("update");
  ...
  await tx
    ...
    .where(
      inArray(
        ...
        expired.map((row: typeof discountUsageReservation.$inferSelect) => row.id),
      ),
    );
  await tx
    .update(discountUsageCounter)
  ...
  for (const row of expired)
    if (row.codeId) byCode.set(row.codeId, (byCode.get(row.codeId) ?? 0) + 1);
  for (const [expiredCodeId, count] of [...byCode].sort(([left], [right]) =>
    left.localeCompare(right),
  ))
    await tx
      .update(discountCodeUsageCounter)
      ...
      .where(
        and(
          ...
          eq(discountCodeUsageCounter.codeId, expiredCodeId),
        ),
      );
}
...
function groupRequirements(
  ...
  >();
  for (const row of requirements.filter((requirement) => requirement.reservationRequired)) {
    const key = `${row.discountId}:${row.codeId ?? ""}:${row.customerId ?? ""}`;
  ...
  return [...groups.values()].sort(
    (a, b) =>
      a.discountId.localeCompare(b.discountId) || (a.codeId ?? "").localeCompare(b.codeId ?? ""),
  );
}
