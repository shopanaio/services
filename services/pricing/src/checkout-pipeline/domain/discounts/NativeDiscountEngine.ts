...
export function collectNativeLineCandidates(
  ...
  return owners(context, inputCodes, snapshot, ["PRODUCT", "ORDER"])
    ...
    .map((candidate) => ({
      ...
      inputCode: candidate.inputIndex === null
        ? null
        : inputCodes[candidate.inputIndex]!,
...
export function initialLineCodeResolutions(
  ...
  return inputCodes.map((inputCode) => {
    ...
    const owner = snapshot.discounts.find(
      (candidate) => candidate.id === code.discountId,
    ) ?? null;
    if (!owner) return rejected(inputCode, null, code, "NOT_ACTIVE");
    const reason = discountEligibility(
      owner,
      code,
      context,
      lines,
...
export function collectNativeShippingCandidates(
  ...
  const inputCodes = preliminary.discountCodeResolutions.map(
    (resolution) => resolution.inputCode,
  );
  ...
  return owners(context, inputCodes, snapshot, ["SHIPPING"])
    ...
    .flatMap((candidate): ShippingDiscountCandidate[] => {
      const rule = snapshot.freeShipping.find(
      ...
      return [{
        ...
        value: { type: "FREE" },
        maximumShippingPrice: rule.maximumShippingPriceMinor === null
          ? null
...
function owners(context: Pricing.PricingCheckoutEvaluationContext, codes: readonly string[], snapshot: DiscountEvaluationSnapshot, classes: Owner["discountClass"][]) {
  const result: Array<{ owner: Owner; code: DiscountEvaluationSnapshot["codes"][number] | null; inputIndex: number | null }> = [];
  for (const owner of snapshot.discounts) { if (owner.calculationStrategy !== "NATIVE" || !classes.includes(owner.discountClass)) continue; if (owner.method === "AUTOMATIC") result.push({ owner, code: null, inputIndex: null }); else codes.forEach((input, index) => { const code = snapshot.codes.find((row) => row.discountId === owner.id && row.normalizedCode === normalize(input)); if (code) result.push({ owner, code, inputIndex: index }); }); }
  return result;
}
export function discountEligibility(owner: Owner, code: DiscountEvaluationSnapshot["codes"][number] | null, context: Pricing.PricingCheckoutEvaluationContext, lines: Pricing.PricingCheckoutQuotedLine[], s: DiscountEvaluationSnapshot): Pricing.PricingCheckoutDiscountCodeRejectionReason | null {
  if (code?.status === "DISABLED") return "DISABLED"; const at = context.effectiveAt; if (owner.state !== "ACTIVE" || owner.startsAt > at || (owner.endsAt !== null && owner.endsAt <= at)) return "NOT_ACTIVE"; if (owner.currency !== context.currencyCode) return "NOT_ACTIVE";
  if (!s.channels.some((row) => row.discountId === owner.id && row.channelCode === context.channelCode)) return "CHANNEL_NOT_ELIGIBLE";
  if (lines.some((line) => line.purchase.type === "ONE_TIME") && !owner.appliesOnOneTimePurchase || lines.some((line) => line.purchase.type === "SUBSCRIPTION") && !owner.appliesOnSubscription) return "PURCHASE_TYPE_NOT_ELIGIBLE";
  const buyer = s.buyerContexts.find((row) => row.discountId === owner.id); if (buyer?.contextType === "CUSTOMERS" && !s.customers.some((row) => row.discountId === owner.id && row.customerId === context.buyerEligibility?.customerId && row.referenceStatus === "VALID")) return "BUYER_NOT_ELIGIBLE"; if (buyer?.contextType === "SEGMENTS" && !s.segments.some((row) => row.discountId === owner.id && context.buyerEligibility?.segmentIds.includes(row.segmentId) && row.referenceStatus === "VALID")) return "BUYER_NOT_ELIGIBLE";
  const count = s.counters.find((row) => row.discountId === owner.id); if (owner.usageLimit !== null && (count?.reservedCount ?? 0n) + (count?.committedCount ?? 0n) - (count?.reversedCount ?? 0n) >= owner.usageLimit) return "USAGE_LIMIT_REACHED"; const cc = code && s.codeCounters.find((row) => row.codeId === code.id); if (code?.usageLimit !== null && code?.usageLimit !== undefined && (cc?.reservedCount ?? 0n) + (cc?.committedCount ?? 0n) - (cc?.reversedCount ?? 0n) >= code.usageLimit) return "USAGE_LIMIT_REACHED";
...
function amountOff(owner: Owner, lines: Pricing.PricingCheckoutQuotedLine[], remaining: Map<string, bigint>, s: DiscountEvaluationSnapshot): Allocation[] {
  ...
  if (rule.allocationMethod === "EACH") {
    ...
    const rawTotal = raw.reduce((sum, row) => sum + row.amount, 0n); const capped = rule.maximumDiscountMinor === null ? rawTotal : min(rawTotal, rule.maximumDiscountMinor);
    return capped === rawTotal ? raw.filter((row) => row.amount > 0n) : allocateProportionally(capped, raw.map((row) => ({ lineId: row.lineId, weight: row.amount })));
  }
  let amount = rule.valueType === "PERCENTAGE" ? total * BigInt(rule.percentageBps!) / 10_000n : min(rule.amountMinor!, total); if (rule.maximumDiscountMinor !== null) amount = min(amount, rule.maximumDiscountMinor);
  return allocateProportionally(amount, eligible.map((line) => ({ lineId: line.lineId, weight: remaining.get(line.lineId)! })));
}
function buyXGetY(owner: Owner, lines: Pricing.PricingCheckoutQuotedLine[], remaining: Map<string, bigint>, s: DiscountEvaluationSnapshot): Allocation[] {
  const rule = s.buyXGetY.find((row) => row.discountId === owner.id); if (!rule) return [];
  const plan = planBuyXGetYUnits({
    ...
    requirement: rule.requirementType === "QUANTITY"
  ...
  for (const selection of plan.benefitSelections) {
    ...
    const per = rule.benefitStrategy === "FREE"
      ? unit
...
export function allocateProportionally(
  ...
  const shares = eligible.map(({ weight }) =>
    min(weight, amount * weight / totalCapacity),
  );
  let remainder =
    amount - shares.reduce((sum, current) => sum + current, 0n);
...
export function canonicalCounterVersions(snapshot: Pick<DiscountEvaluationSnapshot, "counters" | "codeCounters">) {
  return {
    ...
    codeCounters: [...snapshot.codeCounters]
      .sort((left, right) => left.discountId.localeCompare(right.discountId) || left.codeId.localeCompare(right.codeId))
      .map((row) => ({ id: row.codeId, version: String(row.version) })),
  };
}
function rejected(inputCode: string, owner: Owner | null, code: DiscountEvaluationSnapshot["codes"][number] | null, reason: Pricing.PricingCheckoutDiscountCodeRejectionReason): Pricing.PricingCheckoutDiscountCodeResolution { return { inputCode, normalizedCode: normalize(inputCode), status: "REJECTED", discountId: owner?.id ?? null, codeId: code?.id ?? null, reason, message: reason.replaceAll("_", " ").toLowerCase(), retryable: false }; }
...
function normalize(code: string) { return code.trim().toUpperCase(); } function min(a: bigint, b: bigint) { return a < b ? a : b; }
