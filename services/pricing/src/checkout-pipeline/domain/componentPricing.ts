...
export function componentUnitPrice(
  ...
  const delta =
    rule.value.type === "FIXED_AMOUNT"
      ? BigInt(rule.value.amount.amountMinor)
      : (base * BigInt(rule.value.percentageBps)) / 10_000n;
  return rule.operation === "INCREASE" ? base + delta : base > delta ? base - delta : 0n;
}

export function money(amount: bigint, currencyCode: string): Pricing.PricingCheckoutMoney {
