...
export class BrokerCatalogMerchandiseAdapter implements PricingCatalogMerchandisePort {
  ...
  async resolve(
    ...
    let result: unknown;
    try {
      result = await this.broker.call(CatalogCheckoutActions.resolveMerchandise, params);
    } catch (error) {
      throw new PricingCheckoutError(
        "PRICING_CATALOG_UNAVAILABLE",
        "Catalog merchandise action failed",
        true,
        error,
      );
    }
    ...
    if (!parsed.success)
      throw new PricingCheckoutError(
        ...
        "Catalog returned an invalid merchandise response",
        false,
