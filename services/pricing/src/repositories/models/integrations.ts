...
import { externalSyncDirectionEnum, externalSyncStatusEnum, pricingSchema } from "./schema.js";
...
export const discountExternalReference = pricingSchema.table(
  "discount_external_reference",
  {
    ...
    externalType: varchar("external_type", { length: 64 }).notNull().default("discount"),
  ...
  (table) => [
    ...
    check("discount_external_reference_type_check", sql`length(btrim(${table.externalType})) > 0`),
    check("discount_external_reference_id_check", sql`length(btrim(${table.externalId})) > 0`),
    check(
...
export type DiscountExternalReference = typeof discountExternalReference.$inferSelect;
export type NewDiscountExternalReference = typeof discountExternalReference.$inferInsert;
