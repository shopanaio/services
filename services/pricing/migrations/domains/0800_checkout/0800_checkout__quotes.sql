...
CREATE TABLE "pricing"."checkout_preliminary_quote" (
  ...
  CONSTRAINT "checkout_preliminary_quote_text_check" CHECK (length("request_digest") > 0 AND length("revision") > 0)
);
...
CREATE INDEX "checkout_preliminary_quote_revision_idx" ON "pricing"."checkout_preliminary_quote" ("store_id", "revision");
...
CREATE TABLE "pricing"."checkout_final_quote" (
  ...
  CONSTRAINT "checkout_final_quote_text_check" CHECK (length("request_digest") > 0 AND length("revision") > 0)
);
...
CREATE INDEX "checkout_final_quote_revision_idx" ON "pricing"."checkout_final_quote" ("store_id", "revision");
