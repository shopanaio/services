...
> {
  ...
  protected async execute(
    ...
    const product = await this.repository.product.findById(
    ...
    if (operation.type === "productComponentRemove") {
      await this.repository.component.removeByProductId(
        operation.params.productId,
      );
      return { userErrors: [] };
    }
    ...
    if (operation.type === "productComponentConfigurationCreate") {
      const configuration =
        await this.repository.component.createConfiguration(
          operation.params.productId,
    ...
    if (operation.type === "productComponentConfigurationUpdate") {
      const configuration =
        await this.repository.component.updateConfiguration(
          operation.params.configurationId,
          operation.params.name,
        );
      return configuration
        ...
        : this.error(
            "Product component configuration not found",
            ["configurationId"],
            "NOT_FOUND",
          );
    }
    ...
    if (operation.type === "productComponentConfigurationDelete") {
      await this.repository.component.deleteConfiguration(
        operation.params.configurationId,
      );
      return {
    ...
    if (operation.type === "productComponentGroupsSync") {
      await this.repository.component.syncGroups(
        operation.params,
        this.getLocale(),
      );
    } else if (
  ...
  private async validateGroups(
    ...
    const existingGroups =
      await this.repository.component.getGroupsByConfigurationIds([
    ...
    const existingSelections =
      await this.repository.component.getOptionSelectionsByItemIds(
    ...
    const existingValues =
      await this.repository.component.getOptionValueSelectionsBySelectionIds(
        existingSelections.map((selection) => selection.id),
      );
    const valueById = new Map(existingValues.map((value) => [value.id, value]));
    const templates =
      await this.repository.component.getPricingTemplatesByConfigurationIds([
        params.configurationId,
      ]);
    const templateIds = new Set(templates.map((template) => template.id));
    const priceRules =
      await this.repository.component.getPriceRulesByConfigurationIds([
        params.configurationId,
      ]);
    ...
    for (const [groupIndex, group] of params.groups.entries()) {
      ...
      for (const [itemIndex, item] of group.items.entries()) {
        ...
        if (
          item.id &&
          (itemById.get(item.id)?.groupId !== group.id || !group.id)
        ) {
        ...
        if (
          ...
          !templateIds.has(item.pricingTemplateId)
        ) {
          errors.push(
            this.notFound("Pricing template", [
              ...itemPrefix,
              "pricingTemplateId",
            ]),
          );
        ...
        if (item.priceRule) {
          errors.push(
            ...this.validatePriceRule(item.priceRule, [
              ...itemPrefix,
              "priceRule",
            ]),
          );
        ...
        if (item.refProductId) {
          ...
          if (!product) {
            errors.push(
              this.notFound("Referenced product", [
                ...itemPrefix,
                "refProductId",
              ]),
            );
        ...
        if (item.refVariantId) {
          const variant = await this.repository.variant.findById(
            item.refVariantId,
          );
          if (!variant) {
            errors.push(
              this.notFound("Referenced variant", [
                ...itemPrefix,
                "refVariantId",
              ]),
            );
        ...
        for (const [selectionIndex, selection] of (
          ...
          if (
            selection.id &&
            (selectionById.get(selection.id)?.itemId !== item.id || !item.id)
          ) {
            errors.push(
              this.notFound("Option selection", [...selectionPrefix, "id"]),
            );
          }
          const referencedOption =
            await this.repository.option.findById(selection.optionId);
          if (
            !referencedOption ||
            referencedOption.productId !== item.refProductId
          ) {
            errors.push(
              this.notFound("Referenced product option", [
                ...selectionPrefix,
                "optionId",
              ]),
            );
          ...
          if (selection.parentOptionId) {
            const parentOption = await this.repository.option.findById(
              selection.parentOptionId,
            );
            if (
              !parentOption ||
              parentOption.productId !== item.refProductId
            ) {
              errors.push(
                this.notFound("Parent product option", [
                  ...selectionPrefix,
                  "parentOptionId",
                ]),
              );
          ...
          for (const [valueIndex, value] of selection.values.entries()) {
            if (
              value.id &&
              (valueById.get(value.id)?.optionSelectionId !== selection.id ||
                !selection.id)
            ) {
              errors.push(
            ...
            if (value.optionValueId) {
              const optionValue =
                await this.repository.option.findValueById(
                  value.optionValueId,
                );
              if (
                !optionValue ||
  ...
  private async validatePricingTemplates(
    ...
    const [existingTemplates, priceRules] = await Promise.all([
      this.repository.component.getPricingTemplatesByConfigurationIds([
        params.configurationId,
      ]),
      this.repository.component.getPriceRulesByConfigurationIds([
        params.configurationId,
      ]),
    ]);
    ...
    return params.pricingTemplates.flatMap((template, index) => {
      ...
      const errors = this.validatePriceRule(template.priceRule, [
        ...prefix,
        "priceRule",
      ]);
      ...
      if (
        template.priceRule.id &&
        !priceRuleIds.has(template.priceRule.id)
      ) {
        errors.push(
          this.notFound("Price rule", [...prefix, "priceRule", "id"]),
        );
      }
      return errors;
    });
  ...
  private async validateDependencyRules(
    ...
    const errors: UserError[] = [];
    const existingRules =
      await this.repository.component.getDependencyRulesByConfigurationIds([
        params.configurationId,
      ]);
    const ruleById = new Map(existingRules.map((rule) => [rule.id, rule]));
    const existingGroups =
      await this.repository.component.getConditionGroupsByRuleIds(
        existingRules.map((rule) => rule.id),
      );
    const groupById = new Map(existingGroups.map((group) => [group.id, group]));
    const existingConditions =
      await this.repository.component.getConditionsByGroupIds(
        existingGroups.map((group) => group.id),
      );
    const conditionById = new Map(
      existingConditions.map((condition) => [condition.id, condition]),
    );
    const existingActions =
      await this.repository.component.getDependencyActionsByRuleIds(
        existingRules.map((rule) => rule.id),
      );
      ...
      const valid =
        targetType === "ITEM"
          ...
          : targetType === "GROUP"
            ...
            : targetId == null || targetId === params.productId;
      if (!valid) {
    ...
    for (const [ruleIndex, rule] of params.dependencyRules.entries()) {
      ...
      for (const [groupIndex, group] of rule.conditionGroups.entries()) {
        ...
        if (
          group.id &&
          (groupById.get(group.id)?.ruleId !== rule.id || !rule.id)
        ) {
        ...
        for (const [conditionIndex, condition] of group.conditions.entries()) {
          ...
          validateTarget(condition.targetType, condition.targetId, [
            ...prefix,
            "targetId",
          ]);
        }
      ...
      for (const [actionIndex, action] of rule.actions.entries()) {
        ...
        if (
          action.id &&
          (actionById.get(action.id)?.ruleId !== rule.id || !rule.id)
        ) {
          errors.push(this.notFound("Dependency action", [...prefix, "id"]));
        }
        validateTarget(action.targetType, action.targetId, [
          ...prefix,
          "targetId",
        ]);
        if (action.priceRule) {
          errors.push(
            ...this.validatePriceRule(action.priceRule, [
              ...prefix,
              "priceRule",
            ]),
          );
          if (
            action.priceRule.id &&
            !priceRuleIds.has(action.priceRule.id)
          ) {
            errors.push(
              this.notFound("Price rule", [
                ...prefix,
  ...
  private validatePriceRule(
    rule: ProductComponentPriceRuleInput,
    field: string[],
  ): UserError[] {
    const errors: UserError[] = [];
    ...
    if (
      rule.strategy !== "ADJUSTMENT" &&
      (rule.operation != null || rule.valueType != null)
    ) {
      errors.push({
        message:
          "Operation and value type are only allowed for adjustment price rules",
        field,
        code: "FIELD_NOT_ALLOWED",
      });
    }
