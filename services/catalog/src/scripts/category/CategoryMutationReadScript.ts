...
export type CategoryMutationReadQuery =
...
export type CategoryMutationReadResult =
...
export class CategoryMutationReadScript extends BaseScript<
  CategoryMutationReadQuery,
  CategoryMutationReadResult
> {
  protected async execute(query: CategoryMutationReadQuery): Promise<CategoryMutationReadResult> {
    switch (query.type) {
      case "exists":
        return {
          type: query.type,
  ...
  protected handleError(error: unknown): CategoryMutationReadResult {
    throw error;
  }
}
