  ...
  async resolve(
    ...
    const [items, weights, dimensions, locations, stocks] = await Promise.all([
      ...
      this.db
        ...
        .from(itemWeight)
        .where(
          and(eq(itemWeight.storeId, params.storeId), inArray(itemWeight.variantId, variantIds)),
        ),
      this.db
        .select()
        .from(itemDimensions)
        .where(
          and(
            eq(itemDimensions.storeId, params.storeId),
      ...
      this.db
        .select()
        .from(warehouses)
        .where(eq(warehouses.storeId, params.storeId))
...
function rejected(
  line: { lineId: string; variantId: string },
  code: string,
  message: string,
): Catalog.ResolveCheckoutDeliveryLineResult {
  return {
    status: "REJECTED",
    lineId: line.lineId,
    variantId: line.variantId,
