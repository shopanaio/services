...
export class FeatureLoader {
  ...
  public readonly featureValueTranslation: DataLoader<
    string,
  ...
  constructor(repository: Repository) {
    this.featureTranslation = new DataLoader<string, ProductFeatureTranslation | null>(
      async (featureIds) => {
    ...
    this.featureValueIds = new DataLoader<string, string[]>(async (featureIds) => {
      ...
      return featureIds.map((id) => results.filter((v) => v.featureId === id).map((v) => v.id));
    });
    ...
    this.featureValueTranslation = new DataLoader<string, ProductFeatureValueTranslation | null>(
      async (featureValueIds) => {
