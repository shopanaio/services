...
export class FeatureResolver extends CatalogType<string, ProductFeature> {
  async $preload() {
    ...
    if (!feature) {
      throw new PreloadNotFoundError(`Feature with ID ${this.$props} not found`);
    }
    return feature;
  }
  ...
  async parent() {
    ...
    return this.resolvers.feature(parentId);
  }
  ...
  async values() {
    ...
    return Promise.all(ids.map((id) => this.resolvers.featureValue(id)));
  }
}
