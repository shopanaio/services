...
} from "../../../scripts/feature/validation/index.js";
import {
  validateOptionSyncParams,
  type OptionSyncValidationResult,
} from "../../../scripts/option/validation/index.js";
import type {
...
export interface ProductUpdateReadResultMap {
  ...
  bulkFenceCurrent: boolean;
  bulkFenceCurrentForUpdate: boolean;
}
...
> {
  protected async execute(query: ProductUpdateReadQuery): Promise<ProductUpdateReadResponse> {
    switch (query.type) {
      ...
      case "bulkFenceCurrent":
        return {
          type: query.type,
          result: await this.repository.bulkFence.isCurrent(query),
        };
      case "bulkFenceCurrentForUpdate":
        return {
          type: query.type,
          result: await this.repository.bulkFence.isCurrentForUpdate(query),
        };
    }
  }
