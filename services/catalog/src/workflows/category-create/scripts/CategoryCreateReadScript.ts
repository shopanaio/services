...
export type CategoryCreateReadScriptQuery =
...
export type CategoryCreateReadScriptResult =
...
export class CategoryCreateReadScript extends BaseScript<
  CategoryCreateReadScriptQuery,
  CategoryCreateReadScriptResult
> {
  protected async execute(
    query: CategoryCreateReadScriptQuery,
  ): Promise<CategoryCreateReadScriptResult> {
    switch (query.type) {
      case "exists":
        return {
          type: query.type,
  ...
  protected handleError(error: unknown): CategoryCreateReadScriptResult {
    throw error;
  }
}
