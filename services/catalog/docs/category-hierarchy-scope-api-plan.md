...
- `services/catalog/src/api/graphql-admin/schema/category.graphql`
  ...
  - добавить input `CategoryCategoriesMetaInput`.
- `services/catalog/src/api/graphql-admin/schema/base.graphql`
  - добавить аргумент `meta: CategoryCategoriesMetaInput` в `CatalogQuery.categories`.
...
`meta.hierarchyScope.referenceId` уже raw UUID, а не GraphQL global ID:
