...
@Injectable()
export class ExternalRewardWorkflow extends BrokerWorkflows<ExternalRewardInput, void> {
  constructor(@InjectBroker("loyalty") broker: ServiceBroker) {
    super(broker);
  }

  @Workflow("processExternalReward")
  async run(input: ExternalRewardInput): Promise<void> {
    await this.stepIngest(input);
  }

  @WorkflowStep({ retry: { maxAttempts: 10, intervalSeconds: 1, backoffRate: 2 } })
  private async stepIngest(input: ExternalRewardInput): Promise<void> {
