...
export const programVersions = loyaltySchema.table(
  "program_version",
  {
    ...
    activationDelaySeconds: integer("activation_delay_seconds").notNull().default(0),
    ...
    minimumRedeemPoints: numeric("minimum_redeem_points", { mode: "bigint" }).notNull().default(1n),
    maximumRedeemPointsPerOrder: numeric("maximum_redeem_points_per_order", {
      mode: "bigint",
    }),
    maximumOrderPercentageBps: integer("maximum_order_percentage_bps").notNull().default(10000),
    roundingMode: roundingModeEnum("rounding_mode").notNull().default("DOWN"),
    refundPolicy: refundPolicyEnum("refund_policy").notNull().default("PROPORTIONAL"),
    debtPolicy: debtPolicyEnum("debt_policy").notNull().default("TRACK_DEBT"),
    restoredPointsExpiryPolicy: restoredPointsExpiryPolicyEnum("restored_points_expiry_policy")
      .notNull()
    ...
    referenceReconciliationStatus: referenceReconciliationStatusEnum(
      ...
      .notNull()
      .default("VALID"),
    referenceReconciliationCheckedAt: timestamp("reference_reconciliation_checked_at", {
      withTimezone: true,
      mode: "string",
    }),
  },
  (table) => [
    ...
    unique("loyalty_program_version_number_unique").on(table.programId, table.version),
    unique("loyalty_program_version_id_store_unique").on(table.id, table.storeId),
...
export const earningRules = loyaltySchema.table(
  "earning_rule",
  {
    ...
    triggerSchemaVersion: integer("trigger_schema_version").notNull().default(1),
    ...
    actionSchemaVersion: integer("action_schema_version").notNull().default(1),
  ...
  (table) => [
    ...
    unique("loyalty_earning_rule_version_code_unique").on(table.programVersionId, table.code),
    ...
    check("loyalty_earning_rule_code_check", sql`${table.code} ~ '^[a-z][a-z0-9_-]{1,63}$'`),
    check("loyalty_earning_rule_name_check", sql`btrim(${table.name}) <> ''`),
    check("loyalty_earning_rule_priority_check", sql`${table.priority} >= 0`),
    check(
