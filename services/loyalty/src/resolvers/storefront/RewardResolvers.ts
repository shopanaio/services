...
export class LoyaltyAvailableRewardConnectionResolver
  extends BaseStorefrontConnectionResolver<AvailableRewardConnectionInput>
{
  $preload() { return this.$ctx.kernel.repository.reward.getAvailableConnection(this.$props); }
  protected createNodeResolver(id: string) { return this.resolvers.availableReward(id); }
  async nodes() {
...
export class LoyaltyAvailableRewardResolver
  extends LoyaltyStorefrontType<string, RewardEntitlement>
{
  async $preload() {
    const row = await this.$ctx.loaders.rewardEntitlement.load(this.$props);
    const now = Date.parse(this.$ctx.loaders.effectiveAt);
    if (!row || row.status !== "ISSUED" || Date.parse(row.validFrom) > now
  ...
  id() { return this.encodeId(this.$props, GlobalIdEntity.LoyaltyAvailableReward); }

  async reward() {
    const definition = await this.definition();
    const configuration = (await this.$get("configurationSnapshot"))!;
    const copy = copyFrom(configuration, this.$ctx.customer?.language ?? this.$ctx.locale, definition.name);
  ...
  validUntil() { return this.$get("validTo"); }
  async revision() {
