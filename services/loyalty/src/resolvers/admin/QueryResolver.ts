...
export class QueryResolver extends LoyaltyType<Record<string, never>> {
  loyaltyQuery() {
    return this.resolvers.loyaltyQuery();
  }
}
...
export class LoyaltyQueryResolver extends LoyaltyType<Record<string, never>> {
  async node(args: LoyaltyQueryNodeArgs) {
    ...
    switch (parsed.typeName) {
      case GlobalIdEntity.LoyaltyProgram:
        return (await this.$ctx.loaders.program.load(parsed.id))
          ? this.resolvers.program(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyProgramVersion:
        return (await this.$ctx.loaders.programVersion.load(parsed.id))
          ? this.resolvers.programVersion(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyEarningRule:
        return (await this.$ctx.loaders.earningRule.load(parsed.id))
          ? this.resolvers.earningRule(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyRewardDefinition:
        return (await this.$ctx.loaders.rewardDefinition.load(parsed.id))
          ? this.resolvers.rewardDefinition(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyTier:
        return (await this.$ctx.loaders.tier.load(parsed.id))
          ? this.resolvers.tier(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyTierPolicy:
        return (await this.$ctx.loaders.tierPolicy.load(parsed.id))
          ? this.resolvers.tierPolicy(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyAccount:
        return (await this.$ctx.loaders.account.load(parsed.id))
          ? this.resolvers.account(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyTierMembership:
        return (await this.$ctx.loaders.tierMembership.load(parsed.id))
          ? this.resolvers.tierMembership(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyTierMembershipEvent:
        return (await this.$ctx.loaders.tierMembershipEvent.load(parsed.id))
          ? this.resolvers.tierMembershipEvent(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyTransaction:
        return (await this.$ctx.loaders.transaction.load(parsed.id))
          ? this.resolvers.transaction(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyLedgerEntry:
        return (await this.$ctx.loaders.ledgerEntry.load(parsed.id))
          ? this.resolvers.ledgerEntry(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyPointLot:
        return (await this.$ctx.loaders.pointLot.load(parsed.id))
          ? this.resolvers.pointLot(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyLotAllocation:
        return (await this.$ctx.loaders.lotAllocation.load(parsed.id))
          ? this.resolvers.lotAllocation(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyReservation:
        return (await this.$ctx.loaders.reservation.load(parsed.id))
          ? this.resolvers.reservation(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyReservationEvent:
        return (await this.$ctx.loaders.reservationEvent.load(parsed.id))
          ? this.resolvers.reservationEvent(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyEventFact:
        return (await this.$ctx.loaders.eventFact.load(parsed.id))
          ? this.resolvers.eventFact(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyEventEvaluation:
        return (await this.$ctx.loaders.eventEvaluation.load(parsed.id))
          ? this.resolvers.eventEvaluation(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyEarningRuleUsage:
        return (await this.$ctx.loaders.earningRuleUsageById.load(parsed.id))
          ? this.resolvers.earningRuleUsage(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyRewardEntitlement:
        return (await this.$ctx.loaders.rewardEntitlement.load(parsed.id))
          ? this.resolvers.rewardEntitlement(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyRewardEntitlementEvent:
        return (await this.$ctx.loaders.rewardEntitlementEvent.load(parsed.id))
          ? this.resolvers.rewardEntitlementEvent(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyTierRewardBenefit:
        return (await this.$ctx.loaders.tierRewardBenefit.load(parsed.id))
          ? this.resolvers.tierRewardBenefit(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyMonetaryWallet:
        return (await this.$ctx.loaders.monetaryWallet.load(parsed.id))
          ? this.resolvers.monetaryWallet(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyMonetaryTransaction:
        return (await this.$ctx.loaders.monetaryTransaction.load(parsed.id))
          ? this.resolvers.monetaryTransaction(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyMonetaryLedgerEntry:
        return (await this.$ctx.loaders.monetaryEntry.load(parsed.id))
          ? this.resolvers.monetaryEntry(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyMonetaryCreditLot:
        return (await this.$ctx.loaders.monetaryCreditLot.load(parsed.id))
          ? this.resolvers.monetaryCreditLot(parsed.id)
          : null;
      case GlobalIdEntity.LoyaltyMonetaryLotAllocation:
        return (await this.$ctx.loaders.monetaryLotAllocation.load(parsed.id))
          ? this.resolvers.monetaryLotAllocation(parsed.id)
          : null;
      default:
        return null;
    }
  ...
  nodes(args: LoyaltyQueryNodesArgs) {
  ...
  programs(args: LoyaltyQueryProgramsArgs) {
    return this.resolvers.programConnection(normalizeProgramConnection(args));
  }
  ...
  async programVersion(args: LoyaltyQueryProgramVersionArgs) {
    ...
    return (await this.$ctx.loaders.programVersion.load(id))
      ? this.resolvers.programVersion(id)
      : null;
  }
  ...
  async rewardDefinition(args: LoyaltyQueryRewardDefinitionArgs) {
    ...
    return (await this.$ctx.loaders.rewardDefinition.load(id))
  ...
  async customerAccount(args: LoyaltyQueryCustomerAccountArgs) {
    ...
    const account = await this.$ctx.kernel.repository.account.findByCustomerAndProgram(
      customerId,
      programId,
    );
    return account ? this.resolvers.account(account.id) : null;
  }
  accounts(args: LoyaltyQueryAccountsArgs) {
    return this.resolvers.accountConnection(normalizeAccountConnection(args));
  }
  ...
  transactions(args: LoyaltyQueryTransactionsArgs) {
    return this.resolvers.transactionConnection(normalizeTransactionConnection(args));
  }
  ...
  reservations(args: LoyaltyQueryReservationsArgs) {
    return this.resolvers.reservationConnection(normalizeReservationConnection(args));
  }
  ...
  async tierMembership(args: LoyaltyQueryTierMembershipArgs) {
    ...
    return (await this.$ctx.loaders.tierMembership.load(id))
      ? this.resolvers.tierMembership(id)
      : null;
  }
  async tierMemberships(args: LoyaltyQueryTierMembershipsArgs) {
    ...
    return Promise.all(
      (
        await this.$ctx.kernel.repository.tier.listMembershipsForAccount(
          accountId,
          bounded(args.first),
        )
  ...
  async eventEvaluation(args: LoyaltyQueryEventEvaluationArgs) {
    ...
    return (await this.$ctx.loaders.eventEvaluation.load(id))
      ? this.resolvers.eventEvaluation(id)
      : null;
  }
  async eventEvaluations(args: LoyaltyQueryEventEvaluationsArgs) {
    const rows = await this.$ctx.kernel.repository.event.listEvaluationsFiltered({
      eventFactId: args.where.eventFactId
        ? this.decodeId(args.where.eventFactId, GlobalIdEntity.LoyaltyEventFact)
        : undefined,
      accountId: args.where.accountId
        ? this.decodeId(args.where.accountId, GlobalIdEntity.LoyaltyAccount)
        : undefined,
      earningRuleId: args.where.earningRuleId
        ? this.decodeId(args.where.earningRuleId, GlobalIdEntity.LoyaltyEarningRule)
        : undefined,
  ...
  async rewardEntitlement(args: LoyaltyQueryRewardEntitlementArgs) {
    ...
    return (await this.$ctx.loaders.rewardEntitlement.load(id))
      ? this.resolvers.rewardEntitlement(id)
      : null;
  }
  async rewardEntitlements(args: LoyaltyQueryRewardEntitlementsArgs) {
    const rows = await this.$ctx.kernel.repository.reward.listEntitlementsFiltered({
      accountIds: args.where.accountIds?.map((id) =>
        this.decodeId(id, GlobalIdEntity.LoyaltyAccount),
      ),
      rewardDefinitionIds: args.where.rewardDefinitionIds?.map((id) =>
        this.decodeId(id, GlobalIdEntity.LoyaltyRewardDefinition),
      ),
  ...
  async monetaryWallet(args: LoyaltyQueryMonetaryWalletArgs) {
    ...
    return (await this.$ctx.loaders.monetaryWallet.load(id))
      ? this.resolvers.monetaryWallet(id)
      : null;
  }
  ...
  async monetaryTransaction(args: LoyaltyQueryMonetaryTransactionArgs) {
    ...
    return (await this.$ctx.loaders.monetaryTransaction.load(id))
      ? this.resolvers.monetaryTransaction(id)
      : null;
  }
  async monetaryTransactions(args: LoyaltyQueryMonetaryTransactionsArgs) {
    ...
    return Promise.all(
      (
        await this.$ctx.kernel.repository.wallet.listTransactions(walletId, bounded(args.first))
