...
export class LoyaltyAccountResolver extends LoyaltyType<string, Account> {
  ...
  async customerId() {
    return this.encodeId((await this.$get("customerId"))!, GlobalIdEntity.Customer);
  }
  async customer() {
    return { __typename: "Customer" as const, id: await this.customerId() };
  }
  status() {
  ...
  async rewardEntitlements(args: { first?: number | null }) {
    return Promise.all(
      (
        await this.$ctx.kernel.repository.reward.listEntitlements(
          this.$props,
          Math.max(1, Math.min(args.first ?? 100, 100)),
        )
  ...
  async monetaryWallets() {
    return Promise.all(
      (await this.$ctx.kernel.repository.wallet.listForAccount(this.$props)).map(({ id }) =>
  ...
  suspendedReason() {
    return this.$get("suspendedReason");
  }
  openedAt() {
...
export class LoyaltyAccountBalanceResolver extends LoyaltyType<string, AccountBalance> {
  async $preload() {
    ...
    if (!row)
      throw new PreloadNotFoundError(`Loyalty account balance ${this.$props} was not found`);
    return row;
  }
  async pendingPoints() {
  ...
  async lifetimeEarnedPoints() {
    return String(await this.$get("lifetimeEarnedPoints"));
  }
  async lifetimeRedeemedPoints() {
    return String(await this.$get("lifetimeRedeemedPoints"));
  }
  async lifetimeExpiredPoints() {
    return String(await this.$get("lifetimeExpiredPoints"));
  }
...
export class LoyaltyTierMembershipResolver extends LoyaltyType<string, TierMembership> {
  async $preload() {
    ...
    if (!row)
      throw new PreloadNotFoundError(`Loyalty tier membership ${this.$props} was not found`);
    return row;
  }
  id() {
    return this.encodeId(this.$props, GlobalIdEntity.LoyaltyTierMembership);
  }
  async account() {
  ...
  evaluationPeriodStartedAt() {
    return this.$get("evaluationPeriodStartedAt");
  }
  evaluationPeriodEndedAt() {
    return this.$get("evaluationPeriodEndedAt");
  }
  qualifiedAt() {
    return this.$get("qualifiedAt");
  }
  effectiveFrom() {
  ...
  async events() {
    return Promise.all(
      (await this.$ctx.loaders.tierMembershipEvents.load(this.$props)).map(({ id }) =>
        this.resolvers.tierMembershipEvent(id),
      ),
    );
  }
  createdAt() {
...
export class LoyaltyTierMembershipEventResolver extends LoyaltyType<string, TierMembershipEvent> {
  async $preload() {
    ...
    if (!row)
      throw new PreloadNotFoundError(`Loyalty tier membership event ${this.$props} was not found`);
    return row;
  }
  id() {
    return this.encodeId(this.$props, GlobalIdEntity.LoyaltyTierMembershipEvent);
  }
  async account() {
    return this.resolvers.account((await this.$get("accountId"))!);
  }
  async membership() {
    return this.resolvers.tierMembership((await this.$get("membershipId"))!);
  }
  async previousTier() {
    const id = await this.$get("previousTierId");
    return id ? this.resolvers.tier(id) : null;
  }
  async tier() {
    return this.resolvers.tier((await this.$get("tierId"))!);
  }
  ...
  evaluationRevision() {
    return this.$get("evaluationRevision");
  }
  reasonCode() {
