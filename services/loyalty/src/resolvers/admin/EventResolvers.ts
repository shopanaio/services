...
export class LoyaltyEventFactResolver extends LoyaltyType<string, EventFact> {
  ...
  id() {
    return this.encodeId(this.$props, GlobalIdEntity.LoyaltyEventFact);
  }
  producer() {
    return this.$get("producer");
  }
  externalEventId() {
    return this.$get("externalEventId");
  }
  eventType() {
  ...
  async customerId() {
    const id = await this.$get("customerId");
    return id ? this.encodeId(id, GlobalIdEntity.Customer) : null;
  }
  occurredAt() {
    return this.$get("occurredAt");
  }
  payloadSchemaVersion() {
    return this.$get("payloadSchemaVersion");
  }
  payloadHash() {
    return this.$get("payloadHash");
  }
  payload() {
  ...
  async evaluations() {
    return Promise.all(
      (await this.$ctx.loaders.eventEvaluationsByFact.load(this.$props)).map(({ id }) =>
...
export class LoyaltyEventEvaluationResolver extends LoyaltyType<string, EventEvaluation> {
  async $preload() {
    ...
    if (!row)
      throw new PreloadNotFoundError(`Loyalty event evaluation ${this.$props} was not found`);
    return row;
  }
  id() {
    return this.encodeId(this.$props, GlobalIdEntity.LoyaltyEventEvaluation);
  }
  async eventFact() {
    return this.resolvers.eventFact((await this.$get("eventFactId"))!);
  }
  async earningRule() {
    return this.resolvers.earningRule((await this.$get("earningRuleId"))!);
  }
  async account() {
  ...
  async pointsAwarded() {
    const value = await this.$get("pointsAwarded");
  ...
  async monetaryAmount() {
    ...
    return amountMinor === null || currencyCode === null
      ? null
      : { amountMinor: String(amountMinor), currencyCode };
  }
  async transaction() {
    const id = await this.$get("transactionId");
    return id ? this.resolvers.transaction(id) : null;
  }
  resultSchemaVersion() {
    return this.$get("resultSchemaVersion");
  }
  result() {
...
export class LoyaltyEarningRuleUsageResolver extends LoyaltyType<string, EarningRuleUsage> {
  async $preload() {
    ...
    if (!row)
      throw new PreloadNotFoundError(`Loyalty earning rule usage ${this.$props} was not found`);
    return row;
  }
  id() {
    return this.encodeId(this.$props, GlobalIdEntity.LoyaltyEarningRuleUsage);
  }
  async earningRule() {
    return this.resolvers.earningRule((await this.$get("earningRuleId"))!);
  }
  ...
  windowStartedAt() {
    return this.$get("windowStartedAt");
  }
  windowEndedAt() {
    return this.$get("windowEndedAt");
  }
  async occurrenceCount() {
    return String(await this.$get("occurrenceCount"));
  }
  async pointsAwarded() {
    return String(await this.$get("pointsAwarded"));
  }
  monetaryAmounts() {
