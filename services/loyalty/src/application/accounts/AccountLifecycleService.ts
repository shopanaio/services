...
export class AccountLifecycleService {
  ...
  async changeStatus(input: {
    ...
    return this.repository.runInTransaction(async () => {
      ...
      if (current.status === "MERGED")
        throw new LoyaltyDomainError("ACCOUNT_MERGED", "Merged accounts cannot change state");
      if (current.status === "CLOSED") {
        if (input.status === "CLOSED") return current;
        throw new LoyaltyDomainError(
          "ACCOUNT_CLOSED",
          "A closed loyalty account cannot be reopened",
        );
      }
      ...
      const updated = await this.repository.account.updateState(
        ...
        input.status === "SUSPENDED"
          ? {
              status: "SUSPENDED",
              suspendedReason: input.reason ?? "ADMIN_SUSPENDED",
              suspendedAt: input.occurredAt,
              closedAt: null,
            }
          : input.status === "CLOSED"
            ? {
      ...
      if (!updated)
        throw new LoyaltyDomainError(
          ...
          "Loyalty account changed concurrently",
          true,
        );
      return updated;
    });
  ...
  async adjust(input: {
    ...
    return this.repository.runInTransaction(async () => {
      ...
      if (input.direction === "CREDIT") {
        ...
        const activationAt =
          expiresAt !== null && Date.parse(expiresAt) <= Date.parse(input.occurredAt)
  ...
  private async mergeAccounts(
    ...
    if (source.programId !== target.programId)
      throw new LoyaltyDomainError(
        "MERGE_PROGRAM_MISMATCH",
        "Accounts from different programs cannot be merged",
      );
    await this.repository.runInTransaction(async () => {
      ...
      if (!sourceBalance || !targetBalance)
        throw new LoyaltyDomainError("BALANCE_NOT_FOUND", "Merge account balance was not found");
      ...
      for (const bucket of ["AVAILABLE", "PENDING"] as const) {
        const amount =
          bucket === "AVAILABLE" ? sourceBalance.availablePoints : sourceBalance.pendingPoints;
      ...
      if (sourceBalance.reservedPoints > 0n) {
        throw new LoyaltyDomainError(
          "ACCOUNT_HAS_ACTIVE_RESERVATIONS",
          "An account with reserved points cannot be merged",
          true,
        );
      }
      ...
      if (!updated)
        throw new LoyaltyDomainError(
          "ACCOUNT_CONCURRENT_CHANGE",
          "Source account changed concurrently",
          true,
        );
  ...
  private async remainingLotChunks(
    ...
    for (const allocation of allocations)
      spent.set(allocation.lotId, (spent.get(allocation.lotId) ?? 0n) + allocation.points);
    ...
    if (remaining > 0n)
      throw new LoyaltyDomainError(
        "MERGE_LOT_MISMATCH",
        "Account lots do not cover the balance projection",
        true,
      );
    return chunks;
  }
  ...
  private async mergeWallets(
    ...
    for (const wallet of wallets) {
      ...
      const targetWallet = await service.ensureWallet(
        target,
        wallet.walletType,
        wallet.currencyCode,
      );
      ...
      if (!updated)
        throw new LoyaltyDomainError(
          ...
          "Source wallet changed concurrently",
          true,
        );
    }
  }
