...
export class TierEvaluationService {
  ...
  async evaluate(input: {
    ...
    return this.repository.runInTransaction(async () => {
      ...
      const metrics = await this.collectMetrics(input.account, period.start, period.end, input.additionalMetrics);
      const qualified = [...tiers]
        .sort((left, right) => right.rank - left.rank || left.id.localeCompare(right.id))
        .find((tier) => evaluateTierExpression(
      ...
      const maintained = currentTier && currentTier.maintenance
        ? evaluateTierExpression(currentTier.maintenance as unknown as LoyaltyTierMetricExpressionV1, metrics)
        : currentTier !== null;
      const selected = this.selectTier(policy, current, currentTier, qualified, maintained, input.effectiveAt);
      if (!selected) {
        if (current) await this.endMembership(current, input.effectiveAt, "EXPIRED", input.reasonCode ?? "TIER_NOT_MAINTAINED", metrics);
        return null;
      }
      if (current && current.tierId === selected.id) {
        if (policy.requalificationPolicy === "AUTOMATIC"
          && current.effectiveTo !== null
          && Date.parse(current.effectiveTo) <= Date.parse(input.effectiveAt)) {
          return this.renew(input.account, current, selected, policy, period, input.effectiveAt, metrics);
        }
        return current;
      }
      const previousTier = currentTier;
      if (current) await this.endMembership(current, input.effectiveAt, "EXPIRED", "TIER_REPLACED", metrics);
      const effectiveTo = policy.membershipDurationDays === null
        ? null
        : new Date(Date.parse(input.effectiveAt) + policy.membershipDurationDays * 86_400_000).toISOString();
      ...
      const eventType = previousTier === null
        ? "QUALIFIED"
        : selected.rank > previousTier.rank ? "UPGRADED" : "DOWNGRADED";
      await this.repository.tier.appendMembershipEvent({
        accountId: input.account.id,
        ...
        tierId: selected.id,
        eventType,
        evaluationRevision: canonicalHash({ metrics, programVersionId: input.programVersionId, period }),
  ...
  private selectTier(
    ...
    if (policy.downgradePolicy === "END_OF_MEMBERSHIP"
      && (current.effectiveTo === null || Date.parse(current.effectiveTo) > Date.parse(at))) return currentTier;
  ...
  private async collectMetrics(
    ...
    const inWindow = transactions.filter(({ occurredAt }) =>
    ...
    const qualifyingPoints = inWindow.reduce((sum, transaction) => {
      if (transaction.kind === "EARN_PENDING") {
        return sum + BigInt(String(transaction.metadata.awardedPoints ?? transaction.metadata.points ?? "0"));
      }
    ...
    const windowFacts = facts.filter(({ occurredAt }) =>
      Date.parse(occurredAt) >= Date.parse(start) && Date.parse(occurredAt) <= Date.parse(end),
    );
    ...
    const metrics: Record<string, bigint> = {
      ...
      "REFERRAL_COUNT:": BigInt(windowFacts.filter(({ eventType }) => eventType.toLowerCase().includes("referral")).length),
    };
    for (const [currency, amount] of netSpendByCurrency) metrics[`NET_SPEND_MINOR:${currency}`] = amount;
    for (const fact of windowFacts) {
      ...
      for (const [code, value] of Object.entries(custom as Record<string, unknown>)) {
        if (typeof value === "string" && /^(0|[1-9][0-9]*)$/.test(value)) metrics[`CUSTOM:${code}:`] = (metrics[`CUSTOM:${code}:`] ?? 0n) + BigInt(value);
      }
    }
    return metrics;
  }
  ...
  private evaluationPeriod(policy: TierPolicy, account: Account, at: string) {
    ...
    if (policy.windowType === "ROLLING") {
      return { start: new Date(Date.parse(at) - policy.rollingWindowDays! * 86_400_000).toISOString(), end: at };
    }
    if (policy.calendarPeriod === "MONTH") {
      return { start: new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), 1)).toISOString(), end: at };
    }
    if (policy.calendarPeriod === "QUARTER") {
      return { start: new Date(Date.UTC(date.getUTCFullYear(), Math.floor(date.getUTCMonth() / 3) * 3, 1)).toISOString(), end: at };
    }
  ...
  private async renew(
    ...
    const membership = await this.repository.tier.createMembership({
      ...
      qualifiedAt: at,
      effectiveFrom: at,
      effectiveTo: policy.membershipDurationDays === null
        ? null
  ...
  private metricsJson(metrics: TierMetrics): Record<string, string> {
    return Object.fromEntries(Object.entries(metrics).map(([key, value]) => [key, value.toString()]));
  }
}
