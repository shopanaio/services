...
export class CheckoutRedemptionService {
  ...
  async getCustomerAccount(
    ...
    const version = await this.repository.program.findEffectiveVersion(program.id, params.effectiveAt);
    if (!version) return { found: false, code: "PROGRAM_NOT_FOUND", retryable: false };
    const account = await this.repository.account.findByCustomerAndProgram(params.customerId, program.id);
    ...
    const snapshot: LoyaltyAccountSnapshot = {
      ...
      revision: canonicalHash({ accountRevision: account.revision, balanceRevision: balance.revision, membershipRevision: membership?.revision ?? null }),
    };
  ...
  async quote(
    ...
    if (!context.customerId) return { status: "NOT_APPLICABLE", code: "CUSTOMER_REQUIRED", retryable: false };
    ...
    if (program.status !== "ACTIVE") return this.rejected("PROGRAM_INACTIVE", "Loyalty program is not active", false);
    const version = await this.repository.program.findEffectiveVersion(program.id, context.effectiveAt);
    if (!version) return this.rejected("PROGRAM_INACTIVE", "No effective loyalty program version exists", false);
    if (!version.redemptionEnabled) return this.rejected("REDEMPTION_DISABLED", "Loyalty redemption is disabled", false);
    if (program.defaultCurrencyCode !== context.currencyCode
      || context.payableBeforeLoyalty.currencyCode !== context.currencyCode) {
      return this.rejected("CURRENCY_MISMATCH", "Checkout and loyalty program currencies differ", false);
    }
    ...
    const account = await this.repository.account.findByCustomerAndProgram(context.customerId, program.id);
    if (!account) return this.rejected("ACCOUNT_NOT_FOUND", "Customer loyalty account was not found", false);
    if (account.status !== "ACTIVE") return this.rejected("ACCOUNT_NOT_ACTIVE", "Customer loyalty account is not active", false);
    const balance = await this.repository.balance.findByAccountId(account.id);
    if (!balance || balance.availablePoints === 0n) {
      return { status: "NOT_APPLICABLE", code: "NO_AVAILABLE_POINTS", retryable: false };
    }
    const payable = parsePoints(context.payableBeforeLoyalty.amountMinor, "payableBeforeLoyalty.amountMinor");
    if (payable === 0n) {
      return this.rejected("BELOW_MINIMUM_REDEMPTION", "Checkout has no payable amount to redeem", false);
    }
    const maxDiscount = multiplyBasisPoints(payable, version.maximumOrderPercentageBps, "DOWN");
    const byOrder = divideRounded(maxDiscount * version.redeemPoints, version.redeemAmountMinor, "DOWN");
    let allowed = balance.availablePoints < byOrder ? balance.availablePoints : byOrder;
    if (version.maximumRedeemPointsPerOrder !== null && allowed > version.maximumRedeemPointsPerOrder) {
      allowed = version.maximumRedeemPointsPerOrder;
    }
    const requested = params.requestedPoints === null ? allowed : parsePoints(params.requestedPoints, "requestedPoints");
    if (requested > balance.availablePoints) {
      return this.rejected("REQUEST_EXCEEDS_AVAILABLE_POINTS", "Requested points exceed the available balance", false);
    }
    if (requested > allowed) return this.rejected("REQUEST_EXCEEDS_ORDER_LIMIT", "Requested points exceed the order redemption limit", false);
    if (requested < version.minimumRedeemPoints) {
      return this.rejected("BELOW_MINIMUM_REDEMPTION", "Requested points are below the program minimum", false);
    }
    ...
    const quoteBase = {
      ...
      payableAfterLoyalty: { amountMinor: (payable - discountMinor).toString(), currencyCode: context.currencyCode },
      availablePoints: balance.availablePoints.toString(),
      expiresAt,
  ...
  async reserve(
    ...
    try {
      ...
      return await this.repository.runInTransaction(async () => {
        const existing = await this.repository.reservation.findByIdempotencyKey(params.idempotencyKey);
        if (existing) {
          if (existing.requestHash !== params.requestHash) return this.reserveRejected("IDEMPOTENCY_CONFLICT", "Idempotency key conflicts with an existing reservation", false);
        ...
        if (Date.parse(params.quote.expiresAt) <= Date.parse(params.context.requestedAt)) {
          return this.reserveRejected("QUOTE_EXPIRED", "Loyalty redemption quote has expired", false);
        }
        const account = await this.repository.account.lockById(params.quote.accountId);
        if (!account || account.status !== "ACTIVE") return this.reserveRejected("ACCOUNT_NOT_ACTIVE", "Loyalty account is not active", false);
        ...
        if (!balance || balance.availablePoints < points) {
          return this.reserveRejected("INSUFFICIENT_AVAILABLE_POINTS", "Available points changed after the quote", true);
        }
        if (balance.revision !== params.quote.accountRevision) {
          return this.reserveRejected("CONCURRENT_BALANCE_CHANGE", "Loyalty balance changed after the quote", true);
        }
        ...
        const reservation = await this.repository.reservation.create({
          ...
          points,
          discountAmountMinor: parsePositive(params.quote.discount.amountMinor, "quote.discount.amountMinor"),
      ...
      return this.reserveRejected(this.code(error, "CONCURRENT_BALANCE_CHANGE"), this.message(error), this.retryable(error));
    }
  }
  ...
  async commit(
    ...
    try {
      ...
      return await this.repository.runInTransaction(async () => {
        ...
        if (!reservation) return this.commitRejected("RESERVATION_NOT_FOUND", "Loyalty reservation was not found", false);
        if (reservation.status === "COMMITTED" && reservation.orderId === params.orderId) {
          const transaction = await this.repository.ledger.findTransactionByIdempotencyKey(`points:${params.idempotencyKey}`);
          if (!transaction || transaction.requestHash !== params.requestHash) return this.commitRejected("IDEMPOTENCY_CONFLICT", "Commit idempotency conflict", false);
          return { status: "COMMITTED", reservationId: reservation.id, redemptionTransactionId: transaction.id, points: reservation.points.toString(), discount: { amountMinor: reservation.discountAmountMinor.toString(), currencyCode: reservation.currencyCode }, committedAt: reservation.committedAt! };
        }
        if (reservation.status !== "ACTIVE") return this.commitRejected("RESERVATION_NOT_ACTIVE", "Loyalty reservation is not active", false);
        if (Date.parse(reservation.expiresAt) <= Date.parse(params.committedAt)) return this.commitRejected("RESERVATION_EXPIRED", "Loyalty reservation expired before commit", false);
        if (reservation.checkoutId !== params.checkoutId || reservation.checkoutVersion !== params.checkoutVersion) return this.commitRejected("CHECKOUT_MISMATCH", "Checkout does not match the reservation", false);
        if (reservation.quoteId !== params.quoteId || reservation.quoteRevision !== params.quoteRevision) return this.commitRejected("QUOTE_MISMATCH", "Quote does not match the reservation", false);
        ...
        const updated = await this.repository.reservation.updateState(reservation.id, reservation.revision, {
          status: "COMMITTED",
          orderId: params.orderId,
          orderRevision: params.orderRevision,
          committedAt: params.committedAt,
        });
        if (!updated) return this.commitRejected("RESERVATION_NOT_ACTIVE", "Reservation changed concurrently", true);
        await this.repository.reservation.appendEvent({
          reservationId: reservation.id,
          eventType: "COMMITTED",
        ...
        return { status: "COMMITTED", reservationId: reservation.id, redemptionTransactionId: operation.transaction.id, points: reservation.points.toString(), discount: { amountMinor: reservation.discountAmountMinor.toString(), currencyCode: reservation.currencyCode }, committedAt: params.committedAt };
      });
    } catch (error) {
      return this.commitRejected(this.code(error, "RESERVATION_NOT_ACTIVE"), this.message(error), this.retryable(error));
    }
  }
  ...
  async expire(
    ...
    for (const reservation of candidates) {
      const result = await this.releaseOrExpire({
        ...
        requestHash: canonicalHash({ reservationId: reservation.id, expiresAt: reservation.expiresAt }),
      }, "EXPIRED", params.effectiveAt);
      if (result.status === "RELEASED") expired.push({ reservationId: result.reservationId, releaseTransactionId: result.releaseTransactionId, points: result.points });
    }
  ...
  async reverse(
    ...
    try {
      ...
      return await this.repository.runInTransaction(async () => {
        const reservation = await this.repository.reservation.lockById(params.reservationId);
        if (!reservation) return this.reverseRejected("RESERVATION_NOT_FOUND", "Loyalty reservation was not found", false);
        const prior = await this.repository.ledger.findTransactionByIdempotencyKey(`points:${params.idempotencyKey}`);
        if (prior) {
          if (prior.requestHash !== params.requestHash) {
            return this.reverseRejected("IDEMPOTENCY_CONFLICT", "Reversal idempotency conflict", false);
          }
        ...
        if (reservation.status !== "COMMITTED" && reservation.status !== "REVERSED") return this.reverseRejected("RESERVATION_NOT_COMMITTED", "Reservation was not committed", false);
        if (reservation.orderId !== params.orderId) return this.reverseRejected("ORDER_MISMATCH", "Refund order does not match the reservation", false);
        ...
        const alreadyRestored = transactions
          .filter((transaction) => transaction.kind === "RESTORE_REDEEM" && transaction.metadata.reservationId === reservation.id)
          .reduce((sum, transaction) => sum + BigInt(String(transaction.metadata.points ?? "0")), 0n);
        if (alreadyRestored + points > reservation.points) return this.reverseRejected("REVERSAL_EXCEEDS_REDEMPTION", "Restored points exceed the original redemption", false);
        ...
        if (alreadyRestored + points === reservation.points && reservation.status !== "REVERSED") {
          const updated = await this.repository.reservation.updateState(reservation.id, reservation.revision, { status: "REVERSED", reversedAt: params.occurredAt });
          if (!updated) throw new LoyaltyDomainError("RESERVATION_CONCURRENT_CHANGE", "Reservation changed concurrently", true);
          await this.repository.reservation.appendEvent({
            reservationId: reservation.id,
            eventType: "REVERSED",
        ...
        return { status: "REVERSED", reservationId: reservation.id, restoreTransactionId: operation.transaction.id, points: points.toString(), availablePoints: operation.balance.availablePoints.toString() };
      });
    } catch (error) {
      return this.reverseRejected(this.code(error, "RESERVATION_NOT_COMMITTED"), this.message(error), this.retryable(error));
    }
  }
  ...
  private async releaseOrExpire(
    ...
    try {
      ...
      return await this.repository.runInTransaction(async () => {
        ...
        if (reservation.checkoutId !== params.checkoutId) return { status: "REJECTED", code: "RESERVATION_NOT_FOUND", message: "Checkout does not match the reservation", retryable: false };
        const account = await this.requireAccount(reservation.accountId);
        ...
        const updated = await this.repository.reservation.updateState(reservation.id, reservation.revision, target === "EXPIRED"
        ...
        return { status: "RELEASED", reservationId: reservation.id, releaseTransactionId: operation.transaction.id, points: reservation.points.toString(), releasedAt: occurredAt };
      });
    } catch (error) {
      return { status: "REJECTED", code: this.code(error, "IDEMPOTENCY_CONFLICT") as "IDEMPOTENCY_CONFLICT", message: this.message(error), retryable: this.retryable(error) };
    }
  }
  ...
  private quoteMismatch(params: ReserveCheckoutLoyaltyRedemptionParams): string | null {
    ...
    if (quote.basedOnCheckoutVersion !== context.checkoutVersion) return "Checkout version changed after quote";
    if (quote.basedOnPricingQuoteRevision !== context.pricingQuoteRevision) return "Pricing quote changed after loyalty quote";
    if (quote.basedOnCustomerEligibilityRevision !== context.customerEligibilityRevision) return "Customer eligibility changed after loyalty quote";
    if (quote.discount.currencyCode !== context.currencyCode) return "Quote currency does not match checkout";
    return null;
  }
  ...
  private programSnapshot(program: Program, version: ProgramVersion): LoyaltyProgramSnapshot {
    return {
      ...
      policyRevision: canonicalHash({ id: version.id, revision: version.revision, rules: version.rules }),
    };
  }
  ...
  private async originalLotChunks(
    ...
    if (!created) throw new LoyaltyDomainError("RESERVATION_AUDIT_MISSING", "Reservation creation audit is missing", true);
    const entries = await this.repository.ledger.listEntries(created.transactionId);
    const debit = entries.find(({ bucket, pointsDelta }) => bucket === "AVAILABLE" && pointsDelta < 0n);
    if (!debit) throw new LoyaltyDomainError("RESERVATION_AUDIT_MISSING", "Reservation lot debit is missing", true);
    ...
    for (const allocation of allocations.filter(({ debitEntryId }) => debitEntryId === debit.id)) {
      ...
      const resetExpiry = version?.restoredPointsExpiryPolicy === "RESET_FROM_RESTORE"
        && version.pointsExpiryDays !== null;
      result.push({
        points,
        activatedAt: !resetExpiry && source.expiresAt
          && Date.parse(source.expiresAt) <= Date.parse(restoredAt)
          ? source.activatedAt
          : restoredAt,
    ...
    if (remaining > 0n) throw new LoyaltyDomainError("RESERVATION_AUDIT_MISSING", "Reservation allocations do not cover restored points", true);
    return result;
  }
  ...
  private async requireReservationTransaction(reservation: Reservation): Promise<string> {
    const event = (await this.repository.reservation.listEvents(reservation.id)).find(({ eventType }) => eventType === "CREATED");
    if (!event) throw new LoyaltyDomainError("RESERVATION_AUDIT_MISSING", "Reservation transaction audit is missing", true);
    return event.transactionId;
  }

  private reserved(reservation: Reservation, transactionId: string): ReserveCheckoutLoyaltyRedemptionResult {
    return { status: "RESERVED", reservationId: reservation.id, transactionId, accountId: reservation.accountId, points: reservation.points.toString(), discount: { amountMinor: reservation.discountAmountMinor.toString(), currencyCode: reservation.currencyCode }, expiresAt: reservation.expiresAt, reservationRevision: reservation.revision };
  }

  private rejected(code: Extract<QuoteCheckoutLoyaltyRedemptionResult, { status: "REJECTED" }>["code"], message: string, retryable: boolean): QuoteCheckoutLoyaltyRedemptionResult {
    return { status: "REJECTED", code, message, retryable };
  }

  private reserveRejected(code: Extract<ReserveCheckoutLoyaltyRedemptionResult, { status: "REJECTED" }>["code"], message: string, retryable: boolean): ReserveCheckoutLoyaltyRedemptionResult {
    return { status: "REJECTED", code, message, retryable };
  }

  private commitRejected(code: Extract<CommitCheckoutLoyaltyRedemptionResult, { status: "REJECTED" }>["code"], message: string, retryable: boolean): CommitCheckoutLoyaltyRedemptionResult {
    return { status: "REJECTED", code, message, retryable };
  }

  private reverseRejected(code: Extract<ReverseCheckoutLoyaltyRedemptionResult, { status: "REJECTED" }>["code"], message: string, retryable: boolean): ReverseCheckoutLoyaltyRedemptionResult {
    return { status: "REJECTED", code, message, retryable };
  }

  private requireHash(hash: string): void {
    if (!SHA256.test(hash)) throw new LoyaltyDomainError("IDEMPOTENCY_CONFLICT", "requestHash must be a lowercase SHA-256 hash");
  }
...
function earliestExpiry(
  chunks: readonly { expiresAt: string | null }[],
): string | null {
