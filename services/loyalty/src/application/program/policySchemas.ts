...
const catalogSelectorSchema = z.discriminatedUnion("type", [
  ...
  z
    .object({
      type: z.enum(["PRODUCT", "VARIANT", "CATEGORY", "TAG", "FEATURE", "OPTION_VALUE"]),
      ids: z.array(nonBlank).min(1),
    })
    .strict(),
]);
  ...
  z
    .discriminatedUnion("type", [
      z
        .object({
          type: z.enum(["ALL", "ANY"]),
          conditions: z.array(loyaltyConditionExpressionSchema),
        })
        .strict(),
      ...
      z
        .object({
          ...
          segmentIds: z.array(nonBlank).min(1),
        })
        .strict(),
      ...
      z
        .object({ type: z.literal("PAYMENT_METHOD"), paymentMethodCodes: z.array(nonBlank).min(1) })
        .strict(),
...
const earningActions = {
  AWARD_FIXED_POINTS: z
    .object({ type: z.literal("AWARD_FIXED_POINTS"), points: positiveDecimal })
    .strict(),
  AWARD_SPEND_RATIO: z
    .object({
      type: z.literal("AWARD_SPEND_RATIO"),
      points: positiveDecimal,
      amountMinor: positiveDecimal,
    })
    .strict(),
  AWARD_CASHBACK: z
    .object({
      type: z.literal("AWARD_CASHBACK"),
      basisPoints: z.number().int().positive().max(10_000),
      settlement: z.enum(["POINTS", "MONETARY"]),
      currencyCode: z
        .string()
        .regex(/^[A-Z]{3}$/)
        .nullable(),
    ...
    .strict()
    .superRefine((value, ctx) => {
      if ((value.settlement === "MONETARY") !== (value.currencyCode !== null))
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["currencyCode"],
          message: "currencyCode is required only for MONETARY settlement",
        });
    }),
  APPLY_MULTIPLIER: z
    .object({ type: z.literal("APPLY_MULTIPLIER"), multiplierBps: z.number().int().positive() })
    .strict(),
  ISSUE_REWARD: z
    .object({ type: z.literal("ISSUE_REWARD"), rewardDefinitionCode: nonBlank })
    .strict(),
} satisfies Record<LoyaltyEarningActionType, z.ZodTypeAny>;

const earningTriggerConfigSchema = z
  .object({
  ...
  .strict()
  .superRefine((value, ctx) => {
    if (value.eventType && value.eventTypes)
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "eventType and eventTypes are mutually exclusive",
      });
  });
  ...
  .superRefine((value, ctx) => {
    if ((value.type === "ROLLING") !== (value.rollingWindowSeconds !== null)) {
      ctx.addIssue({
        ...
        message: "rollingWindowSeconds is required only for ROLLING windows",
      });
    }
  });

export const loyaltyEarningLimitsSchema = z
  .object({
    startsAt: dateTime.nullable(),
    endsAt: dateTime.nullable(),
    ...
    perAccount: z
      .object({
        maxOccurrences: decimal.nullable(),
        maxPoints: decimal.nullable(),
    ...
    campaign: z
      .object({
        maxOccurrences: decimal.nullable(),
        maxPoints: decimal.nullable(),
        maxMonetaryMinorByCurrency: z.record(z.string().regex(/^[A-Z]{3}$/), decimal),
      })
      .strict()
      .nullable(),
  })
...
export const loyaltyTierExpressionSchema: z.ZodTypeAny = z.lazy(() =>
  z
    .discriminatedUnion("type", [
      z
        .object({ type: z.enum(["ALL", "ANY"]), expressions: z.array(loyaltyTierExpressionSchema) })
        .strict(),
    ...
    .superRefine((value, ctx) => {
      ...
      if ((value.metric === "CUSTOM") !== (value.customMetricCode !== null))
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["customMetricCode"],
          message: "customMetricCode is required only for CUSTOM metrics",
        });
      if ((value.metric === "NET_SPEND_MINOR") !== (value.currencyCode !== null))
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["currencyCode"],
          message: "currencyCode is required only for NET_SPEND_MINOR",
        });
    }),
);
...
const rewardConfigurations = {
  ...
  MONETARY_CREDIT: z
    .object({
      ...
      walletType: z.enum(["CASHBACK", "STORE_CREDIT"]),
    })
    .strict(),
...
export function validateEarningRulePolicy(input: {
  ...
  requireV1(
    [
      ...
      input.conditionSchemaVersion,
      input.actionSchemaVersion,
      input.limitSchemaVersion,
    ],
    "earning rule",
  );
...
export function validateTierPolicyExpression(
  value: unknown,
  field: string,
  schemaVersion = 1,
): void {
...
export function rewardExternalDiscountId(
  rewardType: LoyaltyRewardType,
  configuration: unknown,
): string | null {
  const parsed = rewardConfigurations[rewardType].parse(configuration) as {
...
function requireV1(versions: readonly number[], name: string): void {
  if (versions.some((version) => version !== 1))
    throw new LoyaltyDomainError(
      ...
      `Only ${name} schema version 1 is supported`,
    );
}
