...
import type { LoyaltyCatalogSelector, LoyaltyProgramRulesV1, LoyaltyRewardType } from "../../contracts/types.js";
...
export class BrokerLoyaltyReferenceValidator implements LoyaltyReferenceValidator {
  ...
  async validate(input: {
    ...
    rewardDefinitions?: readonly Readonly<{ rewardType: LoyaltyRewardType; configuration: unknown }>[];
    ...
    ].filter((selector): selector is Exclude<LoyaltyCatalogSelector, { type: "ALL" }> => selector.type !== "ALL");
    for (const rule of input.earningRules ?? []) {
      collectConditionReferences(rule.conditions, segmentIds, selectors);
    }
    const segments: Customers.ValidateLoyaltySegmentReferencesResult = segmentIds.size === 0
      ? { ok: true, missingSegmentIds: [] }
      : await this.broker.call<Customers.ValidateLoyaltySegmentReferencesResult, Customers.ValidateLoyaltySegmentReferencesParams>(
        CustomersLoyaltyActions.validateSegments,
        { storeId: input.storeId, segmentIds: [...segmentIds] },
      );
    if (!segments.ok) throw new LoyaltyDomainError(segments.code, segments.message, segments.retryable);
    for (const id of segments.missingSegmentIds) issues.push({ field: ["eligibility", "segmentIds"], message: `Customer segment ${id} was not found` });

    const catalog: Catalog.ValidateLoyaltyCatalogReferencesResult = selectors.length === 0
      ? { ok: true, missing: [] }
      : await this.broker.call<Catalog.ValidateLoyaltyCatalogReferencesResult, Catalog.ValidateLoyaltyCatalogReferencesParams>(
        CatalogLoyaltyActions.validateReferences,
        { storeId: input.storeId, references: selectors },
      );
    if (!catalog.ok) throw new LoyaltyDomainError(catalog.code, catalog.message, catalog.retryable);
    for (const missing of catalog.missing) for (const id of missing.ids) issues.push({ field: ["earning", "selectors"], message: `${missing.type} ${id} was not found` });
    ...
    const pricing: Pricing.ValidateLoyaltyRewardReferencesResult = discountIds.length === 0
      ? { ok: true, missingDiscountIds: [] }
      : await this.broker.call<Pricing.ValidateLoyaltyRewardReferencesResult, Pricing.ValidateLoyaltyRewardReferencesParams>(
        PricingLoyaltyActions.validateRewardReferences,
        { storeId: input.storeId, discountIds },
      );
    if (!pricing.ok) throw new LoyaltyDomainError(pricing.code, pricing.message, pricing.retryable);
    for (const id of pricing.missingDiscountIds) issues.push({ field: ["rewardDefinitions"], message: `Pricing discount ${id} was not found` });
    return issues;
  }
}
