...
export class AdminDomainLoader {
  ...
  constructor(repository: Repository) {
    ...
    this.eventEvaluationsByFact = new DataLoader(async (ids: readonly string[]) => {
      const rows = grouped(await repository.event.getEvaluationsByFactIds(ids), (row) => row.eventFactId);
    ...
    this.rewardEntitlementEvents = new DataLoader(async (ids: readonly string[]) => {
      const rows = grouped(await repository.reward.getEntitlementEventsByEntitlementIds(ids), (row) => row.entitlementId);
      return ids.map((id) => rows.get(id) ?? []);
    });
    ...
    this.tierRewardBenefits = new DataLoader(async (ids: readonly string[]) => {
      const rows = grouped(await repository.reward.getTierBenefitsByTierIds(ids), (row) => row.tierId);
      return ids.map((id) => rows.get(id) ?? []);
    });
    ...
    this.monetaryEntries = new DataLoader(async (ids: readonly string[]) => {
      const rows = grouped(await repository.wallet.getEntriesByTransactionIds(ids), (row) => row.transactionId);
      return ids.map((id) => rows.get(id) ?? []);
    });
    ...
    this.monetaryCreditLots = new DataLoader(async (ids: readonly string[]) => {
      const rows = grouped(await repository.wallet.getCreditLotsByWalletIds(ids), (row) => row.walletId);
      return ids.map((id) => rows.get(id) ?? []);
    });
    ...
    this.monetaryLotAllocations = new DataLoader(async (ids: readonly string[]) => {
      const rows = grouped(await repository.wallet.getLotAllocationsByLotIds(ids), (row) => row.lotId);
      return ids.map((id) => rows.get(id) ?? []);
    });
  }
}
