...
export class StorefrontLoader {
  ...
  readonly catalogProduct: DataLoader<string, Catalog.CatalogProductSnapshot | null>;
  ...
  constructor(
    ...
    this.activeProgram = new DataLoader(async (storeIds: readonly string[]) => {
      ...
      return storeIds.map((storeId) => (row && row.storeId === storeId ? row : null));
    });
    ...
    this.accountByCustomerProgram = new DataLoader<
      CustomerProgramKey,
      Awaited<ReturnType<Repository["account"]["findByCustomerAndProgram"]>>,
      string
    >(
      async (keys) => {
    ...
    this.catalogProduct = new DataLoader(
        ...
        >("catalog.query", {
          ...
          selection: {
            populate: {
              products: {
                args: {
                  first: Math.min(productIds.length, 100),
    ...
    this.catalogVariant = new DataLoader(
    ...
    this.earningRuleUsage = new DataLoader<
      EarningRuleUsageKey,
      {
    ...
    >(
      async (keys) => {
        const scopes = [
          ...new Set(keys.flatMap(({ accountId }) => [`account:${accountId}`, "campaign"])),
        ];
        ...
        return keys.map(({ earningRuleId, accountId }) => ({
          ...
          campaign:
            rows.find(
              (row) => row.earningRuleId === earningRuleId && row.scopeKey === "campaign",
            ) ?? null,
        }));
      },
    ...
    this.rewardDefinitionUsage = new DataLoader<
      RewardDefinitionUsageKey,
      {
        total: bigint;
        account: bigint;
      },
    ...
    >(
      async (keys) => {
        ...
        return keys.map(({ rewardDefinitionId, accountId }) => ({
          total:
            rows.find(
              (row) => row.rewardDefinitionId === rewardDefinitionId && row.accountId === null,
            )?.quantity ?? 0n,
          account:
            rows.find(
              (row) => row.rewardDefinitionId === rewardDefinitionId && row.accountId === accountId,
            )?.quantity ?? 0n,
        }));
      },
