...
const stageFailureDefaults: Record<CheckoutPipelineStage, readonly [string, string]> = {
  PRICING_PRELIMINARY: [
    "CHECKOUT_PRELIMINARY_PRICING_FAILED",
    "Checkout preliminary pricing could not be calculated.",
  ],
  DELIVERY: ["CHECKOUT_DELIVERY_FAILED", "Checkout delivery options could not be calculated."],
  PRICING_FINAL: [
    "CHECKOUT_FINAL_PRICING_FAILED",
    "Checkout final pricing could not be calculated.",
  ],
...
function sanitizeFailure(stage: CheckoutPipelineStage, error: unknown): CheckoutPipelineStageError {
  ...
  if (
    error instanceof CheckoutPipelineBoundaryError ||
...
function deliveryIssues(result: CalculateDeliveryOptionsResult): CheckoutPipelineIssue[] {
  return result.issues.map((issue) => ({
    ...
    effect: "CONTINUE",
    field:
      issue.carrierServiceAccountId !== null
        ? [
            "delivery",
            "groups",
            issue.groupId ?? "unassigned",
            "carrierServices",
            issue.carrierServiceAccountId,
          ]
        : issue.groupId !== null
...
function finalPricingIssues(
  ...
  const preliminaryByCode = new Map(
    preliminary.discountCodeResolutions.map((resolution) => [
  ...
  finalQuote.discountCodeResolutions.forEach((resolution, index) => {
    if (
      ...
      preliminaryByCode.get(resolution.normalizedCode)?.status === "PENDING"
    ) {
      issues.push({
        stage: "PRICING_FINAL",
        code: `DISCOUNT_CODE_REJECTED_${resolution.reason}`,
...
export class CheckoutPipeline {
  ...
  async recalculate(
    rawRequest: CheckoutRecalculationRequest,
    ...
    try {
      ...
      const runStage = async <TRequest, TResult, TStage extends CheckoutPipelineStage>(input: {
        stage: TStage;
        request: TRequest;
        ...
        const guarded = await runWithCheckoutDeadline(this.runtime, deadlineAt, () =>
          input.call(input.request),
        );
        if (!guarded.accepted) {
      ...
      const buildStageRequest = <TRequest, TResult, TStage extends CheckoutPipelineStage>(
        stage: TStage,
        builder: () => TRequest,
      ...
      const deliveryBuild =
        preliminaryPricing.status === "SUCCESS"
          ...
          : noStageRequest<
            ...
            >();
      const delivery =
        deliveryBuild.failure ??
        (deliveryBuild.request === undefined
          ? this.skipped<CalculateDeliveryOptionsResult, "DELIVERY">(
              "DELIVERY",
              firstBlockingStage!,
              this.runtime.now(),
            )
      ...
      const preliminaryData =
        preliminaryPricing.status === "SUCCESS" ? preliminaryPricing.data : null;

      const finalBuild =
        preliminaryPricing.status === "SUCCESS" && delivery.status === "SUCCESS"
          ? buildStageRequest<
              FinalizePricingQuoteRequest,
              FinalizePricingQuoteResult,
              "PRICING_FINAL"
      ...
      const finalPricing =
        finalBuild.failure ??
        (finalBuild.request === undefined
          ? this.skipped<FinalizePricingQuoteResult, "PRICING_FINAL">(
              "PRICING_FINAL",
              firstBlockingStage!,
              this.runtime.now(),
            )
      ...
      const loyaltyBuild =
        finalPricing.status === "SUCCESS"
          ? buildStageRequest<QuoteCheckoutLoyaltyRequest, CheckoutLoyaltyQuoteResult, "LOYALTY">(
              "LOYALTY",
              () => ({
                context: {
                  ...
                  customerEligibilityRevision:
                    request.context.buyer?.segmentMembershipRevision ?? "guest",
      ...
      const loyalty =
        loyaltyBuild.failure ??
        (loyaltyBuild.request === undefined
          ? this.skipped<CheckoutLoyaltyQuoteResult, "LOYALTY">(
              "LOYALTY",
              firstBlockingStage!,
              this.runtime.now(),
            )
      ...
      const paymentBuild =
        ...
        loyalty.status === "SUCCESS"
          ...
          : noStageRequest<
            ...
            >();
      const payment =
        paymentBuild.failure ??
        (paymentBuild.request === undefined
          ? this.skipped<GetAvailablePaymentMethodsResult, "PAYMENT">(
              "PAYMENT",
              firstBlockingStage!,
              this.runtime.now(),
            )
      ...
      const validationBuild =
        ...
        payment.status === "SUCCESS"
          ? buildStageRequest<ValidateCheckoutRequest, ValidateCheckoutResult, "VALIDATION">(
              "VALIDATION",
              () =>
                parseValidateCheckoutRequest({
      ...
      const validation =
        validationBuild.failure ??
        (validationBuild.request === undefined
          ? this.skipped<ValidateCheckoutResult, "VALIDATION">(
              "VALIDATION",
              firstBlockingStage!,
              this.runtime.now(),
            )
  ...
  private skipped<T, TStage extends CheckoutPipelineStage>(
    stage: TStage,
