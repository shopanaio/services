      ...
      await this.checkoutMutationCoordinator.execute({
        ...
        apply: (draft, current) => {
          ...
          const existing = new Set(
            draft.cartIntent.destinations.map(({ destinationId }) => destinationId),
          );
          for (const id of ids)
            if (!existing.has(id))
          ...
          const removedGroups =
            current.result.delivery.status === "SUCCESS"
              ? new Set(
                  current.result.delivery.data.groups
                    .filter(({ destinationId }) => ids.has(destinationId))
                    .map(({ groupId }) => groupId),
                )
              : new Set<string>();
          draft.cartIntent = {
            ...draft.cartIntent,
            destinations: draft.cartIntent.destinations.filter(
              ({ destinationId }) => !ids.has(destinationId),
            ),
            selectedDeliveryOptions: draft.cartIntent.selectedDeliveryOptions.filter(
              ({ groupId }) => !removedGroups.has(groupId),
            ),
          };
        },
      })
