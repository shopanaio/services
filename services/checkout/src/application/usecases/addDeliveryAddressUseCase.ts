            ...
            for (const lineId of address.checkoutLineIds) {
              if (!rootLineIds.has(lineId)) {
                throw invalidCheckoutMutation(
                  "CHECKOUT_DELIVERY_LINE_INVALID",
                  "A delivery destination can only assign existing root checkout lines.",
                );
              }
              if (assigned.has(lineId)) {
                throw invalidCheckoutMutation(
                  "CHECKOUT_DELIVERY_LINE_ALREADY_ASSIGNED",
                  "A checkout line cannot be assigned to multiple delivery destinations.",
                );
              }
              assigned.add(lineId);
            }
...
export function toPipelineAddress(address: Omit<CheckoutDeliveryAddressFields, "checkoutLineIds">) {
  ...
  if (!address1 || !city || !countryCode) {
    throw invalidCheckoutMutation(
      "CHECKOUT_DELIVERY_ADDRESS_INVALID",
      "Delivery address requires address1, city and countryCode.",
    );
  }
  return {
    id: address.id,
