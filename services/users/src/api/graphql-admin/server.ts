...
export async function startServer(config: ServerConfig) {
  ...
  if (databaseUrl) {
    ...
    console.log("[USERS] Database connected, Kernel initialized");
  } else {
    console.warn(
      "[USERS] No DATABASE_URL configured, running without database"
    );
  }
  ...
  app.log.info(
    `Users GraphQL Admin API ready at http://localhost:${config.port}/graphql`
  );
