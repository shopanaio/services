...
// Broker configuration for platform service
const brokerConfig = {
	// Namespace for grouping services
	namespace: "platform",

	// Unique node identifier
	nodeID: "platform-service",
		...
		reporter: [
			{
				type: "Prometheus",
				options: {
					port: parseInt(process.env.METRICS_PORT || "3036"),
					path: "/metrics",
...
// Create service
broker.createService(PlatformService as any);

// Handle graceful shutdown
const shutdown = async () => {
	try {
		broker.logger.info("🛑 Stopping Platform service...");
		await broker.stop();
		broker.logger.info("✅ Platform service stopped gracefully");
		process.exit(0);
	} catch (error) {
...
broker.start()
	.then(() => {
		broker.logger.info("🚀 Platform service started successfully");
	...
	.catch((error) => {
		broker.logger.error("❌ Failed to start Platform service:", error);
		process.exit(1);
	});
