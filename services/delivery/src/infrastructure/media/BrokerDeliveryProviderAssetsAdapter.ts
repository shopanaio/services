...
export class BrokerDeliveryProviderAssetsAdapter implements DeliveryProviderAssetsPort {
  ...
  async ingestLabel(
    input: Parameters<DeliveryProviderAssetsPort["ingestLabel"]>[0],
  ): Promise<Awaited<ReturnType<DeliveryProviderAssetsPort["ingestLabel"]>>> {
    try {
      ...
      if (
        ...
        url.username ||
        url.password ||
        (url.port && !input.policy.allowedPorts.includes(Number(url.port)))
      ) {
      ...
      if (!address || addresses.some(({ address }) => !isPublicAddress(address))) {
        return rejected(
          "DELIVERY_LABEL_DNS_REJECTED",
          "The label host resolved to a non-public address",
        );
      }
      ...
      const body = await fetchPinned(
        ...
        input.policy.fetchTimeoutMs,
        expectedMime,
      );
      if (!matchesFormat(body, input.label.format))
        return rejected(
          "DELIVERY_LABEL_CONTENT_INVALID",
          "The label content does not match its declared format",
        );
      ...
      if (!result.fileId || result.userErrors.length > 0) {
        return rejected(
          result.userErrors[0]?.code ?? "DELIVERY_LABEL_MEDIA_FAILED",
          result.userErrors[0]?.message ?? "The label could not be stored",
          true,
        );
      }
      return {
        status: "INGESTED",
        label: {
          format: input.label.format,
          mediaId: result.fileId,
          assetPolicyRevision: input.policy.revision,
          expiresAt: input.label.expiresAt,
        },
      };
    } catch (error) {
      return rejected(
        "DELIVERY_LABEL_INGEST_FAILED",
        error instanceof Error ? error.message : "The label could not be ingested",
        true,
      );
    }
  }
    ...
    const req = request(
      {
        ...
        headers: { host: url.host, accept: expectedMime },
      },
      (response) => {
        if (response.statusCode !== 200) {
          response.resume();
          reject(new Error(`Label endpoint returned ${response.statusCode ?? "no status"}`));
          return;
        }
        ...
        if (contentType !== expectedMime) {
          response.resume();
          reject(new Error("Label content type does not match its declared format"));
          return;
        }
        const declared = Number(response.headers["content-length"] ?? 0);
        if (declared > maxBytes) {
          response.resume();
          reject(new Error("Label exceeds the configured size limit"));
          return;
        }
        const chunks: Buffer[] = [];
        let length = 0;
        response.on("data", (chunk: Buffer) => {
          length += chunk.length;
          if (length > maxBytes) {
            req.destroy(new Error("Label exceeds the configured size limit"));
            return;
          }
          chunks.push(chunk);
        });
