...
export class DeliveryShipmentService {
  ...
  async prepareCreate(
    ...
    now: string,
  ): Promise<PreparedShipmentOperation | { providerNotConfigured: true; duplicate: boolean }> {
    const scope = `delivery.shipment.create:${params.fulfillmentOrderId}`;
    const idempotency = idem(scope, params.idempotencyKey, params);
    const duplicate = await this.deps.repository.shipments.findOperationByIdempotency(
      params.storeId,
      scope,
      params.idempotencyKey,
    );
    if (duplicate) {
      if (duplicate.idempotency.requestHash !== idempotency.requestHash)
      ...
      return {
        ...
        request: createRequest(shipment, duplicate, params, now),
        duplicate: true,
      };
    }
    const availability = await this.deps.fulfillment.getShipmentPlan(params);
    if (availability.status !== "READY")
      throw new Error(`${availability.code}:${availability.message}`);
    const plan = availability.plan;
    if (!plan.shipmentProvider) return { providerNotConfigured: true, duplicate: false };
    if (!plan.fulfillmentOrder.deliveryMethod)
      throw new Error("DELIVERY_METHOD_COMMITMENT_MISSING");
    ...
    const shipment: Delivery.DeliveryShipmentSnapshot = {
      shipmentId,
      organizationId: plan.fulfillmentOrder.organizationId,
      storeId: params.storeId,
      orderId: plan.fulfillmentOrder.orderId,
      fulfillmentOrderId: params.fulfillmentOrderId,
      fulfillmentOrderRevision: plan.fulfillmentOrder.revision,
      shipmentPlanHash: plan.planHash,
      checkoutId: plan.fulfillmentOrder.checkoutId,
      deliveryGroupId: plan.fulfillmentOrder.deliveryGroupId,
      ...
      parcels: [],
      selectedDeliveryMethod: plan.fulfillmentOrder.deliveryMethod,
      origin: plan.origin,
      ...
      lastProviderShipmentSequence: null,
      lastFailure: null,
      lastFulfillmentState: null,
      revision: 1,
      createdAt: now,
      updatedAt: now,
    };
    ...
    const committed = await this.deps.repository.shipments.commit(
      mutation(
        ...
        null,
        idempotency,
        [{ type: "delivery.shipment.created", payload: created }],
      ),
    );
    ...
    return {
      shipment: reservedShipment,
      operation,
      route,
      request: createRequest(reservedShipment, operation, params, now),
      duplicate: false,
    };
  }
  ...
  async invoke(prepared: PreparedShipmentOperation) {
    if (prepared.operation.type === "CREATE")
      return this.deps.apps.createShipment(
        prepared.route as any,
        prepared.request as Delivery.DeliveryProviderCreateShipmentRequest,
      );
    if (prepared.operation.type === "CANCEL")
      return this.deps.apps.cancelShipment(
        prepared.route as any,
        prepared.request as Delivery.DeliveryProviderCancelShipmentRequest,
      );
    if (prepared.operation.type === "GET")
      return this.deps.apps.getShipment(
        prepared.route as any,
        prepared.request as Delivery.DeliveryProviderGetShipmentRequest,
      );
    return this.deps.apps.reconcileShipment(
      prepared.route as any,
      prepared.request as Delivery.DeliveryProviderReconcileShipmentRequest,
    );
  }
  ...
  async prepareCancel(
    ...
    now: string,
  ): Promise<PreparedShipmentOperation> {
  ...
  async complete(
    ...
    if (result.status === "FAILED") {
      await this.failPrepared(
        ...
        occurredAt,
        result.failure,
        result.providerShipmentReference,
      );
      if (prepared.operation.type === "CREATE" && !result.failure.acceptedByProvider)
        await this.releaseFulfillment(prepared.shipment, occurredAt);
      return;
    }
    const providerReference = result.providerShipmentReference;
    if (
      prepared.shipment.providerShipmentReference &&
      prepared.shipment.providerShipmentReference !== providerReference
    ) {
      throw new Error("DELIVERY_PROVIDER_SHIPMENT_REFERENCE_CONFLICT");
    }
    const observedState =
      result.status === "PENDING" && result.shipmentState === "CANCELLING"
        ? null
        : result.shipmentState;
    const parcels = "parcels" in result ? result.parcels : [];
    const events = "events" in result ? result.events : [];
    const current = await this.requiredShipment(
      prepared.shipment.storeId,
      prepared.shipment.shipmentId,
    );
    const account = await this.deps.repository.providerAccounts.getById(
      current.storeId,
    ...
    if (!capabilities.supportsMultipleParcels && parcels.length > 1)
      throw new Error("DELIVERY_PROVIDER_MULTIPLE_PARCELS_UNSUPPORTED");
    if (!capabilities.supportsLabels && parcels.some(({ labels }) => labels.length > 0))
      throw new Error("DELIVERY_PROVIDER_LABELS_UNSUPPORTED");
    if (
      !capabilities.supportsTracking &&
      (events.length > 0 || parcels.some(({ tracking }) => tracking.length > 0))
    )
      throw new Error("DELIVERY_PROVIDER_TRACKING_UNSUPPORTED");
    const normalized = await this.deps.normalizer.normalize({
      current,
    ...
    if (normalized.status === "REJECTED") {
      await this.failPrepared(
        ...
        correlationId,
        occurredAt,
        failure("INVALID_REQUEST", normalized.code, normalized.message, normalized.retryable),
      );
      return;
    }
    if (prepared.operation.type === "CREATE" && result.status === "SUCCEEDED") {
      ...
      if (current.packages.some(({ packageId }) => !assigned.has(packageId)))
        throw new Error("DELIVERY_PROVIDER_PACKAGE_COVERAGE_INCOMPLETE");
    }
    const latestEvent =
      [...normalized.events]
    ...
    const transition =
      observedState === null
        ...
        : this.deps.transitions.evaluate({
            ...
            occurredAt,
            providerEventId:
              latestEvent?.providerEventId ?? `${prepared.operation.operationId}:completion`,
    ...
    const next: Delivery.DeliveryShipmentSnapshot = {
      ...current,
      state,
      providerShipmentReference: providerReference,
      ...
      lastProviderShipmentSequence:
        latestEvent?.providerShipmentSequence ?? current.lastProviderShipmentSequence,
      lastFailure: null,
    ...
    const completionIdempotency = idem(
      `delivery.shipment.complete:${operation.operationId}`,
      completionResultHash,
      result,
    );
    const domainEvents = await this.observationEvents(
      current,
      next,
      operation,
      prepared.route,
    ...
    const inbox = providerEventId
      ? {
          storeId: current.storeId,
          providerAccountId: current.providerAccountId,
          providerEventId,
          providerShipmentSequence: latestEvent?.providerShipmentSequence ?? null,
          eventHash: completionIdempotency.requestHash,
          occurredAt,
        }
      : null;
    const committed = await this.deps.repository.shipments.commit(
    ...
    if (!["APPLIED", "DUPLICATE"].includes(committed.status))
      throw new Error(`DELIVERY_COMPLETION_${committed.status}`);
    if (committed.status === "APPLIED" || committed.status === "DUPLICATE") {
      await this.propagateState(
        await this.requiredShipment(current.storeId, current.shipmentId),
        occurredAt,
      );
    }
  }
  ...
  async completeProviderOperation(
    ...
    const operations = await this.deps.repository.shipments.listOperations(
      context.storeId,
      params.shipmentId,
    );
    const operation = operations.find(({ operationId }) => operationId === params.operationId);
    if (!operation || operation.type !== params.operationType)
      throw new Error("DELIVERY_PROVIDER_OPERATION_NOT_FOUND");
      ...
      await this.complete(
        {
          ...
          request: {} as Delivery.DeliveryProviderShipmentRequest,
          duplicate: false,
        },
        params.result,
        context.correlationId ?? params.providerEventId,
        params.providerEventId,
      );
    } else if (
      operation.state === "FAILED" &&
      operation.type === "CREATE" &&
      !operation.failure?.acceptedByProvider &&
      shipment.lastFulfillmentState === "SHIPMENT_CREATED"
    ) {
  ...
  async reportProviderEvent(
    ...
    const account = await this.deps.repository.providerAccounts.getByInstallation(
      context.storeId,
    ...
    if (
      ...
      account.appCode !== context.appCode
    )
      throw new Error("DELIVERY_PROVIDER_CONTEXT_MISMATCH");
    const capabilities = account.capabilityStates.shipmentProvider?.capabilities;
    if (!capabilities) throw new Error("DELIVERY_SHIPMENT_PROVIDER_CAPABILITIES_MISSING");
    if (event.type === "SHIPMENT_STATUS_CHANGED" && !capabilities.supportsTracking)
      throw new Error("DELIVERY_PROVIDER_TRACKING_UNSUPPORTED");
    if (event.type === "SHIPMENT_LABEL_AVAILABLE" && !capabilities.supportsLabels)
      throw new Error("DELIVERY_PROVIDER_LABELS_UNSUPPORTED");
    const shipment = await this.deps.repository.shipments.findByProviderReference(
      context.storeId,
      account.providerAccountId,
      event.providerShipmentReference,
    );
    ...
    this.assertProviderContext(shipment, operation.route, context);
    const idempotency = idem(
      `delivery.shipment.provider-event:${account.providerAccountId}`,
      params.providerEventId,
      params,
    );
    ...
    if (event.type === "SHIPMENT_STATUS_CHANGED") {
      ...
      const parcel = shipment.parcels.find(
        ({ providerParcelReference }) => providerParcelReference === event.providerParcelReference,
      );
    ...
    const transition =
      event.type === "SHIPMENT_STATUS_CHANGED"
        ? this.deps.transitions.evaluate({
            current: shipment,
            observedState: event.shipmentState,
            occurredAt: params.occurredAt,
            providerEventId: params.providerEventId,
            providerShipmentSequence: params.providerShipmentSequence,
          })
    ...
    if (transition.status === "IGNORE_STALE") {
      ...
      return {
        ...
        shipmentRevision: current.revision,
      };
    }
    ...
    const next = {
      ...
      parcels:
        event.type === "SHIPMENT_LABEL_AVAILABLE"
          ? normalized.parcels.map((parcel) =>
              parcel.providerParcelReference === event.providerParcelReference
                ? {
                    ...parcel,
                    state:
                      shipment.parcels.find(
                        (prior) => prior.providerParcelReference === parcel.providerParcelReference,
                      )?.state ?? parcel.state,
                  }
                : parcel,
            )
      ...
      lastProviderShipmentSequence:
        params.providerShipmentSequence ?? shipment.lastProviderShipmentSequence,
      revision: shipment.revision + 1,
      updatedAt: params.occurredAt,
    };
    const events = await this.observationEvents(
      shipment,
      next,
      operation,
      operation.route,
    ...
    const result = await this.deps.repository.shipments.commit({
      ...
      providerInbox: {
        ...
        providerEventId: params.providerEventId,
        providerShipmentSequence: params.providerShipmentSequence,
        eventHash: idempotency.requestHash,
        occurredAt: params.occurredAt,
      },
      domainEvents: events,
    });
  ...
  private async prepareExisting(
    type: "CANCEL" | "RECONCILE",
    params: Delivery.CancelDeliveryShipmentParams | Delivery.ReconcileDeliveryShipmentParams,
    now: string,
  ): Promise<PreparedShipmentOperation> {
    const scope = `delivery.shipment.${type.toLowerCase()}:${params.shipmentId}`;
    const idempotency = idem(scope, params.idempotencyKey, params);
    const prior = await this.deps.repository.shipments.findOperationByIdempotency(
      params.storeId,
      scope,
      params.idempotencyKey,
    );
    const shipment = await this.requiredShipment(params.storeId, params.shipmentId);
    if (prior) {
      if (prior.idempotency.requestHash !== idempotency.requestHash)
        throw new Error("DELIVERY_IDEMPOTENCY_CONFLICT");
      return {
        shipment,
        operation: prior,
        route: prior.route,
        request: existingRequest(shipment, prior, params, now),
        duplicate: true,
      };
    }
    ...
    if (
      type === "CANCEL" &&
      !(
        DeliveryShipmentTransitions[shipment.state] as readonly Delivery.DeliveryShipmentState[]
      ).includes("CANCELLING")
    )
      throw new Error("DELIVERY_SHIPMENT_CANCELLATION_INVALID");
    if (type === "RECONCILE" && (shipment.state === "CANCELLED" || shipment.state === "RETURNED"))
      throw new Error("DELIVERY_SHIPMENT_RECONCILIATION_INVALID");
    const account =
      type === "CANCEL"
    ...
    const providerOperation =
      type === "CANCEL"
        ...
        : account.supportedOperations.includes("reconcileShipment")
          ? ("reconcileShipment" as const)
          : ("getShipment" as const);
    ...
    const operationType: Delivery.DeliveryShipmentOperationType =
      type === "CANCEL" ? "CANCEL" : providerOperation === "getShipment" ? "GET" : "RECONCILE";
    const operation = operationSnapshot(
      await this.deps.repository.generateUuidV7(),
      shipment.shipmentId,
      operationType,
    ...
    const next = {
      ...shipment,
      state: type === "CANCEL" ? ("CANCELLING" as const) : shipment.state,
      revision: shipment.revision + 1,
      updatedAt: now,
    };
    const result = await this.deps.repository.shipments.commit(
    ...
    return {
      ...
      operation,
      route,
      request: existingRequest(next, operation, params, now),
      duplicate: false,
    };
  }
  ...
  private async requiredReconciliationAccount(storeId: string, providerAccountId: string) {
    ...
    if (
      !account ||
      capability?.status !== "ACTIVE" ||
      !account.supportedOperations.some(
        (operation) => operation === "reconcileShipment" || operation === "getShipment",
      )
    ) {
  ...
  private async requiredRoute(
    account: Delivery.DeliveryProviderAccountSnapshot,
    operation: Delivery.DeliveryShipmentProviderOperation,
  ) {
    const route = await this.deps.apps.resolveRoute({
    ...
    if (!route || route.appCode !== account.appCode || route.appVersion !== account.appVersion)
      throw new Error("DELIVERY_SHIPMENT_PROVIDER_ROUTE_UNAVAILABLE");
  ...
  private async ensureFulfillmentReserved(
    shipment: Delivery.DeliveryShipmentSnapshot,
    operation: Delivery.DeliveryShipmentOperationSnapshot,
    route: PreparedShipmentOperation["route"],
    params: Delivery.CreateDeliveryShipmentParams,
    now: string,
  ): Promise<Delivery.DeliveryShipmentSnapshot> {
    if (shipment.revision > 1) return shipment;
    const reserved = await this.deps.fulfillment.applyShipmentUpdate({
    ...
    const next = {
      ...
      lastFulfillmentState: "SHIPMENT_CREATED" as const,
      revision: 2,
      updatedAt: now,
    };
    const reserveIdempotency = idem(
      `delivery.shipment.reserve:${shipment.shipmentId}`,
      `${params.idempotencyKey}:reserve`,
      { fulfillmentOrderRevision: reserved.fulfillmentOrderRevision },
    );
    const committed = await this.deps.repository.shipments.commit(
      mutation(
    ...
    if (!["APPLIED", "DUPLICATE"].includes(committed.status))
      throw new Error("DELIVERY_SHIPMENT_RESERVATION_PERSIST_FAILED");
    return committed.status === "DUPLICATE"
      ? this.requiredShipment(shipment.storeId, shipment.shipmentId)
      : next;
  }

  private assertProviderContext(
    shipment: Delivery.DeliveryShipmentSnapshot,
    route: Delivery.DeliveryProviderRouteSnapshot,
    context: DeliveryProviderCompletionContext,
  ) {
    if (
      shipment.organizationId !== context.organizationId ||
      shipment.storeId !== context.storeId ||
      route.installationId !== context.installationId ||
      route.appCode !== context.appCode ||
      route.appVersion !== context.appVersion
    ) {
      throw new Error("DELIVERY_PROVIDER_CONTEXT_MISMATCH");
    }
  }

  private async failPrepared(
    shipment: Delivery.DeliveryShipmentSnapshot,
    operation: Delivery.DeliveryShipmentOperationSnapshot,
    route: PreparedShipmentOperation["route"],
    correlationId: string,
    at: string,
    providerFailure: Delivery.DeliveryProviderFailure,
    providerShipmentReference: string | null = null,
  ) {
    const current = await this.requiredShipment(shipment.storeId, shipment.shipmentId);
    const state =
      operation.type === "GET" || operation.type === "RECONCILE"
        ? current.state
        : ("FAILED" as const);
    const next = {
      ...current,
      state,
      providerShipmentReference: providerShipmentReference ?? current.providerShipmentReference,
    ...
    const failedOperation = {
      ...
      providerShipmentReference: providerShipmentReference ?? operation.providerShipmentReference,
      failure: providerFailure,
      revision: operation.revision + 1,
      completedAt: at,
    };
    const eventId = await this.deps.repository.generateUuidV7();
    const payload: DeliveryEvents.OperationFailed = {
      ...eventBase(eventId, operation.operationId, correlationId, next, failedOperation, route, at),
      shipmentState: next.state,
      failure: providerFailure,
    };
    const domainEvents: import("../../contracts/ports.js").DeliveryDomainEvent[] = [
      { type: "delivery.shipment.operation_failed", payload },
    ];
    if (current.state !== next.state) {
      const stateEventId = await this.deps.repository.generateUuidV7();
      domainEvents.unshift({
    ...
    const completion = idem(
      `delivery.shipment.fail:${operation.operationId}`,
      `${operation.idempotency.key}:failed`,
      providerFailure,
    );
    const result = await this.deps.repository.shipments.commit(
      mutation(
    ...
    if (!["APPLIED", "DUPLICATE"].includes(result.status))
      throw new Error(`DELIVERY_FAILURE_${result.status}`);
  }

  private async releaseFulfillment(
    shipment: Delivery.DeliveryShipmentSnapshot,
    occurredAt: string,
  ) {
    const current = await this.requiredShipment(shipment.storeId, shipment.shipmentId);
    ...
    const released = await this.deps.fulfillment.applyShipmentUpdate({
      storeId: current.storeId,
      update: {
    ...
    const next = {
      ...
      lastFulfillmentState: "CANCELLED" as const,
      revision: current.revision + 1,
      updatedAt: occurredAt,
    };
    const sync = idem(
      `delivery.shipment.fulfillment-release:${current.shipmentId}`,
      `${current.revision}:${released.fulfillmentOrderRevision}`,
      released,
    );
    const committed = await this.deps.repository.shipments.commit(
      mutation(
    ...
    if (!["APPLIED", "DUPLICATE"].includes(committed.status))
      throw new Error(`DELIVERY_FULFILLMENT_RELEASE_${committed.status}`);
  }
  ...
  private async propagateState(shipment: Delivery.DeliveryShipmentSnapshot, occurredAt: string) {
    ...
    const result = await this.deps.fulfillment.applyShipmentUpdate({
      storeId: shipment.storeId,
      update: {
        fulfillmentOrderId: shipment.fulfillmentOrderId,
        shipmentId: shipment.shipmentId,
    ...
    if (
      ...
      const next = {
        ...shipment,
        fulfillmentOrderRevision: result.fulfillmentOrderRevision,
        lastFulfillmentState: state,
        revision: shipment.revision + 1,
        updatedAt: occurredAt,
      };
      const syncIdempotency = idem(
        `delivery.shipment.fulfillment-sync:${shipment.shipmentId}`,
        `${shipment.revision}:${result.fulfillmentOrderRevision}`,
        result,
      );
      const committed = await this.deps.repository.shipments.commit(
        mutation(
      ...
      if (!["APPLIED", "DUPLICATE"].includes(committed.status))
        throw new Error(`DELIVERY_FULFILLMENT_SYNC_${committed.status}`);
    }
  }

  private async observationEvents(
    previous: Delivery.DeliveryShipmentSnapshot,
    next: Delivery.DeliveryShipmentSnapshot,
    operation: Delivery.DeliveryShipmentOperationSnapshot,
    route: PreparedShipmentOperation["route"],
    correlationId: string,
    tracking: readonly Delivery.DeliveryTrackingEventSnapshot[],
    at: string,
  ) {
    ...
    if (previous.state !== next.state) {
      ...
      events.push({
        type: "delivery.shipment.state_changed",
        payload: {
          ...eventBase(id, operation.operationId, correlationId, next, operation, route, at),
    ...
    const priorLabels = new Set(
      previous.parcels.flatMap(({ labels }) => labels.map(({ mediaId }) => mediaId)),
    );
      ...
      for (const label of parcel.labels) {
        ...
        events.push({
          ...
          payload: {
            ...
            providerParcelReference: parcel.providerParcelReference,
            label,
          },
        });
      }
...
function operationSnapshot(
  operationId: string,
  shipmentId: string,
  type: Delivery.DeliveryShipmentOperationType,
  state: Delivery.DeliveryShipmentOperationState,
  idempotency: Delivery.DeliveryIdempotencySnapshot,
  route: PreparedShipmentOperation["route"],
  configurationRevision: string,
  now: string,
): Delivery.DeliveryShipmentOperationSnapshot {
  return {
    operationId,
    shipmentId,
    type,
...
function createRequest(
  ...
  return {
    ...
    operation: "CREATE",
    orderReference: shipment.orderId,
    fulfillmentOrderReference: shipment.fulfillmentOrderId,
    shipmentConfigurationRevision: operation.configurationRevision,
    shipmentPlanHash: shipment.shipmentPlanHash,
    deliveryMethodCommitmentId: method.commitmentId,
    ratedFactsHash: shipment.ratedFactsHash,
    deliveryMethodCode: method.code,
    selectedRate:
      method.source === "MANUAL"
        ? { source: "MANUAL", serviceCode: method.serviceCode }
        : {
    ...
    customerInputHash: method.customerInputHash,
  };
}
...
function existingRequest(
  ...
  | Delivery.DeliveryProviderGetShipmentRequest
  ...
  if (operation.type === "CANCEL")
    return { ...base, operation: "CANCEL", reason: "reason" in params ? params.reason : null };
...
function mutation(
  ...
  idempotency: Delivery.DeliveryIdempotencySnapshot,
  domainEvents: DeliveryAtomicMutationRecord["domainEvents"],
): DeliveryAtomicMutationRecord {
  return {
    mutationId,
    cause,
...
function failure(
  category: Delivery.DeliveryProviderFailureCategory,
  code: string,
  message: string,
  retryable: boolean,
): Delivery.DeliveryProviderFailure {
...
function fulfillmentLines(
  ...
  for (const item of shipment.packages.flatMap(({ items }) => items))
    quantities.set(item.lineId, (quantities.get(item.lineId) ?? 0) + item.quantity);
  return [...quantities].map(([fulfillmentOrderLineItemId, quantity]) => ({
...
function fulfillmentState(
  state: Delivery.DeliveryShipmentState,
): Delivery.DeliveryFulfillmentShipmentUpdate["state"] | null {
  if (
    state === "IN_TRANSIT" ||
    state === "DELIVERED" ||
...
function eventBase(
  ...
  return {
    ...
    storeId: shipment.storeId,
    checkoutId: shipment.checkoutId,
...
function isProviderState(
  state: Delivery.DeliveryShipmentState,
): state is Delivery.DeliveryProviderObservedShipmentState {
  return [
    "PENDING",
    "ACCEPTED",
    "IN_TRANSIT",
