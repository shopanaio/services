...
export class DeliveryProviderAccountService {
  ...
  async configure(params: Delivery.ConfigureDeliveryProviderAccountParams): Promise<Delivery.ConfigureDeliveryProviderAccountResult> {
    ...
    for (const capability of params.enabledCapabilities) {
      ...
      if (result.status === "INVALID") throw new Error(`Delivery provider configuration is invalid: ${result.failure?.code ?? "UNKNOWN"}`);
    ...
    const account: Delivery.DeliveryProviderAccountSnapshot = { providerAccountId: id, organizationId: params.organizationId, storeId: params.storeId, installationId: params.installationId,
      appCode: first.route.appCode, appVersion: first.route.appVersion, providerCode: first.result.providerCode, displayName: first.result.displayName, mode: params.mode, revision: 1,
      supportedCountryCodes: [...new Set(results.flatMap(({ result }) => result.supportedCountryCodes))].sort(),
      supportedCurrencyCodes: [...new Set(results.flatMap(({ result }) => result.supportedCurrencyCodes))].sort(),
      supportedOperations: [...new Set(results.flatMap(({ result }) => result.supportedOperations))].sort(),
      capabilityStates: { carrierService: carrier ? state(carrier) : null, shipmentProvider: shipment ? state(shipment) : null }, createdAt: now, updatedAt: now };
    const saved = await this.repository.providerAccounts.save(account, null); if (saved.status !== "SAVED") throw new Error("Delivery provider account persistence conflicted");
    return { providerAccountId: id, workflowId: `delivery-provider-account:${id}:1`, duplicate: false };
  }

  async setStatus(params: Delivery.SetDeliveryProviderCapabilityStatusParams): Promise<Delivery.SetDeliveryProviderCapabilityStatusResult> {
    const current = await this.repository.providerAccounts.getById(params.storeId, params.providerAccountId); if (!current) throw new Error("Delivery provider account not found");
    if (current.revision !== params.expectedAccountRevision) throw new Error("Delivery provider account revision conflict");
    const key = params.capability === "delivery.carrier-service" ? "carrierService" : "shipmentProvider";
    const capability = current.capabilityStates[key]; if (!capability || !["READY", "ACTIVE", "INACTIVE", "DEGRADED"].includes(capability.status)) throw new Error("Delivery provider capability cannot transition to the requested status");
    const account = { ...current, revision: current.revision + 1, updatedAt: new Date().toISOString(), capabilityStates: { ...current.capabilityStates, [key]: { ...capability, status: params.status, statusReason: null } } } as Delivery.DeliveryProviderAccountSnapshot;
    const saved = await this.repository.providerAccounts.save(account, current.revision); if (saved.status !== "SAVED") throw new Error("Delivery provider account revision conflict");
    return { account: saved.account };
  }
}

function state<T extends Delivery.DeliveryProviderCapability>(result: Delivery.DeliveryProviderConfigurationValidationResult<T>): Delivery.DeliveryProviderCapabilityState<any> {
  return result.status === "READY" ? { status: "READY", capabilities: result.capabilities, configurationRevision: result.configurationRevision, statusReason: null }
    : { status: "DEGRADED", capabilities: result.capabilities, configurationRevision: result.configurationRevision, statusReason: { code: result.failure!.code, message: result.failure!.message } };
}
