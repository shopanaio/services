...
describe("DeliveryCheckoutService", () => {
  it("returns canonical empty delivery for a digital-only checkout", async () => {
    ...
    const service = new DeliveryCheckoutService({
      facts: { resolve: unexpected },
      profiles: { listActiveForStore: unexpected } as never,
      ...
      rateCache: {} as never,
      handleSecret: "test-secret-test-secret-test-secret",
    });
    const now = Date.now();
    const result = await service.calculateOptions({
      context: {
      ...
      preliminary: {
        ...
        discountEvaluationRevision: "discounts",
        transformedLines: [],
        ...
        merchandiseRevision: "merchandise",
        availabilityRevision: "availability",
        appliedDiscounts: [],
        discountCodeResolutions: [],
        usageRequirements: [],
        preliminaryTotals: {
          merchandiseSubtotal: { amountMinor: "0", currencyCode: "USD" },
          merchandiseDiscountTotal: { amountMinor: "0", currencyCode: "USD" },
          merchandiseTotal: { amountMinor: "0", currencyCode: "USD" },
        },
      },
      destinations: [],
      selections: [],
