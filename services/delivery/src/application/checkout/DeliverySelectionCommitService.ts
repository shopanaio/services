...
export class DeliverySelectionCommitService {
  ...
  async commit(
  ...
  ): Promise<Delivery.CommitCheckoutDeliverySelectionsResult> {
    ...
    for (const selection of params.selections) {
      ...
      if (resolved.deliveryRevision !== params.deliveryRevision)
        throw new Error("DELIVERY_REVISION_MISMATCH");

      let customerInput = selection.customerInput;
      let shipmentProvider: { providerAccountId: string; configurationRevision: string } | null =
        null;
      if (resolved.binding.source === "CARRIER_SERVICE") {
        const account = await this.dependencies.providerAccounts.getById(
          params.storeId,
          resolved.binding.carrierServiceAccountId,
        );
        if (!account || account.organizationId !== params.organizationId)
          throw new Error("DELIVERY_PROVIDER_ACCOUNT_NOT_FOUND");
        const carrier = account.capabilityStates.carrierService;
        if (
          carrier?.status !== "ACTIVE" ||
          carrier.configurationRevision !== resolved.binding.carrierServiceConfigurationRevision
        ) {
        ...
        if (
          ...
          account.supportedOperations.includes("createShipment")
        ) {
          shipmentProvider = {
            providerAccountId: account.providerAccountId,
            configurationRevision: shipment.configurationRevision,
          };
        }
        if (resolved.binding.customerInputContract !== null) {
          ...
          if (
            !route ||
            route.operation !== "resolveCustomerInput" ||
            route.appCode !== account.appCode
          )
            throw new Error("DELIVERY_CUSTOMER_INPUT_ROUTE_UNAVAILABLE");
  ...
  ): Promise<Delivery.ReleaseCheckoutDeliverySelectionsResult> {
