...
@Injectable()
export class DeliveryShipmentOutboxWorkflow extends BrokerWorkflows<
  DeliveryShipmentOutboxWorkflowInput,
  void
> {
  constructor(
  ...
  @WorkflowStep() private load(storeId: string, shipmentId: string) {
    return this.repository.shipments.listPendingOutbox(storeId, shipmentId);
  }
  @WorkflowStep() private mark(id: string, emittedAt: string) {
    return this.repository.shipments.markOutboxEmitted(id, emittedAt);
  }
}
