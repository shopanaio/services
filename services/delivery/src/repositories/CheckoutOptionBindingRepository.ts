...
export class CheckoutOptionBindingRepository
  extends BaseRepository
  implements DeliveryOptionBindingsPort
{
  async stageCheckoutSnapshot(input: {
    ...
    if (input.targetCheckoutVersion !== input.basedOnCheckoutVersion + 1) {
      return {
        status: "STALE_CHECKOUT_VERSION" as const,
        currentCheckoutVersion: input.basedOnCheckoutVersion,
      };
    }
    return this.connection.transaction(async (tx) => {
      ...
      for (const candidate of input.options) {
        ...
        await tx
          ...
          .onConflictDoUpdate({
            target: [
              checkoutOptionBindings.storeId,
              checkoutOptionBindings.checkoutId,
              checkoutOptionBindings.targetCheckoutVersion,
              checkoutOptionBindings.groupId,
              checkoutOptionBindings.optionHandle,
            ],
  ...
  async resolve(input: {
    ...
    const [row] = await this.connection
      .select({
        snapshot: checkoutOptionBindings.snapshot,
        option: checkoutOptionBindings.option,
        deliveryRevision: checkoutOptionBindings.deliveryRevision,
        expiresAt: checkoutOptionBindings.expiresAt,
      })
      .from(checkoutOptionBindings)
      .where(
        and(
          eq(checkoutOptionBindings.storeId, input.storeId),
          eq(checkoutOptionBindings.checkoutId, input.checkoutId),
          eq(checkoutOptionBindings.targetCheckoutVersion, input.checkoutVersion),
          eq(checkoutOptionBindings.groupId, input.groupId),
  ...
  async commitSelection(input: {
    ...
    shipmentProvider: { providerAccountId: string; configurationRevision: string } | null;
    effectiveAt: string;
    idempotencyKey: string;
  }) {
    return this.txManager.run(async () => {
      await this.connection.execute(
        sql`SELECT pg_advisory_xact_lock(hashtextextended(${`${input.storeId}:${input.checkoutId}:${input.checkoutVersion}:${input.groupId}`}, 0))`,
      );
      const resolved = await this.resolve(input);
      if (resolved.status !== "FOUND")
        return {
          status: resolved.status,
          code: `DELIVERY_BINDING_${resolved.status}`,
          message: "Delivery option binding is unavailable.",
        } as const;
      if (resolved.deliveryRevision !== input.deliveryRevision) {
        return {
          status: "DELIVERY_REVISION_MISMATCH" as const,
          code: "DELIVERY_REVISION_MISMATCH",
          message: "Delivery option binding revision is stale.",
        };
      }
      if (resolved.binding.customerInputContract === null && input.customerInput !== null) {
        return {
          status: "CUSTOMER_INPUT_INVALID" as const,
          code: "DELIVERY_CUSTOMER_INPUT_UNEXPECTED",
          message: "The selected delivery option does not accept customer input.",
        };
      }
      if (resolved.option.phoneRequired && !input.recipient.phone?.trim()) {
        return {
          ...
          message: "The selected delivery option requires a recipient phone number.",
        };
      }
      const requestHash = revision("dcommit_request_v1", {
        ...
        shipmentProvider: input.shipmentProvider,
      });
      const [existing] = await this.connection
        .select()
        .from(checkoutSelectionCommitments)
        .where(
          and(
            ...
            eq(checkoutSelectionCommitments.groupId, input.groupId),
          ),
        )
        .limit(1)
        .for("update");
      if (existing) {
        if (existing.requestHash !== requestHash)
          return {
            ...
            code: "DELIVERY_COMMITMENT_CONFLICT",
            message: "The delivery selection was already committed with different input.",
          };
      ...
      if (resolved.option.deliveryMethodType === "NONE") {
        return {
          status: "CUSTOMER_INPUT_INVALID" as const,
          code: "DELIVERY_METHOD_NOT_SHIPPABLE",
          message: "A non-delivery option cannot be committed for fulfillment.",
        };
      }
      const commitmentId = await this.generateUuidV7();
      const customerInputHash =
        input.customerInput === null ? null : revision("dcustomer_v1", input.customerInput);
      const common = {
        commitmentId,
        ...
        presentedName: resolved.option.title,
        methodType: resolved.option.deliveryMethodType,
        cost: resolved.option.cost,
        estimatedMinDeliveryAt: resolved.option.estimatedMinDeliveryAt,
        estimatedMaxDeliveryAt: resolved.option.estimatedMaxDeliveryAt,
        ratedFactsHash: resolved.binding.ratedFactsHash,
        originalOptionHandle: input.optionHandle,
        customerInput: input.customerInput,
        customerInputHash,
        additionalInformation: resolved.option.publicData,
      } as const;
      const deliveryMethod: Delivery.DeliveryCommittedMethodSnapshot =
        resolved.binding.source === "MANUAL"
          ? {
              ...common,
              source: "MANUAL",
              serviceCode: resolved.option.code,
          ...
          : {
              ...
              carrierServiceAccountId: resolved.binding.carrierServiceAccountId,
              carrierCode: resolved.binding.carrierCode,
              carrierServiceConfigurationRevision:
                resolved.binding.carrierServiceConfigurationRevision,
              quoteRevision: resolved.binding.quoteRevision,
            };
      const commitment: Delivery.DeliveryCommittedGroupSnapshot = {
        groupId: input.groupId,
        lineIds: resolved.binding.fulfillment.lineIds,
        deliveryMethod,
        shipmentProvider: input.shipmentProvider,
        origin: resolved.binding.fulfillment.origin,
        destination: resolved.binding.fulfillment.destination,
        sender: resolved.binding.fulfillment.sender,
        recipient: input.recipient,
        packages: resolved.binding.fulfillment.packages,
      };
  ...
  async releaseCommitments(input: {
    ...
    const rows = await this.connection
      .update(checkoutSelectionCommitments)
  ...
  async deleteExpired(now: string): Promise<number> {
    const rows = await this.connection
      .delete(checkoutOptionBindings)
      .where(lt(checkoutOptionBindings.expiresAt, now))
      .returning({ id: checkoutOptionBindings.id });
    return rows.length;
  }
}
