...
export const providerAccounts = deliverySchema.table(
  "provider_accounts",
  {
    ...
    supportedOperations: jsonb("supported_operations")
      .$type<readonly Delivery.DeliveryProviderOperation[]>()
      .notNull(),
...
export const profiles = deliverySchema.table(
  "profiles",
  {
    id: uuid("id").primaryKey(),
    storeId: uuid("store_id").notNull(),
...
export const profileSets = deliverySchema.table(
  "profile_sets",
  {
    id: uuid("id").primaryKey(),
    storeId: uuid("store_id").notNull(),
...
export const profileAssignmentMemberships = deliverySchema.table(
  "profile_assignment_memberships",
  {
    id: uuid("id").primaryKey(),
    storeId: uuid("store_id").notNull(),
  ...
  (table) => [
    unique("profile_assignment_membership_key").on(
      table.assignmentSetId,
      table.membershipType,
      table.resourceId,
    ),
    index("profile_assignment_lookup_idx").on(
      table.storeId,
      table.membershipType,
      table.resourceId,
    ),
