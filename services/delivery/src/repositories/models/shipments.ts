...
export const shipments = deliverySchema.table("shipments", {
  ...
  unique("shipments_provider_reference_key").on(table.storeId, table.providerAccountId, table.providerShipmentReference),
]);

export const shipmentOperations = deliverySchema.table("shipment_operations", {
  id: uuid("id").primaryKey(),
  storeId: uuid("store_id").notNull(),
  ...
  unique("shipment_operations_idempotency_key").on(table.storeId, table.idempotencyScope, table.idempotencyKey),
  index("shipment_operations_shipment_idx").on(table.storeId, table.shipmentId, table.createdAt),
]);

export const shipmentTrackingEvents = deliverySchema.table("shipment_tracking_events", {
  id: uuid("id").primaryKey(),
  storeId: uuid("store_id").notNull(),
  shipmentId: uuid("shipment_id").notNull(),
  ...
  unique("shipment_tracking_events_provider_key").on(table.storeId, table.shipmentId, table.providerEventId),
  index("shipment_tracking_events_timeline_idx").on(table.storeId, table.shipmentId, table.occurredAt),
]);

export const providerInbox = deliverySchema.table("provider_inbox", {
  id: uuid("id").primaryKey(),
  storeId: uuid("store_id").notNull(),
  providerAccountId: uuid("provider_account_id").notNull(),
  providerEventId: text("provider_event_id").notNull(),
  ...
  occurredAt: timestamp("occurred_at", { withTimezone: true, mode: "string" }).notNull(),
  createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
}, (table) => [unique("provider_inbox_event_key").on(table.storeId, table.providerAccountId, table.providerEventId)]);

export const deliveryMutations = deliverySchema.table("shipment_mutations", {
  id: uuid("id").primaryKey(),
  storeId: uuid("store_id").notNull(),
  shipmentId: uuid("shipment_id").notNull(),
  idempotencyScope: text("idempotency_scope").notNull(),
  ...
  createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
}, (table) => [unique("shipment_mutations_idempotency_key").on(table.storeId, table.idempotencyScope, table.idempotencyKey)]);

export const deliveryOutbox = deliverySchema.table("shipment_outbox", {
  id: uuid("id").primaryKey(),
  storeId: uuid("store_id").notNull(),
  shipmentId: uuid("shipment_id").notNull(),
  ...
  index("shipment_outbox_pending_idx").on(table.emittedAt, table.createdAt),
]);
