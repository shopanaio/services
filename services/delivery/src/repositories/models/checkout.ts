...
export const checkoutOptionBindings = deliverySchema.table(
  "checkout_option_bindings",
  {
  ...
  (table) => [
    unique("checkout_option_bindings_identity_key").on(
      table.storeId,
      table.checkoutId,
    ...
    index("checkout_option_bindings_lookup_idx").on(
      table.storeId,
      table.checkoutId,
      table.targetCheckoutVersion,
      table.groupId,
...
export const checkoutSelectionCommitments = deliverySchema.table(
  "checkout_selection_commitments",
  {
    id: uuid("id").primaryKey(),
  ...
  (table) => [
    unique("checkout_selection_commitments_group_key").on(
      table.storeId,
      table.checkoutId,
      table.checkoutVersion,
      table.groupId,
    ),
    unique("checkout_selection_commitments_idempotency_key").on(
      table.storeId,
      table.idempotencyKey,
      table.groupId,
    ),
    index("checkout_selection_commitments_checkout_idx").on(
      table.storeId,
      table.checkoutId,
      table.checkoutVersion,
    ),
...
export const providerRateCache = deliverySchema.table(
  "provider_rate_cache",
  {
    id: uuid("id").primaryKey(),
    storeId: uuid("store_id").notNull(),
