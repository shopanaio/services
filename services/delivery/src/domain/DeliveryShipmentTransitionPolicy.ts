...
export class DeliveryShipmentTransitionPolicy implements DeliveryShipmentTransitionPolicyPort {
  evaluate(
    input: Parameters<DeliveryShipmentTransitionPolicyPort["evaluate"]>[0],
  ): ReturnType<DeliveryShipmentTransitionPolicyPort["evaluate"]> {
    const last = input.current.lastProviderShipmentSequence;
    if (
      last !== null &&
      input.providerShipmentSequence !== null &&
      BigInt(input.providerShipmentSequence) <= BigInt(last)
    ) {
      return { status: "IGNORE_STALE", currentState: input.current.state };
    }
    if (
      input.providerShipmentSequence === null &&
      input.current.lastTrackingEvent &&
      Date.parse(input.occurredAt) <= Date.parse(input.current.lastTrackingEvent.occurredAt)
    ) {
      return { status: "IGNORE_STALE", currentState: input.current.state };
    }
    ...
    if (!isReachable(input.current.state, input.observedState)) {
      return {
        ...
        code: "DELIVERY_SHIPMENT_TRANSITION_INVALID",
      };
    }
...
function isReachable(
  from: Delivery.DeliveryShipmentState,
  target: Delivery.DeliveryShipmentState,
): boolean {
