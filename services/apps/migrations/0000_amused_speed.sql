...
CREATE TABLE "platform"."provider_configs" (
	...
	CONSTRAINT "provider_configs_store_id_provider_key" UNIQUE("store_id","provider")
);
--> statement-breakpoint
...
CREATE TABLE "platform"."slots" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "slots_store_id_domain_provider_key" UNIQUE("store_id","domain","provider")
);
--> statement-breakpoint
...
CREATE INDEX "idx_provider_configs_store" ON "platform"."provider_configs" USING btree ("store_id");--> statement-breakpoint
CREATE INDEX "idx_slot_assignments_resolve" ON "platform"."slot_assignments" USING btree ("store_id","aggregate","aggregate_id","domain","status","precedence");--> statement-breakpoint
CREATE INDEX "idx_slot_assignments_slot" ON "platform"."slot_assignments" USING btree ("slot_id");--> statement-breakpoint
CREATE INDEX "idx_slots_store_domain" ON "platform"."slots" USING btree ("store_id","domain");--> statement-breakpoint
