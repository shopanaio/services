...
import { appInstallationManifestSnapshots, appInstallations } from "../models/index.js";

export const appManifestSnapshotRelayQuery = createRelayQuery(
  createQuery(appInstallationManifestSnapshots).include(["id"]).maxLimit(100).defaultLimit(20),
  { name: "appManifestSnapshot", tieBreaker: "id" },
);

export type AppManifestSnapshotRelayInput = InferRelayInput<typeof appManifestSnapshotRelayQuery>;
...
export class AppManifestSnapshotRepository extends BaseRepository {
  ...
  async save(installationId: string, snapshot: AppManifestSnapshot): Promise<void> {
  ...
  async find(
    ...
    const rows = await this.connection
      ...
      .where(
        and(
          eq(appInstallationManifestSnapshots.installationId, installationId),
          eq(appInstallationManifestSnapshots.version, version),
          eq(appInstallationManifestSnapshots.manifestHash, manifestHash),
        ),
      )
      .limit(1);
  ...
  async findByIdForStore(id: string): Promise<AppManifestSnapshotRecord | null> {
    const rows = await this.connection
      .select(getTableColumns(appInstallationManifestSnapshots))
      .from(appInstallationManifestSnapshots)
      .innerJoin(
        appInstallations,
        eq(appInstallations.id, appInstallationManifestSnapshots.installationId),
      )
      .where(
        and(
  ...
  async getByIdsForStore(ids: readonly string[]): Promise<AppManifestSnapshotRecord[]> {
    if (ids.length === 0) {
      return [];
    }
    const rows = await this.connection
      ...
      .where(
        and(
          eq(appInstallations.storeId, this.storeId),
          inArray(appInstallationManifestSnapshots.id, [...new Set(ids)]),
        ),
      );
    return rows.map(mapSnapshot);
  }

  async findLatest(installationId: string): Promise<AppManifestSnapshotRecord | null> {
    const rows = await this.connection
      .select()
      .from(appInstallationManifestSnapshots)
      .where(eq(appInstallationManifestSnapshots.installationId, installationId))
      .orderBy(desc(appInstallationManifestSnapshots.createdAt))
      .limit(1);
    return rows[0] ? mapSnapshot(rows[0]) : null;
  }

  async listByInstallation(installationId: string): Promise<AppManifestSnapshotRecord[]> {
    const rows = await this.connection
      .select()
      .from(appInstallationManifestSnapshots)
      .where(eq(appInstallationManifestSnapshots.installationId, installationId))
  ...
  async getConnection(
    ...
    const [result, totalCount] = await Promise.all([
      appManifestSnapshotRelayQuery.execute(this.connection, relayInput),
