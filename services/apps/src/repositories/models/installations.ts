...
export const appInstallationStatus = appsSchema.enum("app_installation_status", [
  "PENDING_CONSENT",
  "INSTALLING",
  "ACTIVE",
  "INSTALL_FAILED",
...
export const appInstallationHealthStatus = appsSchema.enum("app_installation_health_status", [
  "UNKNOWN",
  "HEALTHY",
  "DEGRADED",
  "UNHEALTHY",
...
export const appLifecycleOperationType = appsSchema.enum("app_lifecycle_operation_type", [
  "INSTALL",
  "UPDATE",
  "SUSPEND",
  "RESUME",
...
export const appLifecycleOperationStatus = appsSchema.enum("app_lifecycle_operation_status", [
  "PENDING",
  "RUNNING",
  "SUCCEEDED",
  "FAILED",
...
export const appInstallations = appsSchema.table(
  "app_installations",
  {
    ...
    status: appInstallationStatus("status").notNull().default("PENDING_CONSENT"),
  ...
  (table) => [
    ...
    index("app_installations_store_status_idx").on(table.storeId, table.status),
...
export const appInstallationManifestSnapshots = appsSchema.table(
  ...
  (table) => [
    ...
    index("app_installation_manifest_installation_idx").on(table.installationId),
  ],
);
...
export const appInstallationScopes = appsSchema.table(
  ...
  (table) => [
    unique("app_installation_scope_key").on(table.installationId, table.scope),
    index("app_installation_scopes_installation_idx").on(table.installationId),
  ],
);
...
export const appInstallationSecrets = appsSchema.table(
  ...
  (table) => [
    unique("app_installation_secret_key").on(table.installationId, table.name),
    index("app_installation_secrets_installation_idx").on(table.installationId),
  ],
);
...
export const appLifecycleOperations = appsSchema.table(
  "app_lifecycle_operations",
  {
    ...
    status: appLifecycleOperationStatus("status").notNull().default("PENDING"),
    targetVersion: varchar("target_version", { length: 64 }).notNull(),
    previousInstallationStatus: appInstallationStatus("previous_installation_status"),
  ...
  (table) => [
    ...
    index("app_lifecycle_operations_installation_idx").on(table.installationId, table.createdAt),
...
export type AppLifecycleOperationModel = typeof appLifecycleOperations.$inferSelect;
