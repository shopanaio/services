...
export class AppInstallationStore {
  protected readonly txManager: Repository["txManager"];

  constructor(
    @Inject(Repository)
    private readonly repository: Repository,
  ) {
    this.txManager = repository.txManager;
  }
  ...
  async assignCapabilityResource(input: {
    ...
    return this.repository.capability.assignResource(input);
  }

  @Transactional()
  async unassignCapabilityResource(input: {
    readonly storeId: string;
    readonly installationId: string;
    readonly capability: string;
    readonly target: CapabilityRouteTarget;
  }): Promise<number> {
    return this.repository.capability.unassignResource(input);
  }
