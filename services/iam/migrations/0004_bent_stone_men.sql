...
ALTER TABLE "iam"."service_linked_resource" ADD CONSTRAINT "service_linked_resource_organization_id_organization_id_fk" FOREIGN KEY ("organization_id") REFERENCES "iam"."organization"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
CREATE UNIQUE INDEX "uq_service_linked_resource_active" ON "iam"."service_linked_resource" USING btree ("organization_id","resource_kind","resource_id") WHERE deleted_at IS NULL;--> statement-breakpoint
