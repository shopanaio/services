...
import { applicationAuthProviderScopesSchema } from "../auth/applicationAuthConfiguration.js";
import {
...
export class ApplicationAuthAdminQueryRepository extends BaseRepository {
  ...
  @ReadOnly()
  async getByApplicationKeys(keys: readonly ApplicationKey[]): Promise<ApplicationAuthAdminView[]> {
    ...
    const [origins, deliveryProfiles, providers] = await Promise.all([
      ...
      this.connection
        ...
        .where(inArray(applicationAuthDeliveryProfile.applicationId, allowedApplicationIds)),
      this.connection
        .select({
        ...
        .where(inArray(applicationAuthProvider.applicationId, allowedApplicationIds))
        .orderBy(applicationAuthProvider.applicationId, applicationAuthProvider.provider),
    ]);
    ...
    const providersByApplication = groupBy(
      providers.map((provider) => {
        ...
        const scopes = applicationAuthProviderScopesSchema.parse(provider.scopesJson);
    ...
    return allowedConfigurations.map(({ organizationId, configuration }) => ({
      ...
      configuration,
      origins: Object.freeze([...(originsByApplication.get(configuration.applicationId) ?? [])]),
      deliveryProfile: deliveryByApplication.get(configuration.applicationId) ?? null,
