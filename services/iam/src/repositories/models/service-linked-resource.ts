...
export const serviceLinkedResource = iamSchema.table(
  ...
  (table) => [
    ...
    check("service_linked_owner_type_not_empty", sql`${table.linkedOwnerType} <> ''`),
    ...
    index("idx_service_linked_owner_active")
      .on(table.organizationId, table.linkedService, table.linkedOwnerType, table.linkedOwnerId)
      .where(sql`deleted_at IS NULL`),
  ],
);
