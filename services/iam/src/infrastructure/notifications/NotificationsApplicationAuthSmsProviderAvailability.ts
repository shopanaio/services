...
export class NotificationsApplicationAuthSmsProviderAvailability implements ApplicationAuthSmsProviderAvailabilityPort {
  constructor(
  ...
  async isConfiguredForApplication(applicationId: string): Promise<boolean> {
    const [application] = await this.repository.application.getByKeys([{ id: applicationId }]);
  ...
  private async isConfiguredForStore(storeId: string): Promise<boolean> {
    ...
    for (const route of routes) {
      try {
        ...
        const capabilities = result.data as Notifications.NotificationProviderCapabilities;
        if (Array.isArray(capabilities?.channels) && capabilities.channels.includes("SMS")) {
          return true;
        }
      } catch {
