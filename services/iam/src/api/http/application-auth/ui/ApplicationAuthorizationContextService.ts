...
export class ApplicationAuthorizationContextService {
  ...
  async captureSignedOAuthQuery(input: {
    ...
    const parsed = await this.verifySignedOAuthQuery(input.runtime, input.rawQuery);
    if (
      input.currentStep === "consent" &&
      (!input.sessionId || parsed.postLoginSessionId !== input.sessionId)
    ) {
      throw new ApplicationAuthRequestError("Authorization context session is invalid");
    }
    if (input.currentStep === "login" && parsed.postLoginSessionId !== undefined) {
      throw new ApplicationAuthRequestError("Authorization context session is unexpected");
    }
    ...
    if (!client || !client.redirectUris.includes(parsed.redirectUri)) {
      throw new ApplicationAuthRequestError("Authorization context client is invalid");
    }
    if (input.previousOpaqueId) {
      await this.contexts.consume(input.runtime.applicationId, input.previousOpaqueId);
    }
  ...
  async readFromRequest(
    ...
    const context = await this.contexts.findActive(runtime.applicationId, opaqueId);
  ...
  async rotate(
    ...
    const rotated = await this.contexts.rotate(runtime.applicationId, active.opaqueId, input);
    if (!rotated) {
  ...
  async consume(
    ...
    const consumed = await this.contexts.consume(runtime.applicationId, active.opaqueId);
    if (!consumed) {
      throw new ApplicationAuthRequestError("Authorization context is unavailable or expired");
    }
  }
  ...
  async assertCsrfToken(
    ...
    const expected = await this.createCsrfToken(runtime, active.opaqueId, action);
  ...
  async buildSignedOAuthQuery(
    ...
    if (active.context.resource !== runtime.resource) {
      throw new ApplicationAuthRequestError("Authorization context client is unavailable");
    }
    const redirectUri = await this.resolveBoundRedirectUri(runtime, active);
    if (hashValue(POST_LOGIN_RETURN_PATH) !== active.context.postLoginReturnPathHash) {
      throw new ApplicationAuthRequestError("Authorization context redirect binding is invalid");
    }
    ...
    if (active.context.currentStep === "consent" && active.context.sessionId) {
      params.set("ba_pl", active.context.sessionId);
    }
    appendSignedParameterNames(params);
    const signature = await makeSignature(
      canonicalize(params).toString(),
      this.secrets.deriveRealmSecret(runtime.applicationId, runtime.secretKeyVersion),
    );
  ...
  async assertOAuthRedirectTarget(
    ...
    if (!isBoundOAuthRedirectTarget(target, redirectUri)) {
      throw new ApplicationAuthRequestError("Authorization redirect target is invalid");
    }
  }

  async serializeCookie(runtime: ApplicationAuthFactoryRuntime, opaqueId: string): Promise<string> {
    const signature = await makeSignature(
      contextCookiePayload(runtime.applicationId, opaqueId),
      this.contextSecret(runtime),
    );
  ...
  private async verifySignedOAuthQuery(
    ...
    if ([...new Set(params.keys())].some((name) => !ALLOWED_SIGNED_QUERY_PARAMETERS.has(name))) {
      throw new ApplicationAuthRequestError("Authorization query contains unsupported parameters");
    }
    const signature = single(params, "sig", 16, 1024);
    const signedNames = params.getAll("ba_param");
    if (signedNames.length === 0 || new Set(signedNames).size !== signedNames.length) {
      throw new ApplicationAuthRequestError("Signed authorization query is malformed");
    }
    const signedSet = new Set(signedNames);
    for (const key of new Set(params.keys())) {
      if (key !== "sig" && !signedSet.has(key)) {
        throw new ApplicationAuthRequestError("Authorization query contains unsigned parameters");
      }
    }
    if (!signedSet.has("ba_param")) {
      throw new ApplicationAuthRequestError("Signed authorization query is incomplete");
    }
    ...
    if (!constantTimeEqual(signature, expected)) {
      throw new ApplicationAuthRequestError("Signed authorization query is invalid");
    }
    ...
    if (
      ...
      throw new ApplicationAuthRequestError("Signed authorization query is expired or invalid");
    }
    if (single(params, "response_type", 1, 16) !== "code") {
      throw new ApplicationAuthRequestError("Authorization response type is invalid");
    }
    const prompt = params.getAll("prompt");
    if (prompt.length > 1) {
      throw new ApplicationAuthRequestError("Authorization prompt is invalid");
    }
    if (prompt[0]) {
      ...
      if (
        ...
        values.some((value) => value !== "login" && value !== "create" && value !== "consent")
      ) {
        throw new ApplicationAuthRequestError("Authorization prompt is invalid");
      }
    }
    ...
    if (resources.length > 1 || (resources[0] !== undefined && resources[0] !== runtime.resource)) {
      throw new ApplicationAuthRequestError("Authorization resource is invalid");
    }
    const postLoginSessionIds = params.getAll("ba_pl");
    if (
      postLoginSessionIds.length > 1 ||
      (postLoginSessionIds[0] !== undefined &&
        (postLoginSessionIds[0].length < 16 || postLoginSessionIds[0].length > 512))
    ) {
      throw new ApplicationAuthRequestError("Authorization context session is invalid");
    }
    const scope = single(params, "scope", 1, 2048).split(" ").filter(Boolean);
    if (scope.length === 0 || new Set(scope).size !== scope.length) {
      throw new ApplicationAuthRequestError("Authorization scopes are invalid");
    }
    return {
...
function single(
  ...
  if (
    ...
    throw new ApplicationAuthRequestError(`Authorization query parameter ${name} is invalid`);
  }
  return value;
}
...
function isBoundOAuthRedirectTarget(targetValue: string, registeredValue: string): boolean {
  ...
  return [...registered.searchParams.entries()].every(([registeredName, registeredValue]) =>
    targetEntries.some(
