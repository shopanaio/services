 ...
 */
export class InMemoryApplicationAuthRateLimitAdapter implements ApplicationAuthRateLimitPort {
...
export class ApplicationAuthRateLimiter {
  ...
  async assertEmailOtpRequest(input: {
    ...
    await this.consume(
      [
        bucket(input, "email-otp-request:cooldown", identityAndRequester, 1, 60),
        bucket(input, "email-otp-request:identity-window", identity, 3, 15 * 60),
        bucket(input, "email-otp-request:identity-day", identity, 20, 24 * 60 * 60),
        bucket(input, "email-otp-request:ip-day", ip, 100, 24 * 60 * 60),
      ],
      "required",
    );
  }
  ...
  async assertEmailOtpVerify(input: {
    ...
    await this.consume(
      [
        bucket(input, "email-otp-verify:challenge", verification, 3, 5 * 60),
        bucket(input, "email-otp-verify:ip-window", ip, 10, 15 * 60),
      ],
      "required",
    );
  }
  ...
  async assertToken(input: {
    ...
    const subject = `${digest(input.secret, input.clientId)}:${digest(input.secret, input.ip)}`;
    await this.consume(
      [
    ...
    if (this.port) {
      try {
        ...
        if (availability === "required") {
          throw new ApplicationAuthRateLimitError(503, 60, "temporarily_unavailable");
        }
      }
