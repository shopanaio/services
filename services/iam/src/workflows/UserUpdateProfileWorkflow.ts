...
export interface UserUpdateProfileWorkflowInput extends UserUpdateProfileParams {
  userId: string;
...
/**
 * Durable workflow for user profile update.
 *
 * Steps:
 ...
 */
@Injectable()
export class UserUpdateProfileWorkflow extends BrokerWorkflows {
  ...
  @Workflow("userUpdateProfile")
  async run(input: UserUpdateProfileWorkflowInput): Promise<UserUpdateProfileResult> {
  ...
  @Step()
  async updateUserProfile(input: UserUpdateProfileParams): Promise<UserUpdateProfileResult> {
    return this.kernel.runScript(UserUpdateProfileScript, input);
  }

  @Step()
  async syncAvatarBackRefs(
    userId: string,
    previousAvatarId: string | null,
    nextAvatarId: string | null
  ): Promise<void> {
