...
export async function startServer(config: ServerConfig) {
  ...
  if (databaseUrl) {
    ...
    console.log("[CUSTOMERS] Database connected, Kernel initialized");
  } else {
    console.warn(
      "[CUSTOMERS] No DATABASE_URL configured, running without database"
    );
  }
  ...
  app.get("/", async (_request, reply) => {
    return reply.send({ status: "ok", service: "customers" });
  });
  ...
  app.log.info(
    `Customers GraphQL Admin API ready at http://localhost:${config.port}/graphql`
  );
