...
import { customersSchema, taxExemptionStatusEnum, taxIdentifierStatusEnum } from "./schema.js";
...
export const customerTaxIdentifier = customersSchema.table(
  "customer_tax_identifier",
  {
    ...
    status: taxIdentifierStatusEnum("status").notNull().default("UNVERIFIED"),
  ...
  (table) => [
    check("customer_tax_identifier_type_check", sql`length(btrim(${table.identifierType})) > 0`),
    check("customer_tax_identifier_value_check", sql`length(btrim(${table.normalizedValue})) > 0`),
    check(
...
export const customerTaxExemption = customersSchema.table(
  ...
  (table) => [
    check("customer_tax_exemption_code_check", sql`length(btrim(${table.code})) > 0`),
    check(
