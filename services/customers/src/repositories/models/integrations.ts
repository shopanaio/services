...
import { check, index, jsonb, timestamp, uniqueIndex, uuid, varchar } from "drizzle-orm/pg-core";
...
export const customerExternalReference = customersSchema.table(
  "customer_external_reference",
  {
    ...
    externalType: varchar("external_type", { length: 64 }).notNull().default("customer"),
  ...
  (table) => [
    ...
    check("customer_external_reference_type_check", sql`length(btrim(${table.externalType})) > 0`),
    check("customer_external_reference_id_check", sql`length(btrim(${table.externalId})) > 0`),
...
export type CustomerExternalReference = typeof customerExternalReference.$inferSelect;
export type NewCustomerExternalReference = typeof customerExternalReference.$inferInsert;
