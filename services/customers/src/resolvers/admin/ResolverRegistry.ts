...
export class ResolverRegistry {
  ...
  async customerConnection(input: CustomerConnectionInput) {
    const { CustomerConnectionResolver } = await import(
      "./CustomerConnectionResolver.js"
    );
  ...
  async addressConnection(input: CustomerAddressConnectionInput) {
    const { CustomerAddressConnectionResolver } = await import(
      "./CustomerAddressConnectionResolver.js"
    );
    return new CustomerAddressConnectionResolver(input, this.ctx);
  }

  async taxIdentifier(id: string) {
    const { CustomerTaxIdentifierResolver } = await import(
      "./CustomerTaxIdentifierResolver.js"
    );
  ...
  async taxIdentifierConnection(input: CustomerTaxIdentifierConnectionInput) {
    const { CustomerTaxIdentifierConnectionResolver } = await import(
      "./CustomerTaxIdentifierConnectionResolver.js"
    );
    return new CustomerTaxIdentifierConnectionResolver(input, this.ctx);
  }

  async taxExemption(id: string) {
    const { CustomerTaxExemptionResolver } = await import(
      "./CustomerTaxExemptionResolver.js"
    );
  ...
  async taxExemptionConnection(input: CustomerTaxExemptionConnectionInput) {
    const { CustomerTaxExemptionConnectionResolver } = await import(
      "./CustomerTaxExemptionConnectionResolver.js"
    );
    return new CustomerTaxExemptionConnectionResolver(input, this.ctx);
  }

  async consent(id: string) {
    const { CustomerConsentResolver } = await import(
      "./CustomerConsentResolver.js"
    );
    return new CustomerConsentResolver(id, this.ctx);
  }

  async consentEvent(id: string) {
    const { CustomerConsentEventResolver } = await import(
      "./CustomerConsentEventResolver.js"
    );
  ...
  async consentEventConnection(input: CustomerConsentEventConnectionInput) {
    const { CustomerConsentEventConnectionResolver } = await import(
      "./CustomerConsentEventConnectionResolver.js"
    );
    return new CustomerConsentEventConnectionResolver(input, this.ctx);
  }

  async group(id: string) {
    const { CustomerGroupResolver } = await import(
      "./CustomerGroupResolver.js"
    );
    return new CustomerGroupResolver(id, this.ctx);
  }

  async groupConnection(input: CustomerGroupRelayInput) {
    const { CustomerGroupConnectionResolver } = await import(
      "./CustomerGroupConnectionResolver.js"
    );
    return new CustomerGroupConnectionResolver(input, this.ctx);
  }

  async groupMembership(id: string) {
    const { CustomerGroupMembershipResolver } = await import(
      "./CustomerGroupMembershipResolver.js"
    );
    return new CustomerGroupMembershipResolver(id, this.ctx);
  }

  async groupMembershipConnection(
    input: CustomerGroupMembershipConnectionInput
  ) {
    const { CustomerGroupMembershipConnectionResolver } = await import(
      "./CustomerGroupMembershipConnectionResolver.js"
    );
  ...
  async tagConnection(input: CustomerTagRelayInput) {
    const { CustomerTagConnectionResolver } = await import(
      "./CustomerTagConnectionResolver.js"
    );
    return new CustomerTagConnectionResolver(input, this.ctx);
  }

  async tagAssignment(id: string) {
    const { CustomerTagAssignmentResolver } = await import(
      "./CustomerTagAssignmentResolver.js"
    );
  ...
  async tagAssignmentConnection(input: CustomerTagAssignmentConnectionInput) {
    const { CustomerTagAssignmentConnectionResolver } = await import(
      "./CustomerTagAssignmentConnectionResolver.js"
    );
    return new CustomerTagAssignmentConnectionResolver(input, this.ctx);
  }

  async segment(id: string) {
    const { CustomerSegmentResolver } = await import(
      "./CustomerSegmentResolver.js"
    );
    return new CustomerSegmentResolver(id, this.ctx);
  }

  async segmentConnection(input: CustomerSegmentRelayInput) {
    const { CustomerSegmentConnectionResolver } = await import(
      "./CustomerSegmentConnectionResolver.js"
    );
    return new CustomerSegmentConnectionResolver(input, this.ctx);
  }

  async segmentMembership(id: string) {
    const { CustomerSegmentMembershipResolver } = await import(
      "./CustomerSegmentMembershipResolver.js"
    );
    return new CustomerSegmentMembershipResolver(id, this.ctx);
  }

  async segmentMembershipConnection(
    input: CustomerSegmentMembershipConnectionInput
  ) {
    const { CustomerSegmentMembershipConnectionResolver } = await import(
      "./CustomerSegmentMembershipConnectionResolver.js"
    );
    return new CustomerSegmentMembershipConnectionResolver(input, this.ctx);
  }

  async statistics(customerId: string) {
    const { CustomerStatisticsResolver } = await import(
      "./CustomerStatisticsResolver.js"
    );
    return new CustomerStatisticsResolver(customerId, this.ctx);
  }

  async monetaryStatistics(id: string) {
    const { CustomerMonetaryStatisticsResolver } = await import(
      "./CustomerMonetaryStatisticsResolver.js"
    );
    return new CustomerMonetaryStatisticsResolver(id, this.ctx);
  }

  async monetaryStatisticsConnection(
    input: CustomerMonetaryStatisticsConnectionInput
  ) {
    const { CustomerMonetaryStatisticsConnectionResolver } = await import(
      "./CustomerMonetaryStatisticsConnectionResolver.js"
    );
    return new CustomerMonetaryStatisticsConnectionResolver(input, this.ctx);
  }

  async merge(id: string) {
    const { CustomerMergeResolver } = await import(
      "./CustomerMergeResolver.js"
    );
    return new CustomerMergeResolver(id, this.ctx);
  }

  async mergeConnection(input: CustomerMergeRelayInput) {
    const { CustomerMergeConnectionResolver } = await import(
      "./CustomerMergeConnectionResolver.js"
    );
    return new CustomerMergeConnectionResolver(input, this.ctx);
  }

  async dataRequest(id: string) {
    const { CustomerDataRequestResolver } = await import(
      "./CustomerDataRequestResolver.js"
    );
    return new CustomerDataRequestResolver(id, this.ctx);
  }

  async dataRequestConnection(input: CustomerDataRequestRelayInput) {
    const { CustomerDataRequestConnectionResolver } = await import(
      "./CustomerDataRequestConnectionResolver.js"
    );
