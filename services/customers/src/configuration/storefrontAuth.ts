...
export function resolveStorefrontAuthUrls(storeName: string): StorefrontAuthUrls {
  ...
  const renderedOrigin = configuration.origin_template.replaceAll("{store}", storeName);
  ...
  if (
    ...
    throw new Error("Customers storefront_auth.origin_template must resolve to an origin");
  }
  ...
  return {
    ...
    postLogoutRedirectUri: new URL(configuration.post_logout_path, `${origin}/`).href,
  };
}
