...
-- All tables are scoped to project_id
...
CREATE INDEX idx_buckets_status ON buckets (project_id, status) WHERE deleted_at IS NULL;
...
CREATE INDEX idx_files_provider ON files (project_id, provider) WHERE deleted_at IS NULL;
CREATE INDEX idx_files_created_at ON files (project_id, created_at DESC) WHERE deleted_at IS NULL;
CREATE INDEX idx_files_deleted_at ON files (deleted_at) WHERE deleted_at IS NOT NULL;

-- Idempotency indexes (prevent duplicates)
CREATE UNIQUE INDEX idx_files_source_url ON files (project_id, source_url) WHERE deleted_at IS NULL AND source_url IS NOT NULL;
CREATE UNIQUE INDEX idx_files_idempotency_key ON files (project_id, idempotency_key) WHERE deleted_at IS NULL AND idempotency_key IS NOT NULL;
...
CREATE UNIQUE INDEX idx_s3_objects_key ON s3_objects (bucket_id, object_key);
CREATE UNIQUE INDEX idx_s3_objects_hash ON s3_objects (project_id, content_hash) WHERE content_hash IS NOT NULL;
...
CREATE INDEX idx_external_media_external_id ON external_media (project_id, external_id);
...
CREATE TABLE
  bucket_rotation_log (
    ...
    old_bucket_id uuid REFERENCES buckets (id) ON DELETE SET NULL,
    -- New bucket that became active
    new_bucket_id uuid REFERENCES buckets (id) ON DELETE SET NULL,
    -- Reason for rotation
...
CREATE INDEX idx_bucket_rotation_log_project ON bucket_rotation_log (project_id, created_at DESC);
