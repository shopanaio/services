...
export class CdnConfigurationResolver extends MediaType<string, CdnConfiguration> {
  async $preload() {
    ...
    const configuration = assetGroup
      ? await this.$ctx.kernel.repository.cdnConfiguration.findById(assetGroup.id, this.$props)
      : null;
    if (!configuration) {
  ...
  id() {
    return encodeGlobalIdByType(this.$props, GlobalIdEntity.CdnConfiguration);
  }
  ...
  async signingMode() {
    return this.$get("signingMode");
  }
  async secretRef() {
    return this.$get("secretRef");
  }
  async transformStrategy() {
    return this.$get("transformStrategy");
  }
  async urlTemplate() {
  ...
  async transformConfig() {
    return this.$get("transformConfig");
  }
  async createdAt() {
