...
export class ContentLoader {
  ...
  public readonly contentTranslation: DataLoader<string, ContentTranslation | null>;
  public readonly contentPublications: DataLoader<string, ContentPublication[]>;
  public readonly contentPublication: DataLoader<string, ContentPublication | null>;
  ...
  constructor(repository: Repository) {
    ...
    this.contentMetrics = new DataLoader(async (contentIds) => {
      ...
      return contentIds.map((id) => rows.find((row) => row.contentId === id) ?? null);
    });

    this.contentTranslations = new DataLoader(async (contentIds) => {
      const rows = await repository.content.getTranslationsByContentIds(contentIds);
      return contentIds.map((id) => rows.filter((row) => row.contentId === id));
    });
    ...
    this.contentPublications = new DataLoader(async (contentIds) => {
      const rows = await repository.content.getPublicationsByContentIds(contentIds);
      return contentIds.map((id) => rows.filter((row) => row.contentId === id));
    });
