...
export class RatingCriterionConnectionResolver extends BaseConnectionResolver<RatingCriterionRelayInput> {
  $preload(): Promise<ConnectionData> {
    const where: RatingCriterionRelayInput["where"] = {
      _and: [{ isActive: { _eq: true } }, ...(this.$props.where ? [this.$props.where] : [])],
    };
  ...
  protected createNodeResolver(id: string) {
    return this.resolvers.ratingCriterion(id);
  }
}
...
export class ApplicableRatingCriterionConnectionResolver extends BaseConnectionResolver<ApplicableCriterionConnectionInput> {
  async $preload(): Promise<ConnectionData> {
    ...
    const after = this.$props.after
      ? edges.findIndex((edge) => edge.cursor === this.$props.after) + 1
      : 0;
    const beforeIndex = this.$props.before
      ? edges.findIndex((edge) => edge.cursor === this.$props.before)
      : edges.length;
    ...
    const startIndex = selected[0]
      ? edges.findIndex((edge) => edge.cursor === selected[0]!.cursor)
      : -1;
    const endIndex = selected.at(-1)
      ? edges.findIndex((edge) => edge.cursor === selected.at(-1)!.cursor)
      : -1;
    return {
      edges: selected,
...
export async function getApplicableCriteria(
  ctx: ServiceContext,
  productId: string,
): Promise<RatingCriterion[]> {
  const catalog = await ctx.kernel
    .getServices()
  ...
  if (!catalog.ok)
    throw new Error(`Catalog product categories could not be resolved: ${catalog.code}`);
  const product = catalog.data.products?.edges?.[0]?.node;
  const categoryIds = new Set((product?.categories ?? []).map((category) => category.id));
  const connection = await ctx.kernel.repository.configuration.getCriterionConnection({
    first: 100,
    where: { isActive: { _eq: true } },
  });
  const rows = (
    await ctx.loaders.ratingCriterion.loadMany(connection.edges.map((edge) => edge.nodeId))
  ).filter((row): row is RatingCriterion => !(row instanceof Error) && !!row);
  const assignments = await Promise.all(
    rows.map((row) => ctx.loaders.ratingCriterionAssignments.load(row.id)),
  );
  return rows.filter(
    (row, index) =>
