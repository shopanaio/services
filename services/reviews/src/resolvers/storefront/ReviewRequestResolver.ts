...
export class ReviewRequestResolver extends ReviewsType<string, ReviewRequest> {
  async $preload() { const row = await this.$ctx.loaders.reviewRequest.load(this.$props); if (!row || row.customerId !== this.$ctx.customer?.id) throw new PreloadNotFoundError("Review request not found"); return row; }
  ...
  async review() { const id = await this.$get("reviewId"); return id ? this.resolvers.review(id) : null; }
  ...
  events(args: ReviewRequestEventRelayInput) { return new ReviewRequestEventConnectionResolver({ ...args, reviewRequestId: this.$props }, this.$ctx); }
...
export class ReviewRequestEventResolver extends ReviewsType<string, ReviewRequestEvent> {
  async $preload() { const row = await this.$ctx.loaders.reviewRequestEvent.load(this.$props); if (!row) throw new PreloadNotFoundError("Review request event not found"); const request = await this.$ctx.loaders.reviewRequest.load(row.reviewRequestId); if (!request || request.customerId !== this.$ctx.customer?.id) throw new PreloadNotFoundError("Review request event not found"); return row; }
