...
export class RatingCriterionResolver extends ReviewsType<string, RatingCriterion> {
  async $preload() {
    ...
    if (!row || !row.isActive || row.deletedAt)
      throw new PreloadNotFoundError("Active rating criterion not found");
    return row;
  }
  id() {
  ...
  async description() {
    const rows = await this.$ctx.loaders.ratingCriterionTranslations.load(this.$props);
    return (
      rows.find((row) => row.locale === this.$ctx.locale)?.description ??
      this.$get("defaultDescription")
    );
  }
  weight() {
  ...
  async translations() {
    return (await this.$ctx.loaders.ratingCriterionTranslations.load(this.$props)).map(
      (row) => new RatingCriterionTranslationResolver(row, this.$ctx),
    );
  }
  async assignments() {
...
> {
  async $preload() {
    const row = await this.$ctx.loaders.ratingCriterionAssignment.load(this.$props);
    if (!row) throw new PreloadNotFoundError("Rating criterion assignment not found");
    return row;
  }
  id() {
  ...
  async target() {
    const row = await this.$preload();
    return row.targetType === "PRODUCT"
      ? productReference(row.targetId)
      : categoryReference(row.targetId);
  }
  async isRequired() {
    const value = await this.$get("isRequiredOverride");
    return (
      value ?? (await this.resolvers.ratingCriterion(await this.$get("criterionId"))).isRequired()
    );
  }
  async sortIndex() {
    const value = await this.$get("sortIndexOverride");
    return (
      value ?? (await this.resolvers.ratingCriterion(await this.$get("criterionId"))).sortIndex()
    );
  }
  createdAt() {
    return this.$get("createdAt");
