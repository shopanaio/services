...
export class MutationResolver extends ReviewsType<Record<string, never>> {
  ...
  async reviewCreate(args: { input: ReviewCreateInput }) {
    ...
    const variantId = args.input.variantId
      ...
      : null;
    if (!args.input.content.idempotencyKey.trim())
      errors.push(
        error("Idempotency key is required", "INVALID_IDEMPOTENCY_KEY", [
          "input",
          "content",
          "idempotencyKey",
        ]),
    ...
    if (!config.reviewsEnabled)
      errors.push(error("Reviews are disabled", "REVIEWS_DISABLED", ["input"]));
    if ((args.input.media?.length ?? 0) > config.maxReviewMediaCount)
      errors.push(
        error(
          `A review may contain at most ${config.maxReviewMediaCount} media items`,
    ...
    const purchase = await this.verifyPurchase(
      ...
      productId,
      variantId,
      config.verifiedPurchaseRequired,
    );
    if (purchase.error) errors.push(purchase.error);
    if (this.$ctx.customer && config.reviewDuplicatePolicy !== "ALLOW_MULTIPLE") {
      const orderLineId = args.input.orderLineId
        ? this.decodeId(args.input.orderLineId, GlobalIdEntity.OrderLine)
        : null;
      ...
      if (duplicate.totalCount > 0)
        errors.push(
          error("A review already exists for this purchase", "DUPLICATE_REVIEW", [
            "input",
            config.reviewDuplicatePolicy === "ONE_PER_ORDER_LINE" ? "orderLineId" : "productId",
          ]),
        );
    }
    ...
    for (const [index, rating] of (args.input.ratings ?? []).entries()) {
      ...
      if (!applicableIds.has(criterionId))
        errors.push(
          error("Rating criterion is not applicable to this product", "CRITERION_NOT_APPLICABLE", [
            "input",
            "ratings",
            String(index),
            "criterionId",
    ...
    for (const criterion of applicableCriteria) {
      if (criterion.isRequired && !submittedIds.has(criterion.id))
        errors.push(
          error("A required rating criterion is missing", "REQUIRED_CRITERION_MISSING", [
            "input",
            "ratings",
          ]),
        );
    ...
    const replay = await this.idempotentContent(
      args.input.content.idempotencyKey,
      "REVIEW",
      args.input.content.author?.email,
    );
    ...
    const params = {
      ...
      orderLineId: args.input.orderLineId
        ? this.decodeId(args.input.orderLineId, GlobalIdEntity.OrderLine)
        : null,
      rating: args.input.rating,
      ratings: (args.input.ratings ?? []).map((item) => ({
        criterionId: this.decodeId(item.criterionId, GlobalIdEntity.ReviewRatingCriterion),
        value: item.value,
      })),
  ...
  async reviewUpdate(args: { input: ReviewUpdateInput }) {
    ...
    if (args.input.rating != null && (args.input.rating < 1 || args.input.rating > 5))
      errors.push(error("Rating must be between 1 and 5", "INVALID_RATING", ["input", "rating"]));
    const config = await this.configuration();
    if ((args.input.media?.length ?? 0) > config.maxReviewMediaCount)
      errors.push(
        error(
    ...
    if (args.input.ratings && review) {
      errors.push(
        ...(await this.validateRatingReplacement(review.review.productId, args.input.ratings)),
      );
    }
  ...
  async reviewDelete(args: { input: ReviewContentDeleteInput }) {
    ...
    if (result.productId)
      await this.$ctx.kernel.repository.summary.refreshProductReviewSummary(result.productId);
    return {
      deletedReviewId: result.deletedReviewId ? args.input.id : null,
  ...
  async productQuestionCreate(args: { input: ProductQuestionCreateInput }) {
    ...
    if (!config.questionsEnabled)
      errors.push(error("Product questions are disabled", "QUESTIONS_DISABLED", ["input"]));
    ...
    if (replay.id)
      return { productQuestion: await this.resolvers.productQuestion(replay.id), userErrors: [] };
    const params = {
    ...
    await this.$ctx.kernel.repository.summary.refreshProductQuestionSummary(
      result.productQuestion.productId,
    );
    return {
      productQuestion: await this.resolvers.productQuestion(result.productQuestion.id),
      userErrors: result.userErrors,
    };
  }
  ...
  async productQuestionDelete(args: { input: ReviewContentDeleteInput }) {
    ...
    if (result.productId)
      await this.$ctx.kernel.repository.summary.refreshProductQuestionSummary(result.productId);
    return {
      deletedProductQuestionId: result.deletedProductQuestionId ? args.input.id : null,
      userErrors: result.userErrors,
    };
  }
  ...
  async productQuestionAnswerCreate(args: { input: ProductQuestionAnswerCreateInput }) {
    ...
    if (!customer)
      return { productQuestion: null, answer: null, userErrors: [authenticationError()] };
    const config = await this.configuration();
    if (!config.customerAnswersEnabled)
      return {
        productQuestion: null,
        answer: null,
        userErrors: [error("Customer answers are disabled", "ANSWERS_DISABLED", ["input"])],
      };
    ...
    if (!question || question.content.status !== "PUBLISHED" || question.content.redactedAt)
      return {
        ...
        answer: null,
        userErrors: [
          error("Product question not found", "NOT_FOUND", ["input", "productQuestionId"]),
        ],
      };
    ...
    if (replay.id)
      return {
        productQuestion: await this.resolvers.productQuestion(questionId),
        answer: await this.resolvers.productQuestionAnswer(replay.id),
        userErrors: [],
      };
    ...
    if (created.status === "error")
      return { productQuestion: null, answer: null, userErrors: created.userErrors };
    if (created.status === "limit_exceeded")
      return {
        productQuestion: null,
        answer: null,
        userErrors: [
          error("The question has reached its answer limit", "ANSWER_LIMIT_EXCEEDED", ["input"]),
        ],
      };
    ...
    return {
      productQuestion: await this.resolvers.productQuestion(questionId),
      answer: await this.resolvers.productQuestionAnswer(created.answerId),
      userErrors: [],
    };
  }
  ...
  async productQuestionAnswerUpdate(args: { input: ProductQuestionAnswerUpdateInput }) {
    ...
    if (answer) {
      ...
      if (question) {
        await this.$ctx.kernel.repository.engagement.refreshContentMetrics(question.question.id);
        await this.$ctx.kernel.repository.summary.refreshProductQuestionSummary(
          question.question.productId,
        );
      }
  ...
  async productQuestionSubscriptionSet(args: { input: ProductQuestionSubscriptionSetInput }) {
    ...
    const questionId = this.decodeId(args.input.productQuestionId, GlobalIdEntity.ProductQuestion);
    const question = await this.$ctx.kernel.repository.productQuestion.findById(questionId);
    if (!question || question.content.status !== "PUBLISHED")
      return {
        subscription: null,
        userErrors: [
          error("Product question not found", "NOT_FOUND", ["input", "productQuestionId"]),
    ...
    const current = await this.$ctx.kernel.repository.questionSubscription.findByCustomer(
      questionId,
      customer.id,
      channel,
    );
    ...
    const created = await this.$ctx.kernel.repository.questionSubscription.create({
      questionId,
      subscriberCustomerId: customer.id,
      subscriberKey: `customer:${customer.id}`,
      channel,
      status: args.input.subscribed ? "ACTIVE" : "UNSUBSCRIBED",
      locale: args.input.locale ?? this.$ctx.locale ?? this.$ctx.store.defaultLocale,
      lastNotifiedAt: null,
  ...
  async reviewContentVoteSet(args: { input: ReviewContentVoteSetInput }) {
    ...
    if (!content)
      return {
        content: null,
        vote: null,
        userErrors: [error("Content not found", "NOT_FOUND", ["input", "contentId"])],
      };
    const key = viewerKey(this.$ctx);
    const vote = await this.$ctx.kernel.repository.engagement.upsertVote({
      contentId,
      voterCustomerId: this.$ctx.customer?.id ?? null,
      voterKey: key,
    ...
    return {
      content: await this.resolvers.content(contentId),
      vote: await this.resolvers.contentVote(vote.id),
      userErrors: [],
    };
  }
  ...
  async reviewContentVoteRemove(args: { input: ReviewContentVoteRemoveInput }) {
    ...
    const removed = await this.$ctx.kernel.repository.engagement.deleteVote(
      contentId,
      viewerKey(this.$ctx),
    );
    await this.$ctx.kernel.repository.engagement.refreshContentMetrics(contentId);
    ...
    return {
      content: await this.resolvers.content(contentId),
      removedVoteId: removed ? this.encodeId(removed.id, GlobalIdEntity.ReviewContentVote) : null,
      userErrors: [],
    };
  }
  ...
  async reviewContentReportCreate(args: { input: ReviewContentReportCreateInput }) {
    ...
    if (args.input.reason === "OTHER" && !details)
      return {
        content: null,
        report: null,
        userErrors: [
          error("Details are required when reason is OTHER", "DETAILS_REQUIRED", [
            "input",
            "details",
          ]),
        ],
    ...
    if (details && details.length > 2000)
      return {
        ...
        report: null,
        userErrors: [
          error("Details cannot exceed 2000 characters", "INVALID_DETAILS", ["input", "details"]),
        ],
      };
    const key = viewerKey(this.$ctx);
    const existing = await this.$ctx.kernel.repository.engagement.findActiveReportByViewer(
      contentId,
      key,
    );
    const report =
      existing ??
      (await this.$ctx.kernel.repository.engagement.createReport({
        contentId,
        reporterCustomerId: this.$ctx.customer?.id ?? null,
        reporterKey: key,
    ...
    return {
      content: await this.resolvers.content(contentId),
      report: await this.resolvers.contentReport(report.id),
      userErrors: [],
    };
  }
  ...
  private async verifyPurchase(
    ...
    if (!input.orderId && !input.orderLineId) {
      return required
        ? {
            error: error(
              "A verified order and order line are required",
    ...
    if (!input.orderId || !input.orderLineId) {
      return {
        error: error(
          "orderId and orderLineId must be provided together",
          "INVALID_PURCHASE_REFERENCE",
          ["input", input.orderId ? "orderLineId" : "orderId"],
        ),
      };
    }
    ...
    if (!customerId) {
      return {
        error: error("Authentication is required to verify a purchase", "UNAUTHENTICATED", [
          "input",
          "orderId",
        ]),
      };
    ...
    const result = await this.$ctx.kernel
      .getServices()
      .broker.call<Orders.VerifyReviewPurchaseResult, Orders.VerifyReviewPurchaseParams>(
    ...
    if (!result.eligible) {
      return {
        error: error("The order line is not eligible for this review", result.code, [
          "input",
          "orderLineId",
        ]),
      };
  ...
  private async validateRatingReplacement(
    ...
    for (const [index, rating] of ratings.entries()) {
      ...
      if (rating.value < 1 || rating.value > 5) {
        errors.push(
          error("Criterion rating must be between 1 and 5", "INVALID_RATING", [
            "input",
            "ratings",
            String(index),
            "value",
      ...
      if (submittedIds.has(criterionId)) {
        errors.push(
          error("A criterion may only be rated once", "DUPLICATE_CRITERION", [
            "input",
            "ratings",
            String(index),
            "criterionId",
  ...
  private validateSubmission(
    ...
    if (!author?.email)
      return [
        error("Guest submissions require an email address", "GUEST_EMAIL_REQUIRED", [
          "input",
          "content",
          "author",
          "email",
  ...
  private createContent(
    ...
    return {
      ...
      body: input.body,
      locale: input.locale ?? this.$ctx.locale ?? this.$ctx.store.defaultLocale,
      author: {
        type: customer ? "CUSTOMER" : "GUEST",
        customerId: customer?.id ?? null,
        principalId: null,
        displayName,
        email: customer?.email ?? input.author?.email ?? null,
      },
  ...
  ): Promise<{ content?: ContentItem; error?: UserError }> {
    const content = await this.$ctx.kernel.repository.content.findById(id);
    if (!content || content.kind !== kind || content.authorCustomerId !== this.$ctx.customer?.id)
      return { error: error("Content not found", "NOT_FOUND", ["input", "id"]) };
    const config = await this.configuration();
    const hours =
      kind === "REVIEW"
        ? config.reviewEditWindowHours
        : kind === "PRODUCT_QUESTION"
          ? config.questionEditWindowHours
          : config.answerEditWindowHours;
    if (Date.now() > Date.parse(content.createdAt) + hours * 3_600_000)
      return {
        error: error("The edit window has expired", "EDIT_WINDOW_EXPIRED", ["input", "id"]),
      };
    return { content };
  }
  private decodeContentId(globalId: string) {
    for (const type of [
      ...
      try {
        return this.decodeId(globalId, type);
      } catch {
        continue;
      }
    }
  ...
  private async publicContent(id: string) {
    const row = await this.$ctx.kernel.repository.content.findById(id);
    return row?.status === "PUBLISHED" && !row.redactedAt ? row : null;
  }
  private async idempotentContent(
    key: string,
    kind: ContentItem["kind"],
    guestEmail?: string | null,
  ): Promise<{ id?: string; error?: UserError }> {
    const existing = await this.$ctx.kernel.repository.content.findByIdempotencyKey(
      "STOREFRONT",
      key.trim(),
    );
    if (!existing) return {};
    ...
    return existing.kind === kind && sameAuthor
      ? { id: existing.id }
      : {
          error: error("The idempotency key is already in use", "IDEMPOTENCY_KEY_CONFLICT", [
            "input",
            "content",
            "idempotencyKey",
          ]),
  ...
  private clearContent(id: string) {
    this.$ctx.loaders.content.clear(id);
  }
}

function error(message: string, code: string, field?: string[]): UserError {
...
function validateSubmissionBody(body: string, kind: ContentItem["kind"]) {
  ...
  return length < min || length > 5000
    ? error(`Content body must contain between ${min} and 5000 characters`, "INVALID_BODY", [
        "input",
        "content",
        "body",
      ])
...
function validateEdit(
  input: ReviewContentEditInput | null | undefined,
  kind: ContentItem["kind"],
): UserError[] {
  if (!input) return [];
  const errors: UserError[] = [];
  if (input.body != null) {
    const bodyError = validateSubmissionBody(input.body, kind);
    if (bodyError) errors.push(bodyError);
  }
  if (kind !== "REVIEW" && input.title != null)
    errors.push(
      error("Only reviews may have a title", "INVALID_TITLE", ["input", "content", "title"]),
    );
  if (input.title != null && input.title.trim().length > 150)
    errors.push(
      error("Title cannot exceed 150 characters", "INVALID_TITLE", ["input", "content", "title"]),
    );
  return errors;
}
function validateMediaReplacement(media: NonNullable<ReviewUpdateInput["media"]>): UserError[] {
  ...
  for (const [index, item] of media.entries()) {
    if (item.sortIndex != null && item.sortIndex < 0) {
      errors.push(
        error("Media sort index cannot be negative", "INVALID_SORT_INDEX", [
          "input",
          "media",
          String(index),
          "sortIndex",
    ...
    if (fileIds.has(item.mediaId)) {
      errors.push(
        error("A media file may only be attached once", "DUPLICATE_MEDIA", [
          "input",
          "media",
          String(index),
          "mediaId",
...
function mapCreatedContent(input: StorefrontCreatedContent) {
  ...
  return {
    ...
    authorCustomerId: input.author.customerId,
