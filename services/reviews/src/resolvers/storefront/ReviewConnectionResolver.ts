import type {
  ReviewReplyConnectionInput,
  ReviewReplyRelayInput,
} from "../../repositories/review/ReviewReplyRepository.js";
import type {
...
export class ReviewConnectionResolver extends BaseConnectionResolver<StorefrontReviewConnectionInput> {
  $preload(): Promise<ConnectionData> {
    ...
    const where: ReviewRelayInput["where"] = {
      _and: [
    ...
    return this.$ctx.kernel.repository.review.getConnection({
      ...pagination,
      where,
      orderBy,
    } as ReviewConnectionInput);
  }
  protected createNodeResolver(id: string) {
    return this.resolvers.review(id);
  }
}

export type StorefrontReviewReplyConnectionInput = Pick<
  StorefrontReviewConnectionInput,
...
export class ReviewReplyConnectionResolver extends BaseConnectionResolver<StorefrontReviewReplyConnectionInput> {
  $preload(): Promise<ConnectionData> {
    ...
    const where: ReviewReplyRelayInput["where"] = {
      _and: [
  ...
  protected createNodeResolver(id: string) {
    return this.resolvers.reviewReply(id);
  }
}

function reviewOrder(
  sort: StorefrontReviewConnectionInput["sort"],
): ReviewConnectionInput["orderBy"] {
  ...
  return [
    ...
    { field: "id", direction: fieldDirection },
  ];
}
