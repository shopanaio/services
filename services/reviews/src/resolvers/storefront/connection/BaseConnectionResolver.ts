...
export abstract class BaseConnectionResolver<TArgs = unknown>
  ...
  async edges() {
    return Promise.all((await this.$get("edges")).map(async (edge) => ({
      cursor: edge.cursor,
      node: await this.createNodeResolver(edge.nodeId),
    })));
  }
  async nodes() {
    return Promise.all((await this.$get("edges")).map((edge) => this.createNodeResolver(edge.nodeId)));
  }
