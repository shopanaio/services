...
export class ReviewRequestConnectionResolver extends BaseConnectionResolver<ReviewRequestRelayInput> {
  ...
  protected createNodeResolver(id: string) {
    return this.resolvers.reviewRequest(id);
  }
}
export class ReviewRequestEventConnectionResolver extends BaseConnectionResolver<
  ReviewRequestEventRelayInput & { reviewRequestId: string }
> {
  $preload(): Promise<ConnectionData> {
  ...
  protected createNodeResolver(id: string) {
    return this.resolvers.reviewRequestEvent(id);
  }
}
