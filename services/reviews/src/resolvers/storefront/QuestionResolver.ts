...
export class ProductQuestionResolver extends ContentResolver<ProductQuestion> {
  async $preload() {
    const row = await this.$ctx.loaders.productQuestion.load(this.$props);
    if (!row) throw new PreloadNotFoundError("Product question not found");
  ...
  async viewerSubscription() {
    ...
    const row = await this.$ctx.kernel.repository.questionSubscription.findByCustomer(
      this.$props,
      customerId,
    );
...
export class ProductQuestionAnswerResolver extends ContentResolver<QuestionAnswer> {
  async $preload() {
    const row = await this.$ctx.loaders.productQuestionAnswer.load(this.$props);
    if (!row) throw new PreloadNotFoundError("Product question answer not found");
    await this.loadContent();
    return row;
  }
  async question() {
...
export class QuestionSubscriptionResolver extends ReviewsType<string, QuestionSubscription> {
  async $preload() {
    ...
    if (!row || row.subscriberCustomerId !== this.$ctx.customer?.id)
      throw new PreloadNotFoundError("Question subscription not found");
    return row;
  }
  id() {
    return this.encodeId(this.$props, GlobalIdEntity.ProductQuestionSubscription);
  }
  async question() {
    return this.resolvers.productQuestion(await this.$get("questionId"));
  }
  async subscriberCustomer() {
    return customerReference(await this.$get("subscriberCustomerId"));
  }
  channel() {
  ...
  lastNotifiedAt() {
    return this.$get("lastNotifiedAt");
  }
  createdAt() {
