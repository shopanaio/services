...
import type { Review, ReviewMedia, ReviewRating, ReviewReply } from "../../repositories/models/index.js";
...
export class ReviewResolver extends ContentResolver<Review> {
  async $preload() { const row = await this.$ctx.loaders.review.load(this.$props); if (!row) throw new PreloadNotFoundError("Review not found"); await this.loadContent(); return row; }
  ...
  async ratings() { return (await this.$ctx.loaders.reviewRatings.load(this.$props)).map((row) => new ReviewRatingResolver(row, this.$ctx)); }
  async media() {
    const rows = (await this.$ctx.loaders.reviewMedia.load(this.$props)).filter((row) => row.status === "PUBLISHED");
  ...
  replies(args: ReviewReplyConnectionInput) { return new ReviewReplyConnectionResolver({ ...args, reviewId: this.$props }, this.$ctx); }
}
...
export class ReviewMediaResolver extends ReviewsType<string, ReviewMedia> {
  async $preload() { const row = await this.$ctx.loaders.reviewMediaItem.load(this.$props); if (!row || row.status !== "PUBLISHED") throw new PreloadNotFoundError("Published review media not found"); return row; }
  ...
  async media() { return mediaReference(await this.$get("fileId")); }
...
export class ReviewReplyResolver extends ContentResolver<ReviewReply> {
  async $preload() { const row = await this.$ctx.loaders.reviewReply.load(this.$props); if (!row) throw new PreloadNotFoundError("Review reply not found"); await this.loadContent(); return row; }
  async review() { return this.resolvers.review(await this.$get("reviewId")); }
