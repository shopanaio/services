...
export class ContentVoteResolver extends ReviewsType<string, ContentVote> {
  async $preload() {
    ...
    if (!vote) {
      throw new PreloadNotFoundError(`Review content vote with ID ${this.$props} not found`);
    }
    return vote;
  }
...
export class ContentReportResolver extends ReviewsType<string, ContentReport> {
  async $preload() {
    ...
    if (!report) {
      throw new PreloadNotFoundError(`Review content report with ID ${this.$props} not found`);
    }
    return report;
  }
