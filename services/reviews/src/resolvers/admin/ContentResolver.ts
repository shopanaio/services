...
export abstract class ContentResolver<TData = unknown> extends ReviewsType<string, TData> {
  protected async loadContent(): Promise<ContentItem> {
    ...
    if (!content) {
      throw new PreloadNotFoundError(`Review content with ID ${this.$props} not found`);
    }
    return content;
  }
  ...
  async sourceChannel() {
    return (await this.loadContent()).sourceChannel;
  }
  async sourceMetadata() {
    return (await this.loadContent()).sourceMetadata;
  }
  async idempotencyKey() {
    return (await this.loadContent()).idempotencyKey;
  }
  async status() {
    return (await this.loadContent()).status;
  }
  async moderationNote() {
    return (await this.loadContent()).moderationNote;
  }
  async moderatedByPrincipalId() {
    return (await this.loadContent()).moderatedByPrincipalId;
  }
  async moderatedAt() {
    return (await this.loadContent()).moderatedAt;
  }
  async publishedAt() {
    return (await this.loadContent()).publishedAt;
  }
  async unpublishedAt() {
    return (await this.loadContent()).unpublishedAt;
  }
  async revision() {
  ...
  async deletedAt() {
    return (await this.loadContent()).deletedAt;
  }
  async redactedAt() {
    return (await this.loadContent()).redactedAt;
  }
  ...
  votes(args: RelayPaginationInput) {
    return new ContentVoteConnectionResolver({ ...args, contentId: this.$props }, this.$ctx);
  }

  reports(args: RelayPaginationInput) {
    return new ContentReportConnectionResolver({ ...args, contentId: this.$props }, this.$ctx);
  }

  moderationCases(args: RelayPaginationInput) {
    return new ModerationCaseConnectionResolver({ ...args, contentId: this.$props }, this.$ctx);
  }

  moderationEvents(args: RelayPaginationInput) {
    return new ModerationEventConnectionResolver({ ...args, contentId: this.$props }, this.$ctx);
  }

  revisions(args: RelayPaginationInput) {
    return new ContentRevisionConnectionResolver({ ...args, contentId: this.$props }, this.$ctx);
  }

  moderationSignals(args: RelayPaginationInput) {
    return new ModerationSignalConnectionResolver({ ...args, contentId: this.$props }, this.$ctx);
  }
...
export class ContentAuthorResolver extends ReviewsType<ContentItem> {
  ...
  principalId() {
    return this.$props.authorPrincipalId;
  }
  displayName() {
...
export class ContentTranslationResolver extends ReviewsType<string, ContentTranslation> {
  async $preload() {
    ...
    if (!row) {
      throw new PreloadNotFoundError(`Review content translation with ID ${this.$props} not found`);
    }
    return row;
  }
  ...
  reviewedByPrincipalId() {
    return this.$get("reviewedByPrincipalId");
  }
  reviewedAt() {
...
export class ContentPublicationResolver extends ReviewsType<string, ContentPublication> {
  async $preload() {
    ...
    if (!row) {
      throw new PreloadNotFoundError(`Review content publication with ID ${this.$props} not found`);
    }
    return row;
  }
