...
export class RatingCriterionResolver extends ReviewsType<string, RatingCriterion> {
  async $preload() {
    ...
    if (!criterion) {
      throw new PreloadNotFoundError(
        `Review rating criterion with ID ${this.$props} not found`
      );
    }
    return criterion;
  }
  ...
  async translations() {
    const rows = await this.$ctx.loaders.ratingCriterionTranslations.load(
      this.$props
    );
    return rows.map(
  ...
  async assignments() {
    const rows = await this.$ctx.loaders.ratingCriterionAssignments.load(
      this.$props
    );
    ...
    return rows.map(
      (row) => new RatingCriterionAssignmentResolver(row.id, this.$ctx)
    );
  }
}
...
> {
  async $preload() {
    const assignment =
      await this.$ctx.loaders.ratingCriterionAssignment.load(this.$props);
    if (!assignment) {
      throw new PreloadNotFoundError(
  ...
  isRequiredOverride() { return this.$get("isRequiredOverride"); }
  sortIndexOverride() { return this.$get("sortIndexOverride"); }
  createdAt() { return this.$get("createdAt"); }
}
