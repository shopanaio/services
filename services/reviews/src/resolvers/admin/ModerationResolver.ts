...
export class ModerationCaseResolver extends ReviewsType<string, ModerationCase> {
  async $preload() {
    const moderationCase = await this.$ctx.loaders.moderationCase.load(this.$props);
    if (!moderationCase) {
      throw new PreloadNotFoundError(`Review moderation case with ID ${this.$props} not found`);
    }
  ...
  assignedToPrincipalId() {
    return this.$get("assignedToPrincipalId");
  }
  dueAt() {
  ...
  resolvedByPrincipalId() {
    return this.$get("resolvedByPrincipalId");
  }
  resolvedAt() {
...
export class ModerationEventResolver extends ReviewsType<string, ModerationEvent> {
  async $preload() {
    ...
    if (!event) {
      throw new PreloadNotFoundError(`Review moderation event with ID ${this.$props} not found`);
    }
    return event;
  }
...
export class ContentRevisionResolver extends ReviewsType<string, ContentRevision> {
  async $preload() {
    ...
    if (!revision) {
      throw new PreloadNotFoundError(`Review content revision with ID ${this.$props} not found`);
    }
    return revision;
  }
  ...
  changedByType() {
    return this.$get("changedByType");
  }
  changedById() {
    return this.$get("changedById");
  }
  changeReason() {
...
export class ModerationSignalResolver extends ReviewsType<string, ModerationSignal> {
  async $preload() {
    ...
    if (!signal) {
      throw new PreloadNotFoundError(`Review moderation signal with ID ${this.$props} not found`);
    }
    return signal;
  }
