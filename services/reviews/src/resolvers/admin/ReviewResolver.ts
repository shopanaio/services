...
import { fileReference, productReference, variantReference } from "./references.js";
...
export class ReviewResolver extends ContentResolver<Review> {
  async $preload() {
    ...
    if (!review) {
      throw new PreloadNotFoundError(`Review with ID ${this.$props} not found`);
    }
    return review;
  }
...
export class ReviewMediaResolver extends ReviewsType<string, ReviewMedia> {
  async $preload() {
    ...
    if (!media) {
      throw new PreloadNotFoundError(`Review media with ID ${this.$props} not found`);
    }
    return media;
  }
  ...
  async file() {
    return fileReference(await this.$get("fileId"));
  }
  sortIndex() {
  ...
  moderatedByPrincipalId() {
    return this.$get("moderatedByPrincipalId");
  }
  moderatedAt() {
    return this.$get("moderatedAt");
  }
  createdAt() {
...
export class ReviewReplyResolver extends ContentResolver<ReviewReply> {
  async $preload() {
    ...
    if (!reply) {
      throw new PreloadNotFoundError(`Review reply with ID ${this.$props} not found`);
    }
    return reply;
  }
