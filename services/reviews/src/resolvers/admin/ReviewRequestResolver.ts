...
export class ReviewRequestResolver extends ReviewsType<string, ReviewRequest> {
  async $preload() {
    ...
    if (!request) {
      throw new PreloadNotFoundError(`Review request with ID ${this.$props} not found`);
    }
    return request;
  }
  ...
  async orderId() {
    return this.encodeId(await this.$get("orderId"), GlobalIdEntity.Order);
  }
  async orderLineId() {
    return this.encodeId(await this.$get("orderLineId"), GlobalIdEntity.OrderLine);
  }
  async product() {
...
export class ReviewRequestEventResolver extends ReviewsType<string, ReviewRequestEvent> {
  async $preload() {
    ...
    if (!event) {
      throw new PreloadNotFoundError(`Review request event with ID ${this.$props} not found`);
    }
    return event;
  }
