...
export const productQuestion = reviewsSchema.table(
  "product_question",
  {
    ...
    contentKind: contentKindEnum("content_kind").notNull().default("PRODUCT_QUESTION"),
    storeId: uuid("store_id").notNull(),
    productId: uuid("product_id").notNull(),
    variantId: uuid("variant_id"),
  },
  (table) => [
    index("product_question_store_product_idx").on(table.storeId, table.productId, table.id),
...
export const questionAnswer = reviewsSchema.table(
  "question_answer",
  {
    id: uuid("id")
      .primaryKey()
      .references(() => contentItem.id, { onDelete: "cascade" }),
    contentKind: contentKindEnum("content_kind").notNull().default("QUESTION_ANSWER"),
    storeId: uuid("store_id").notNull(),
  ...
  (table) => [
    ...
    index("question_answer_store_official_idx").on(table.storeId, table.isOfficial, table.id),
  ],
);
...
export const questionSubscription = reviewsSchema.table(
  ...
  (table) => [
    unique("question_subscription_unique").on(table.questionId, table.subscriberKey, table.channel),
    index("question_subscription_store_customer_idx")
      .on(table.storeId, table.subscriberCustomerId, table.status, table.id)
