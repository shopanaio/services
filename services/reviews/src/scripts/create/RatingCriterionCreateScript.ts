...
import type { RatingCriterionCreateParams, RatingCriterionCreateResult } from "./types.js";
...
> {
  ...
  protected async execute(
    ...
    if (!title) {
      errors.push({
        message: "Criterion title cannot be empty",
        code: "INVALID_TITLE",
        field: ["defaultTitle"],
      });
    }
    ...
    for (const [index, translation] of (params.translations ?? []).entries()) {
      if (!translation.locale.trim() || !translation.title.trim()) {
        errors.push({
          message: "Translation locale and title cannot be empty",
          code: "INVALID_TRANSLATION",
          field: ["translations", String(index)],
        });
      }
      ...
      if (translationLocales.has(locale)) {
        errors.push({
          ...
          field: ["translations", String(index), "locale"],
        });
      }
    ...
    for (const [index, assignment] of (params.assignments ?? []).entries()) {
      if (
        assignment.sortIndexOverride !== undefined &&
        assignment.sortIndexOverride !== null &&
        assignment.sortIndexOverride < 0
      ) {
        errors.push({
          message: "Assignment sort index cannot be negative",
          code: "INVALID_SORT_INDEX",
          field: ["assignments", String(index), "sortIndexOverride"],
        });
      }
      ...
      if (assignmentKeys.has(key)) {
        errors.push({
          ...
          field: ["assignments", String(index), "targetId"],
        });
      }
