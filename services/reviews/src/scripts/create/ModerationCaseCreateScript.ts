...
import type { ModerationCaseCreateParams, ModerationCaseCreateResult } from "./types.js";
...
> {
  @Transactional()
  protected async execute(params: ModerationCaseCreateParams): Promise<ModerationCaseCreateResult> {
    ...
    try {
      ...
      if (isUniqueViolation(error, "moderation_case_active_content_unique")) {
        return {
          userErrors: [
            {
              message: "This content already has an active moderation case",
