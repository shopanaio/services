...
export function mapContentCreate(
  ...
  const minimumBodyLength =
    kind === "REVIEW" ? 20 : kind === "PRODUCT_QUESTION" || kind === "QUESTION_ANSWER" ? 10 : 1;
  ...
  if (displayName.length === 0 || displayName.length > 150) {
    errors.push({
      message: "Author display name must contain between 1 and 150 characters",
      code: "INVALID_AUTHOR",
      field: ["content", "author", "displayName"],
    });
  }
  if (input.author.type === "CUSTOMER" && !input.author.customerId) {
    errors.push({
      ...
      code: "INVALID_AUTHOR",
      field: ["content", "author", "customerId"],
    });
  }
  ...
  if (status === "REJECTED" && !moderationNote) {
    errors.push({
      ...
      field: ["content", "moderationNote"],
    });
  }
  if (sourceChannel.length > 64) {
    errors.push({
      message: "Source channel cannot exceed 64 characters",
      code: "INVALID_SOURCE",
      field: ["content", "source", "channel"],
    });
  }
