...
type NotificationWebhook = NonNullable<
  Awaited<ReturnType<WebhookRepository["find"]>>
>;

export class NotificationWebhookResolver extends NotificationsType<
  string,
  NotificationWebhook
> {
  async $preload() {
  ...
  apiVersion() { return this.$get("apiVersion"); }
  async status() {
