      ...
      await this.connection
        ...
        .where(
          and(
            ...
            inArray(notificationDeliveryAttempts.status, [
              "UNKNOWN",
              "ACCEPTED",
            ])
          )
    ...
    await this.connection
      ...
      .where(
        and(
          eq(notificationDeliveries.storeId, this.storeId),
          eq(notificationDeliveries.id, input.deliveryId),
          inArray(notificationDeliveries.status, ["UNKNOWN", "ACCEPTED"])
        )
      );
  ...
  async reconcileFailure(input: {
    ...
    if (attempt) {
      await this.connection
        ...
        .set({
          status: "FAILED",
          errorKind: input.errorKind ?? "PERMANENT",
    ...
    await this.connection
      ...
      .set({
        status: "DEAD",
        nextAttemptAt: null,
        lastErrorKind: input.errorKind ?? "PERMANENT",
